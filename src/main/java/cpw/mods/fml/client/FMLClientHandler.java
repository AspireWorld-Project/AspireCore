 ...
 *
 * The FML entry point is {@link #beginMinecraftLoading(Minecraft, List)} called
 * from {@link Minecraft}
 *
...
public class FMLClientHandler implements IFMLSidedHandler {
	 ...
	 *
	 * @param minecraft
	 *            The minecraft instance being launched
	 * @param resourcePackList
	 *            The resource pack list we will populate with mods
	 * @param resourceManager
	 *            The resource manager
	 */
	@SuppressWarnings("unchecked")
	public void beginMinecraftLoading(Minecraft minecraft, @SuppressWarnings("rawtypes") List resourcePackList,
		...
		resourcePackMap = Maps.newHashMap();
		if (minecraft.isDemo()) {
	...
	private void detectOptifine() {
		try {
			...
			ModMetadata optifineMetadata = MetadataCollection
					.from(getClass().getResourceAsStream("optifinemod.info"), "optifine")
	...
	/**
	 * Called a bit later on during initialization to finish loading mods Also
	...
	public void queryUser(StartupQuery query) throws InterruptedException {
		...
		if (query.isSynchronous()) {
			...
			client.loadingScreen.resetProgresAndWorkingMessage(""); // make sure the blank screen is being drawn at the
																	// end
		}
	}
	...
	/**
	 * If the client is in the midst of loading, we disable saving so that custom
	 * settings aren't wiped out
	 */
	...
	public void addModAsResource(ModContainer container) {
		...
		if (resourcePackType != null) {
			try {
				...
				FMLLog.log(Level.ERROR, "The container %s (type %s) returned an invalid class for it's resource pack.",
	...
	public void serverStopped() {
		// If the server crashes during startup, it might hang the client- reset the
		// client so it can abend properly.
		MinecraftServer server = getServer();

		if (server != null && !server.serverIsInRunLoop()) {
			ObfuscationReflectionHelper.setPrivateValue(MinecraftServer.class, server, true, "field_71296" + "_Q",
					"serverIs" + "Running");
		}
	}
	...
	public NetworkManager getClientToServerNetworkManager() {
		return client.getNetHandler() != null ? client.getNetHandler().getNetworkManager() : null;
	}
	...
	public void tryLoadExistingWorld(GuiSelectWorld selectWorldGUI, String dirName, String saveName) {
		...
		try {
			...
			try {
				...
				FMLLog.warning("There appears to be a problem loading the save %s, both level files are unreadable.",
						dirName);
				return;
			}
		}
	...
	public void bindServerListData(ServerData data, ServerStatusResponse originalResponse) {
		if (extraServerListData.containsKey(originalResponse)) {
			...
			boolean moddedClientAllowed = jsonData.has("clientModsAllowed")
					? jsonData.get("clientModsAllowed").getAsBoolean()
					: true;
			...
			serverDataTag.put(data, new ExtendedServerListData(type,
					FMLNetworkHandler.checkModList(modListMap, Side.SERVER) == null, modListMap, !moddedClientAllowed));
		} else {
			...
			serverDataTag.put(data, new ExtendedServerListData("VANILLA", false, ImmutableMap.<String, String>of(),
					!moddedClientAllowed));
		}
	...
	public String enhanceServerListEntry(ServerListEntryNormal serverListEntry, ServerData serverEntry, int x,
		...
		if (serverDataTag.containsKey(serverEntry)) {
			...
			return null;
		client.getTextureManager().bindTexture(iconSheet);
		Gui.func_146110_a(x + width - 18, y + 10, 0, idx, 16, 16, 256.0f, 256.0f);
		if (blocked) {
	...
	public void setPlayClient(NetHandlerPlayClient netHandlerPlayClient) {
		...
		currentPlayClient = new WeakReference<>(netHandlerPlayClient);
	}
	...
	public void fireNetRegistrationEvent(EventBus bus, NetworkManager manager, Set<String> channelSet, String channel,
		...
		if (side == Side.CLIENT) {
			waitForPlayClient();
			bus.post(new FMLNetworkEvent.CustomPacketRegistrationEvent<>(manager, channelSet, channel, side,
	...
	public void logMissingTextureErrors() {
		...
		for (String resourceDomain : missingTextures.keySet()) {
			...
			logger.error(Strings.repeat("-", 50));
			logger.error("  domain {} is missing {} texture{}", resourceDomain, missing.size(),
					missing.size() != 1 ? "s" : "");
			...
			if (fallbackResourceManager == null) {
				...
				List<IResourcePack> resPacks = ObfuscationReflectionHelper.getPrivateValue(
						FallbackResourceManager.class, fallbackResourceManager, "resourcePacks", "field_110540" + "_a");
				logger.error("    domain {} has {} location{}:", resourceDomain, resPacks.size(),
						resPacks.size() != 1 ? "s" : "");
				for (IResourcePack resPack : resPacks) {
					if (resPack instanceof FMLContainerHolder) {
						...
						logger.error("      mod {} resources at {}", fmlContainer.getModId(),
								fmlContainer.getSource().getPath());
					} else if (resPack instanceof AbstractResourcePack) {
						AbstractResourcePack resourcePack = (AbstractResourcePack) resPack;
						File resPath = ObfuscationReflectionHelper.getPrivateValue(AbstractResourcePack.class,
								resourcePack, "resourcePackFile", "field_110597" + "_b");
						logger.error("      resource pack at path {}", resPath.getPath());
					} else {
						logger.error("      unknown resourcepack type {} : {}", resPack.getClass().getName(),
								resPack.getPackName());
					}
				}
			}
	...
	public void processWindowMessages() {
		...
		if (LWJGLUtil.getPlatform() != LWJGLUtil.PLATFORM_WINDOWS)
			return;
		// If we can't grab the mutex, the update call is blocked, probably in native
		// code, just skip it and carry on
