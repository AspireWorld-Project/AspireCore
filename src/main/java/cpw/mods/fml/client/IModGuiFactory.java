...
public interface IModGuiFactory {
	 ...
	 * instantiated when the "config" button is pressed in the mod list. It will
	 ...
	 * friendly visual way. It should not be abused to set internals such as IDs
	 * (they're gonna keep disappearing anyway), but rather, interesting behaviours.
	 * This config GUI is never run when a server game is running, and should be
	 * used to configure desired behaviours that affect server state. Costs, mod
	 * game modes, stuff like that can be changed here.
	 *
	 ...
	 * special parent 'Help' that will always list first, and is generally meant to
	 * provide Help type content for mods. The remaining parents will sort
	 * alphabetically, though this may change if there is a lot of alphabetic abuse.
	 * "AAA" is probably never a valid category parent.
	 *
	 * Runtime configuration itself falls into two flavours: in-game help, which is
	 * generally non interactive except for the text it wishes to show, and
	 * client-only affecting behaviours. This would include things like toggling
	 * minimaps, or cheat modes or anything NOT affecting the behaviour of the
	 * server. Please don't abuse this to change the state of the server in any way,
	 * this is intended to behave identically when the server is local or remote.
	 *
	 ...
	 * the right hand side option screen for the specified
	...
	/**
	 * Responsible for painting the mod specific section of runtime options GUI for
