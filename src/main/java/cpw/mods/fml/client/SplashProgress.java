...
public class SplashProgress {
	...
	public static void start() {
		...
		// Enable if we have the flag, and there's either no optifine, or optifine has
		// added a key to the blackboard ("optifine.ForgeSplashCompatible")
		// Optifine authors - add this key to the blackboard if you feel your
		// modifications are now compatible with this code.
		enabled = getBool("enabled", true) && (!FMLClientHandler.instance().hasOptifine()
				|| Launch.blackboard.containsKey("optifine.ForgeSplashCompatible"));
		...
		final ResourceLocation logoLoc = new ResourceLocation(
				getString("logoTexture", "textures/gui/title/mojang.png"));
		...
		FMLCommonHandler.instance().registerCrashCallable(new ICrashCallable() {
			...
			public String call() throws Exception {
				return "' Vendor: '" + glGetString(GL_VENDOR) + "' Version: '" + glGetString(GL_VERSION)
		...
		thread = new Thread(new Runnable() {
			...
			private void setGL() {
				...
				glClearColor((float) (backgroundColor >> 16 & 0xFF) / 0xFF,
						(float) (backgroundColor >> 8 & 0xFF) / 0xFF, (float) (backgroundColor & 0xFF) / 0xFF, 1);
	...
	private static class Texture {
		...
		public Texture(ResourceLocation location) {
			...
			try {
				...
				glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size, size, 0, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
						(IntBuffer) null);
				...
				for (int i = 0; i * (size / width) < frames; i++) {
					for (int j = 0; i * (size / width) + j < frames && j < size / width; j++) {
						...
						glTexSubImage2D(GL_TEXTURE_2D, 0, j * width, i * height, width, height, GL_BGRA,
