...
/**
 * This class is the base GuiScreen for all config GUI screens. It can be
 * extended by mods to provide the top-level config screen that will be called
 * when the Config button is clicked from the Main Menu Mods list.
 *
 * @author bspkrs
 */
public class GuiConfig extends GuiScreen {
	...
	/**
	 * When set to a non-null value the OnConfigChanged and PostConfigChanged events
	 * will be posted when the Done button is pressed if any configElements were
	 * changed (includes child screens). If not defined, the events will be posted
	 * if the parent gui is null or if the parent gui is not an instance of
	 * GuiConfig.
	 */
	...
	/**
	 * GuiConfig constructor that will use ConfigChangedEvent when editing is
	 * concluded. If a non-null value is passed for configID, the OnConfigChanged
	 * and PostConfigChanged events will be posted when the Done button is pressed
	 * if any configElements were changed (includes child screens). If configID is
	 * not defined, the events will be posted if the parent gui is null or if the
	 * parent gui is not an instance of GuiConfig.
	 *
	 * @param parentScreen
	 *            the parent GuiScreen object
	 * @param configElements
	 *            a List of IConfigProperty objects
	 * @param modID
	 *            the mod ID for the mod whose config settings will be edited
	 * @param configID
	 *            an identifier that will be passed to the OnConfigChanged and
	 *            PostConfigChanged events. Setting this value will force the save
	 *            action to be called when the Done button is pressed on this screen
	 *            if any configElements were changed.
	 * @param allRequireWorldRestart
	 *            send true if all configElements on this screen require a world
	 *            restart
	 * @param allRequireMcRestart
	 *            send true if all configElements on this screen require MC to be
	 *            restarted
	 * @param title
	 *            the desired title for this screen. For consistency it is
	 *            recommended that you pass the path of the config file being
	 *            edited.
	 */
	@SuppressWarnings("rawtypes")
	...
	/**
	 * GuiConfig constructor that will use ConfigChangedEvent when editing is
	 * concluded. This constructor passes null for configID. If configID is not
	 * defined, the events will be posted if the parent gui is null or if the parent
	 * gui is not an instance of GuiConfig.
	 *
	 * @param parentScreen
	 *            the parent GuiScreen object
	 * @param configElements
	 ...
	 *            edited.
	 * @param titleLine2
	 *            the desired title second line for this screen. Typically this is
	 *            used to send the category name of the category currently being
	 *            edited.
	 */
	@SuppressWarnings("rawtypes")
	public GuiConfig(GuiScreen parentScreen, List<IConfigElement> configElements, String modID,
	...
	/**
	 * GuiConfig constructor that will use ConfigChangedEvent when editing is
	 * concluded. titleLine2 is specified in this constructor. If a non-null value
	 * is passed for configID, the OnConfigChanged and PostConfigChanged events will
	 * be posted when the Done button is pressed if any configElements were changed
	 * (includes child screens). If configID is not defined, the events will be
	 * posted if the parent gui is null or if the parent gui is not an instance of
	 * GuiConfig.
	 *
	 * @param parentScreen
	 *            the parent GuiScreen object
	...
	public GuiConfig(GuiScreen parentScreen, List<IConfigElement> configElements, String modID, String configID,
		...
		initEntries = new ArrayList<>(entryList.listEntries);
	...
	public static String getAbridgedConfigPath(String path) {
		...
		if (mc.mcDataDir.getAbsolutePath().endsWith("."))
			return path.replace("\\", "/").replace(mc.mcDataDir.getAbsolutePath().replace("\\", "/").substring(0,
					mc.mcDataDir.getAbsolutePath().length() - 1), "/.minecraft/");
		else
	...
	public void initGui() {
		...
		buttonList.add(new GuiButtonExt(2000, width / 2 - buttonWidthHalf, height - 29, doneWidth, 20,
				I18n.format("gui.done")));
		buttonList.add(btnDefaultAll = new GuiUnicodeGlyphButton(2001,
		...
		buttonList.add(chkApplyGlobally = new GuiCheckBox(2003,
				width / 2 - buttonWidthHalf + doneWidth + 5 + undoWidth + 5 + resetWidth + 5, height - 24,
				I18n.format("fml.configgui.applyGlobally"), false));
	...
	public void onGuiClosed() {
		...
		if (configID != null && parentScreen instanceof GuiConfig) {
			GuiConfig parentGuiConfig = (GuiConfig) parentScreen;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		btnUndoAll.enabled = entryList.areAnyEntriesEnabled(chkApplyGlobally.isChecked())
				&& entryList.hasChangedEntry(chkApplyGlobally.isChecked());
		...
		if (undoHoverChecker.checkHover(mouseX, mouseY)) {
			drawToolTip(mc.fontRenderer.listFormattedStringToWidth(I18n.format("fml.configgui.tooltip.undoAll"), 300),
					mouseX, mouseY);
		}
		if (resetHoverChecker.checkHover(mouseX, mouseY)) {
			drawToolTip(mc.fontRenderer.listFormattedStringToWidth(I18n.format("fml.configgui.tooltip.resetAll"), 300),
					mouseX, mouseY);
		}
		if (checkBoxHoverChecker.checkHover(mouseX, mouseY)) {
			drawToolTip(
					mc.fontRenderer.listFormattedStringToWidth(I18n.format("fml.configgui.tooltip.applyGlobally"), 300),
					mouseX, mouseY);
		}
	}
