...
/**
 * RuntimeException that gives subclasses the simple opportunity to write extra
 * data when printing the stack trace. Mainly a helper class as printsStackTrace
...
public abstract class EnhancedRuntimeException extends RuntimeException {
	...
	public String getMessage() {
		...
		if (stack.length > 2 && stack[2].getClassName().startsWith("org.apache.logging.log4j.")) {
			// This is a bit of a hack to force ourselves to be able to give a extended
			// description when log4j prints this out.
			// Sadly this text is displayed AFTER the initial exception line, and before the
			// stack trace. But as the intention
