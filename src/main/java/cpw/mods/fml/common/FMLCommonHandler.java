 ...
 *
 * Anything that doesn't require obfuscated or client/server specific code
 * should go in this handler
 *
 * It also contains a reference to the sided handler instance that is valid
 * allowing for common code to access specific properties from the obfuscated
 * world without a direct dependency
 *
 * @author cpw
 *
 */
public class FMLCommonHandler {
	...
	/**
	 * Get the forge mod loader logging instance (goes to the forgemodloader log
	 * file)
	 *
	...
	/**
	 * Return the effective side for the context in the game. This is dependent on
	 * thread analysis to try and determine whether the code is running in the
	...
	public void computeBranding() {
		if (brandings == null) {
			...
			brd.add(String.format("%d mod%s loaded, %d mod%s active", tModCount, tModCount != 1 ? "s" : "", aModCount,
					aModCount != 1 ? "s" : ""));
			brandings = brd.build();
	...
	/**
	 * Used to exit from java, with system exit preventions in place. Will be tidy
	 * about it and just log a message, unless debugging is enabled
	 *
	 * @param exitCode
	 *            The exit code
	 * @param hardExit
	 *            Perform a halt instead of an exit (only use when the world is
	 *            unsavable) - read the warnings at {@link Runtime#halt(int)}
	 */
