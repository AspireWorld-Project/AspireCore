	...
	public void readData(SaveHandler handler, WorldInfo info, Map<String, NBTBase> propertyMap, NBTTagCompound tag) {
		if (tag.hasKey("ModList")) {
			...
			for (int i = 0; i < modList.tagCount(); i++) {
				...
				if (container == null) {
					FMLLog.log("fml.ModTracker", Level.ERROR,
		...
		if (tag.hasKey("ModItemData")) {
			...
			for (int i = 0; i < modList.tagCount(); i++) {
				...
				if (forcedName == null) {
					FMLLog.warning(
							"Found unlabelled item in world save, this may cause problems. The item type %s:%d will not be present",
							itemType, ordinal);
				} else {
					// all entries are Items, blocks were only saved through their ItemBlock
					String itemLabel = String.format("%c%s:%s", '\u0002', forcedModId != null ? forcedModId : modId,
							forcedName);
			...
			try {
				failedElements = GameData.injectWorldIDMap(dataList, blockedIds, blockAliases, itemAliases,
						blockSubstitutions, itemSubstitutions, true, true);
			} catch (IllegalStateException ex) {
				// In the case of IllegalArgumentException the state map is utterly toast. We
				// should immediately abort
