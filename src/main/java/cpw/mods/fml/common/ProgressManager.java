...
public class ProgressManager {
	...
	public static void pop(ProgressBar bar) {
		...
		if (bar.getSteps() != 0) {
			...
			if (bar.timeEachStep) {
				FMLLog.fine("Bar Step: %s - %s took %.3fs", bar.getTitle(), bar.getMessage(),
						(float) (newTime - bar.lastTime) / 1000000 / 1000);
			}
			if (bar.getSteps() == 1) {
				FMLLog.fine("Bar Finished: %s - %s took %.3fs", bar.getTitle(), bar.getMessage(),
						(float) (newTime - bar.startTime) / 1000000 / 1000);
			} else {
				FMLLog.fine("Bar Finished: %s took %.3fs", bar.getTitle(),
						(float) (newTime - bar.startTime) / 1000000 / 1000);
			}
		}
		FMLCommonHandler.instance().processWindowMessages();
	...
	public static class ProgressBar {
		...
		public void step(String message) {
			...
			if (timeEachStep && step != 0) {
				long newTime = System.nanoTime();
				FMLLog.fine("Bar Step: %s - %s took %.3fs", getTitle(), getMessage(),
