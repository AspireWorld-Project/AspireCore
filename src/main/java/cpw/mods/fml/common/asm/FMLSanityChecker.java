...
public class FMLSanityChecker implements IFMLCallHook {
	...
	public Void call() throws Exception {
			...
			if (certificates != null) {
				...
				for (Certificate cert : certificates) {
					...
					if (fingerprint.equals(FMLFINGERPRINT)) {
						...
						FMLRelaunchLog.info("Found valid fingerprint for Minecraft Forge. Certificate fingerprint %s",
								fingerprint);
						goodFML = true;
					} else {
		...
		// Server is not signed, so assume it's good - a deobf env is dev time so it's
		// good too
