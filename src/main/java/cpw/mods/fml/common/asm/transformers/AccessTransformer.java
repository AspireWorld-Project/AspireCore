	...
	public byte[] transform(String name, String transformedName, byte[] bytes) {
		...
		for (Modifier m : mods) {
			if (m.modifyClassVisibility) {
				...
				if (DEBUG) {
					System.out.println(
							String.format("Class: %s %s -> %s", name, toBinary(m.oldAccess), toBinary(m.newAccess)));
				}
				continue;
			}
			if (m.desc.isEmpty()) {
				...
				for (MethodNode n : classNode.methods) {
					if (n.name.equals(m.name) && n.desc.equals(m.desc) || m.name.equals("*")) {
						...
						if (!n.name.equals("<init>")) {
							// if we changed from private to something else we need to replace all
							// INVOKESPECIAL calls to this method with INVOKEVIRTUAL
							// so that overridden methods will be called. Only need to scan this class,
							// because obviously the method was private.
