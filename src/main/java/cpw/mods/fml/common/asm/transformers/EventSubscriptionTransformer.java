...
public class EventSubscriptionTransformer implements IClassTransformer {
	...
	public byte[] transform(String name, String transformedName, byte[] bytes) {
		if (bytes == null || name.equals("cpw.mods.fml.common.eventhandler.Event") || name.startsWith("net.minecraft.")
				|| name.indexOf('.') == -1)
			return bytes;
	...
	private boolean buildEvents(ClassNode classNode) throws Exception {
		// Yes, this recursively loads classes until we get this base class. THIS IS NOT
		// A ISSUE. Coremods should handle re-entry just fine.
		// If they do not this a COREMOD issue NOT a Forge/LaunchWrapper issue.
		Class<?> parent = this.getClass().getClassLoader().loadClass(classNode.superName.replace('/', '.'));
		if (!Event.class.isAssignableFrom(parent))
			return false;

		// Class<?> listenerListClazz =
		// Class.forName("cpw.mods.fml.common.eventhandler.ListenerList", false,
		...
		/*
		 * Add: protected void setup() { super.setup(); if (LISTENER_LIST != NULL) {
		 * return; } LISTENER_LIST = new ListenerList(super.getListenerList()); }
		 */
		...
		method.instructions.add(new MethodInsnNode(INVOKESPECIAL, tList.getInternalName(), "<init>",
				getMethodDescriptor(VOID_TYPE, tList), false));
