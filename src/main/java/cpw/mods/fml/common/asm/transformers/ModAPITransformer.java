...
public class ModAPITransformer implements IClassTransformer {
	...
	private void stripMethod(ClassNode classNode, String methodDescriptor) {
		if (classNode.name.endsWith("$class")) {
			...
			methodDescriptor = methodDescriptor.substring(0, pos) + 'L' + subName + ';'
	...
	private void stripInterface(ClassNode classNode, String interfaceName, boolean stripRefs) {
		...
		if (found && stripRefs) {
			if (logDebugInfo) {
				FMLRelaunchLog.finer("Optional removal - interface %s - stripping method signature references",
						interfaceName);
			}
				...
				if (node.desc.contains(ifaceName)) {
					if (logDebugInfo) {
						FMLRelaunchLog.finer(
								"Optional removal - interface %s - stripping method containing reference %s",
			...
			if (logDebugInfo) {
				FMLRelaunchLog.finer("Optional removal - interface %s - all method signature references stripped",
						interfaceName);
			}
		} else if (found) {
			if (logDebugInfo) {
				FMLRelaunchLog.finer("Optional removal - interface %s - NOT stripping method signature references",
						interfaceName);
			}
		}
	}
