...
public class ModAccessTransformer extends AccessTransformer {
	...
	public ModAccessTransformer() throws Exception {
		...
		// We are in the new ClassLoader here, so we need to get the static field from
		// the other ClassLoader.
		ClassLoader classLoader = this.getClass().getClassLoader().getClass().getClassLoader(); // Bit odd but it gets
