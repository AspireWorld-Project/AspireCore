...
public class ASMModParser {
	...
	public void beginNewTypeName(String typeQName, int classVersion, String superClassQName) {
		asmType = Type.getObjectType(typeQName);
		this.classVersion = classVersion;
		asmSuperType = !Strings.isNullOrEmpty(superClassQName) ? Type.getObjectType(superClassQName) : null;
	}

	public void startClassAnnotation(String annotationName) {
		ModAnnotation ann = new ModAnnotation(AnnotationType.CLASS, Type.getType(annotationName),
	...
	public void validate() {
		...
		// {
		//
		// throw new LoaderException(new RuntimeException("Mod compiled for Java 7
		// detected"));
		// }
	}

	public boolean isBaseMod(List<String> rememberedTypes) {
		return getASMSuperType().equals(Type.getType("LBaseMod;"))
				|| getASMSuperType().equals(Type.getType("Lnet/minecraft/src/BaseMod;"))
				|| rememberedTypes.contains(getASMSuperType().getClassName());
	}

	public void setBaseModProperties(String foundProperties) {
		baseModProperties = foundProperties;
	}

	public String getBaseModProperties() {
		return baseModProperties;
	}

	public void sendToTable(ASMDataTable table, ModCandidate candidate) {
		for (ModAnnotation ma : annotations) {
			table.addASMData(candidate, ma.asmType.getClassName(), asmType.getClassName(), ma.member, ma.values);
		}
	}
	...
	public void startMethodAnnotation(String methodName, String methodDescriptor, String annotationName) {
		ModAnnotation ann = new ModAnnotation(AnnotationType.METHOD, Type.getType(annotationName),
