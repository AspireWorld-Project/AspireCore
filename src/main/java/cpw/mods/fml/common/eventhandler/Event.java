...
public class Event {
	...
	/**
	 * Sets the state of this event, not all events are cancelable, and any attempt
	 * to cancel a event that can't be will result in a IllegalArgumentException.
	 *
	...
	/**
	 * Called by the base constructor, this is used by ASM generated event classes
	 * to setup various functionality such as the listenerlist.
	 */
	protected void setup() {
	}

	/**
	 * Returns a ListenerList object that contains all listeners that are registered
	 * to this event.
	 *
	...
	public void setPhase(@Nonnull EventPriority value) {
		...
		Preconditions.checkArgument(prev < value.ordinal(), "Attempted to set event phase to %s when already %s", value,
				phase);
		phase = value;
	}
}