...
public class CollectionWrapperFactory {
	/**
	 * Return a read only cast view of the supplied ungeneric collection, based on
	 * the element type given
	 *
	 * @param coll
	 *            The collection to cast
	 * @param elementType
	 *            the supertype contained in the collection
	...
	public static <T> Collection<T> wrap(@SuppressWarnings("rawtypes") Collection coll, Class<T> elementType) {
		...
		return Collections2.transform(asGeneric, new TypeCastFunction<>(elementType));
	}

	/**
	 * Return a read only cast view of the supplied ungeneric list, based on the
	 * element type given
	 *
	 * @param list
	 *            The list to cast
	...
	public static <T> List<T> wrap(@SuppressWarnings("rawtypes") List list, Class<T> elementType) {
		List<?> asGeneric = list;
		return Lists.transform(asGeneric, new TypeCastFunction<>(elementType));
	}
}