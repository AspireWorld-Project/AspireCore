...
public class ByteBufUtils {
	...
	/**
	 * Write an integer to the buffer using variable length encoding. The maxSize
	 * constrains how many bytes (and therefore the maximum number) that will be
	 * written.
	 *
	 * @param to
	...
	/**
	 * Read an {@link NBTTagCompound} from the byte buffer. It uses the minecraft
	 * encoding.
	 *
	 * @param from
	 *            The buffer to read from
	...
	public static String getContentDump(ByteBuf buffer) {
		...
		for (i = 0; i < currentLength; i++) {
			...
			returnString.append(Integer.toString((buffer.getByte(i) & 0xF0) >> 4, 16)
