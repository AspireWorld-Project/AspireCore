 ...
 * {@link FMLNetworkEvent.CustomPacketEvent} and
 * {@link FMLNetworkEvent.CustomNetworkEvent} to deliver messages to an event
 * listener. There is one "bus" for each channel, due to the impossibility of
 * filtering a bus for specific events.
 *
 * This event driven system completely wraps the netty code. Mod code deals with
 * FMLProxyPackets directly. It is not possible to enhance the netty pipeline,
 * and I would expect highly unexpected results if it were modified
 * reflectively. Use a real ChannelHandler if you want to use netty.
 *
 * @author cpw
 *
 */
public class FMLEventChannel {
	...
	FMLEventChannel(String name) {
		channels = NetworkRegistry.INSTANCE.newChannel(name, new NetworkEventFiringHandler(this));
		eventBus = new EventBus();
	}

	/**
	 * Register an event listener with this channel and bus. See
	 * {@link SubscribeEvent}
	 *
	 * @param object
	 */
