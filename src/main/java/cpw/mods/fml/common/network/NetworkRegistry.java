...
public enum NetworkRegistry {
	...
	public static final AttributeKey<INetHandler> NET_HANDLER = new AttributeKey<>("fml:netHandler");

	// Version 1: ServerHello only contains this value as a byte
	// Version 2: ServerHello additionally contains a 4 byte (int) dimension for the
	// logging in client
	 ...
	 * <ul>
	 * <li>{@link #newSimpleChannel(String)} provides {@link SimpleNetworkWrapper},
	 * a simple implementation of a netty handler, suitable for those who don't wish
	 * to dive too deeply into netty.
	 * <li>{@link #newEventChannel(String)} provides {@link FMLEventChannel} an
	 * event driven implementation, with lower level access to the network data
	 * stream, for those with advanced bitbanging needs that don't wish to poke
	 * netty too hard.
	 * <li>Alternatively, simply use the netty features provided here and implement
	 * the full power of the netty stack.
	 * </ul>
	 *
	 * There are two channels created : one for each logical side (considered as the
	 * source of an outbound message) The returned map will contain a value for each
	 * logical side, though both will only be working in the integrated server case.
	 *
	 * The channel expects to read and write using {@link FMLProxyPacket}. All
	 * operation is synchronous, as the asynchronous behaviour occurs at a lower
	 * level in netty.
	 *
	 * The first handler in the pipeline is special and should not be removed or
	 * moved from the head - it transforms packets from the outbound of this
	 * pipeline into custom packets, based on the current {@link AttributeKey} value
	 * {@link NetworkRegistry#FML_MESSAGETARGET} and
	 * {@link NetworkRegistry#FML_MESSAGETARGETARGS} set on the channel. For the
	 * client to server channel (source side : CLIENT) this is fixed as "TOSERVER".
	 * For SERVER to CLIENT packets, several possible values exist.
	 *
	 * Mod Messages should be transformed using a something akin to a
	 * {@link MessageToMessageCodec}. FML provides a utility codec,
	 * {@link FMLIndexedMessageToMessageCodec} that transforms from
	 * {@link FMLProxyPacket} to a mod message using a message discriminator byte.
	 * This is optional, but highly recommended for use.
	 *
	 * Note also that the handlers supplied need to be
	 * {@link ChannelHandler.Shareable} - they are injected into two channels.
	 *
	 * @param name
	 * @param handlers
	 * @return
	 ...
	 *
	 * @param fmlModContainer
	 *            The fml mod container
	 * @param clazz
	 *            a class
	 * @param remoteVersionRange
	 *            the acceptable remote range
	 * @param asmHarvestedData
	 *            internal data
	 */
