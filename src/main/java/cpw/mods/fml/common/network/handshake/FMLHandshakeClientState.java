 ...
 *
 * Flow: 1. Wait for server hello. (START). Move to HELLO state. 2. Receive
 * Server Hello. Send customchannel registration. Send Client Hello. Send our
 * modlist. Move to WAITINGFORSERVERDATA state. 3. Receive server modlist. Send
 * ack if acceptable, else send nack and exit error. Receive server IDs. Move to
...
enum FMLHandshakeClientState implements IHandshakeState<FMLHandshakeClientState> {
	...
	HELLO {
		...
		public FMLHandshakeClientState accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg) {
			...
			ctx.writeAndFlush(FMLHandshakeMessage
			...
			if (serverHelloPacket.protocolVersion() > 1) {
				// Server sent us an extra dimension for the logging in player - stash it for
				// retrieval later
				NetworkDispatcher dispatcher = ctx.channel().attr(NetworkDispatcher.FML_DISPATCHER).get();
	...
	WAITINGSERVERCOMPLETE {
		...
		public FMLHandshakeClientState accept(ChannelHandlerContext ctx, FMLHandshakeMessage msg) {
			...
			List<String> locallyMissing = GameData.injectWorldIDMap(modIds.dataList(), modIds.blockSubstitutions(),
					modIds.itemSubstitutions(), false, false);
			if (!locallyMissing.isEmpty()) {
				NetworkDispatcher dispatcher = ctx.channel().attr(NetworkDispatcher.FML_DISPATCHER).get();
				dispatcher.rejectHandshake("Fatally missing blocks and items");
				FMLLog.severe("Failed to connect to server: there are %d missing blocks and items",
						locallyMissing.size());
