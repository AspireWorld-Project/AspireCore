...
public abstract class FMLHandshakeMessage {
	public static FMLProxyPacket makeCustomChannelRegistration(Set<String> channels) {
		...
		FMLProxyPacket proxy = new FMLProxyPacket(Unpooled.wrappedBuffer(salutation.getBytes(Charsets.UTF_8)),
				"REGISTER");
		return proxy;
	}
	...
	public static class ServerHello extends FMLHandshakeMessage {
		...
		public void fromBytes(ByteBuf buffer) {
			...
			if (serverProtocolVersion > 1) {
				...
				FMLLog.fine("Server FML protocol version %d, 4 byte dimension received %d", serverProtocolVersion,
						overrideDimension);
			} else {
	...
	public static class ModIdData extends FMLHandshakeMessage {
		...
		public ModIdData(GameData.GameDataSnapshot snapshot) {
			modIds = snapshot.idMap;
			blockSubstitutions = snapshot.blockSubstitutions;
			itemSubstitutions = snapshot.itemSubstitutions;
		}
