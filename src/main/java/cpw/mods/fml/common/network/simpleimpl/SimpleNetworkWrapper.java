 ...
 * the full power of netty. It provides a simple message driven system, based on
 * a discriminator byte over the custom packet channel. It assumes that you have
 * a series of unique message types with each having a unique handler.
 * Generally, context should be derived at message handling time.
 *
 * Usage is simple:
 * <ul>
 * <li>construct, and store, an instance of this class. It will automatically
 * register and configure your underlying netty channel.
 *
 * <li>Then, call {@link #registerMessage(Class, Class, byte, Side)} for each
 * message type you want to exchange providing an {@link IMessageHandler}
 * implementation class as well as an {@link IMessage} implementation class. The
 * side parameter to that method indicates which side (server or client) the
 * <em>message processing</em> will occur on. The discriminator byte should be
 * unique for this channelName - it is used to discriminate between different
 * types of message that might occur on this channel (a simple form of message
 * channel multiplexing, if you will).
 * <li>To get a packet suitable for presenting to the rest of minecraft, you can
 * call {@link #getPacketFrom(IMessage)}. The return result is suitable for
 * returning from things like {@link TileEntity#getDescriptionPacket()} for
 * example.
 * <li>Finally, use the sendXXX to send unsolicited messages to various classes
 * of recipients.
 * </ul>
 *
 * Example <code>
