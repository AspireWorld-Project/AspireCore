...
public class ClassPatchManager {
	...
	public byte[] applyPatch(String name, String mappedName, byte[] inputData) {
		...
		if (DEBUG) {
			FMLRelaunchLog.fine("Runtime patching class %s (input size %d), found %d patch%s", mappedName,
					inputData == null ? 0 : inputData.length, list.size(), list.size() != 1 ? "es" : "");
		}
		for (ClassPatch patch : list) {
			...
			if (!patch.existsAtTarget && (inputData == null || inputData.length == 0)) {
				...
				FMLRelaunchLog.warning("Patcher expecting empty class data file for %s, but received non-empty",
		...
		if (!ignoredError && DEBUG) {
			FMLRelaunchLog.fine("Successfully applied runtime patches for %s (new size %d)", mappedName,
					inputData.length);
		}
		if (dumpPatched) {
			try {
