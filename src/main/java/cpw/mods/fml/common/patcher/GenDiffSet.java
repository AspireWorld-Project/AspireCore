...
public class GenDiffSet {
	...
	private static final List<String> RESERVED_NAMES = Arrays.asList("CON", "PRN", "AUX", "NUL", "COM1", "COM2", "COM3",
	...
	public static void main(String[] args) throws IOException {
		...
		for (String name : remapper.getObfedClasses()) {
			// Logger.getLogger("GENDIFF").info(String.format("Evaluating path for data
			// :%s",name));
			...
			if (targetFile.exists()) {
				...
				byte[] vanillaBytes = entry != null ? ByteStreams.toByteArray(sourceZip.getInputStream(entry))
						: new byte[0];
				...
				Logger.getLogger("GENDIFF").info(String.format("Wrote patch for %s (%s) at %s", name, targetClassName,
