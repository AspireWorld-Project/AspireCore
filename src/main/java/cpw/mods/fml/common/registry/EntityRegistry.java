...
public class EntityRegistry {
	...
	public static void registerModEntity(Class<? extends Entity> entityClass, String entityName, int id, Object mod,
			int trackingRange, int updateFrequency, boolean sendsVelocityUpdates) {
		instance().doModEntityRegistration(entityClass, entityName, id, mod, trackingRange, updateFrequency,
	...
	private void doModEntityRegistration(Class<? extends Entity> entityClass, String entityName, int id, Object mod,
		...
		EntityRegistration er = new EntityRegistration(mc, entityClass, entityName, id, trackingRange, updateFrequency,
				sendsVelocityUpdates);
		try {
			...
			if (!EntityList.classToStringMapping.containsKey(entityClass)) {
				...
				FMLLog.finer("Automatically registered mod %s entity %s as %s", mc.getModId(), entityName,
						entityModName);
			} else {
				FMLLog.fine("Skipping automatic mod %s entity registration for already registered class %s",
						mc.getModId(), entityClass.getName());
			}
	...
	private int validateAndClaimId(int id) {
		...
		if (id < Byte.MIN_VALUE) {
			FMLLog.warning(
					...
					id, Loader.instance().activeModContainer().getModId(), realId);
			realId += 3000;
		}
		...
		try {
			...
			FMLLog.log(Level.ERROR, "The entity ID %d for mod %s is not an unsigned byte and may not work", id,
		...
		if (!availableIndicies.get(realId)) {
			FMLLog.severe(
					...
					Loader.instance().activeModContainer().getModId(), id);
		}
	...
	public boolean tryTrackingEntity(EntityTracker entityTracker, Entity entity) {
		...
		if (er != null) {
			entityTracker.addEntityToTracker(entity, er.getTrackingRange(), er.getUpdateFrequency(),
