...
public class FMLControlledNamespacedRegistry<I> extends RegistryNamespaced {
	...
	// aliases redirecting legacy names to the actual name, may need recursive
	// application to find the final name.
	// these need to be registry specific, it's possible to only have a loosely
	...
	void validateContent(int maxId, String type, BitSet availabilityMap, Set<Integer> blockedIds,
			FMLControlledNamespacedRegistry<Block> iBlockRegistry) {
		for (I obj : typeSafeIterable()) {
			...
			if (!isSubstituted && id < 0)
				throw new IllegalStateException(
						String.format("Registry entry for %s %s, name %s, doesn't yield an id.", type, obj, name));
			// id is too high
			if (id > maxId)
				throw new IllegalStateException(
						String.format("Registry entry for %s %s, name %s uses the too large id %d.", type, obj, name));
			// name lookup failed -> obj is not in the obj<->name map
			if (name == null)
				throw new IllegalStateException(
						String.format("Registry entry for %s %s, id %d, doesn't yield a name.", type, obj, id));
			// empty name
			if (name.isEmpty())
				throw new IllegalStateException(
						String.format("Registry entry for %s %s, id %d, yields an empty name.", type, obj, id));
			// non-prefixed name
			if (name.indexOf(':') == -1)
				throw new IllegalStateException(String
						.format("Registry entry for %s %s, id %d, has the non-prefixed name %s.", type, obj, id, name));
			...
			if (getRaw(id) != obj)
				throw new IllegalStateException(String.format(
						"Registry entry for id %d, name %s, doesn't yield the expected %s %s.", id, name, type, obj));
			// name -> obj lookup is inconsistent
			if (getRaw(name) != obj)
				throw new IllegalStateException(String.format(
						"Registry entry for name %s, id %d, doesn't yield the expected %s %s.", name, id, type, obj));
			...
			if (!availabilityMap.get(id))
				throw new IllegalStateException(String
						.format("Registry entry for %s %s, id %d, name %s, marked as empty.", type, obj, id, name));
			// entry is blocked, thus should be empty
			if (blockedIds.contains(id))
				throw new IllegalStateException(String
						.format("Registry entry for %s %s, id %d, name %s, marked as dangling.", type, obj, id, name));
			...
			if (obj instanceof ItemBlock) {
				...
				if (iBlockRegistry.getId(block) != id)
					throw new IllegalStateException(String.format(
							"Registry entry for ItemBlock %s, id %d, is missing or uses the non-matching id %d.", obj,
							id, iBlockRegistry.getId(block)));
				// verify id range
				if (id > GameData.MAX_BLOCK_ID)
					throw new IllegalStateException(
							String.format("ItemBlock %s uses the id %d outside the block id range", name, id));
			}
		}
	 ...
	 *
	 * Don't hold onto the id across the world, it's being dynamically re-mapped as
	 * needed.
	 *
	 * Usually the name should be used instead of the id, if using the Block/Item
	 * object itself is not suitable for the task.
	 *
	 * @param thing
	 *            Block/Item object.
	...
	/*
	 * This iterator is used by FML to visit the actual block sets, it should use
	 * the super.iterator method instead Compare #iterator()
	 */
	 ...
	 *
	 * @param id
	 *            ID to use if available, auto-assigned otherwise.
	 * @param name
	 *            Name to use, prefixed by the mod id.
	 * @param thing
	 *            Object to add.
	 * @param availabilityMap
	 *            Map marking available IDs for auto assignment.
	...
	int add(int id, String name, I thing, BitSet availabilityMap) {
		...
		if (name.isEmpty())
			throw new IllegalArgumentException(
					String.format("Can't use an empty name for the registry, object %s.", thing));
		if (name.indexOf(':') == -1)
			throw new IllegalArgumentException(
					String.format("Can't add the name (%s) without a prefix, object %s", name, thing));
		if (thing == null)
		...
		if (getPersistentSubstitutions().containsValue(thing))
			throw new IllegalArgumentException(String.format(
					"The object %s (%s) cannot be added to the registry. It is already being used as a substitute for %s",
					thing.getClass(), name, getPersistentSubstitutions().inverse().get(thing)));
		int idToUse = id;
		...
		if (getRaw(name) != null)
			throw new IllegalArgumentException(
					String.format("The name %s has been registered twice, for %s and %s.", name, getRaw(name), thing));
		if (getId(thing) >= 0) // duplicate object - but only if it's not being substituted
		{
			...
			throw new IllegalArgumentException(String.format(
					...
					thing, System.identityHashCode(thing), getNameForObject(thing), name, otherThing,
	...
	private void addObjectRaw(int id, String name, I thing) {
		if (name == null)
			throw new NullPointerException(
					"The name to be added to the registry is null. This can only happen with a corrupted registry state. Reflection/ASM hackery? Registry bug?");
		if (thing == null)
			throw new NullPointerException(
					"The object to be added to the registry is null. This can only happen with a corrupted registry state. Reflection/ASM hackery? Registry bug?");
		if (!superType.isInstance(thing))
			throw new IllegalArgumentException(
					"The object to be added to the registry is not of the right type. Reflection/ASM hackery? Registry bug?");
	...
	void activateSubstitution(String nameToReplace) {
		if (getPersistentSubstitutions().containsKey(nameToReplace)) {
			...
			if (superType == Item.class) {
				...
				if (original == null) {
					// When we're activated from the server side, we need to set the delegate on the
	...
	void addSubstitutionAlias(String modId, String nameToReplace, Object toReplace)
		...
		if (getPersistentSubstitutions().containsKey(nameToReplace)
				|| getPersistentSubstitutions().containsValue(toReplace)) {
			FMLLog.severe("The substitution of %s has already occured. You cannot duplicate substitutions",
					nameToReplace);
		...
		if (!original.getClass().isAssignableFrom(replacement.getClass())) {
			FMLLog.severe("The substitute %s for %s (type %s) is type incompatible. This won't work",
					replacement.getClass().getName(), nameToReplace, original.getClass().getName());
			throw new IncompatibleSubstitutionException(nameToReplace, replacement, original);
		}
		int existingId = getId(replacement);
		if (existingId != -1) {
			FMLLog.severe("The substitute %s for %s is registered into the game independently. This won't work",
					replacement.getClass().getName(), nameToReplace);
	...
	/*
	 * This iterator is used by some regular MC methods to visit all blocks, we need
	 * to include substitutions Compare #typeSafeIterable()
	 */
	@SuppressWarnings("unchecked")
	@Override
