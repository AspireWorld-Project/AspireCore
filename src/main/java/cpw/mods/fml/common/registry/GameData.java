...
public class GameData {
	...
	public static class GameDataSnapshot {
		...
		public GameDataSnapshot(Map<String, Integer> idMap, Set<String> blockSubstitutions,
	 ...
	 *
	 * Items sharing the same ID with a block, but not sharing the same registry
	 * name will be mapped to an unused id. Losing items instead of blocks should
	 * minimize the damage.
	 *
	 * @param dataList
	 *            List containing the IDs to fix
	 */
	public static void fixBrokenIds(Map<String, Integer> dataList, Set<Integer> blockedIds) {
		...
		Set<Integer> newBlockedIds = new HashSet<>();
		Set<String> itemsToRemove = new HashSet<>();
		Map<String, Integer> itemsToRelocate = new HashMap<>();
		...
		for (Entry<String, Integer> entry : dataList.entrySet()) {
			...
			if (itemName.charAt(0) != '\u0001') // is an item
			{
				...
				if (item == null) // item no longer available
				{
					...
					FMLLog.warning("Item %s (old id %d) is no longer available and thus can't be fixed.", realName,
							oldId);
					...
					if (dataList.containsKey(blockName)) // the item was an ItemBlock before
					{
						...
						if (blockId != oldId) // mis-located ItemBlock
						{
							...
							FMLLog.warning("ItemBlock %s (old id %d) doesn't have the same id as its block (%d).",
					...
					} else // the item hasn't been an ItemBlock before, but it's now
					{
						...
						FMLLog.warning("Item %s (old id %d) has been migrated to an ItemBlock and can't be fixed.",
								realName, oldId);
						itemsToRemove.add(itemName);
						blockThisId = true;
					}
				} else if (availabilityMap.get(oldId)) // normal item, id is already occupied
				{
					...
					FMLLog.warning("Item %s (old id %d) is conflicting with another block/item and can't be fixed.",
							realName, oldId);
					itemsToRemove.add(itemName);
				} else // intact Item
				{
				...
				// handle blocking the id from future use if possible (i.e. not used by a
				// conflicting block)
				// blockThisId requests don't modify availabilityMap, it could only be set by a
				// block (or another item, which isn't being handled)
		...
		Set<String> modsMissing = new HashSet<>();
		...
		for (Iterator<String> it = modsMissing.iterator(); it.hasNext();) {
			...
			if (mod.equals("minecraft") || Loader.isModLoaded(mod)) {
				it.remove();
			}
		}
	...
	public static List<String> injectWorldIDMap(Map<String, Integer> dataList, Set<String> blockSubstitutions,
			Set<String> itemSubstitutions, boolean injectFrozenData, boolean isLocalWorld) {
		return injectWorldIDMap(dataList, new HashSet<Integer>(), new HashMap<String, String>(),
				new HashMap<String, String>(), blockSubstitutions, itemSubstitutions, injectFrozenData, isLocalWorld);
	}

	public static List<String> injectWorldIDMap(Map<String, Integer> dataList, Set<Integer> blockedIds,
			Map<String, String> blockAliases, Map<String, String> itemAliases, Set<String> blockSubstitutions,
			Set<String> itemSubstitutions, boolean injectFrozenData, boolean isLocalWorld) {
		FMLLog.info("Injecting existing block and item data into this %s instance",
				FMLCommonHandler.instance().getEffectiveSide().isServer() ? "server" : "client");
		Map<String, Integer[]> remaps = Maps.newHashMap();
		LinkedHashMap<String, Integer> missingMappings = new LinkedHashMap<>();
		...
		// process blocks and items in the world, blocks in the first pass, items in the
		// second
		...
		for (int pass = 0; pass < 2; pass++) {
			...
			for (Entry<String, Integer> entry : dataList.entrySet()) {
				...
				// names starting with 0x1 are blocks, skip if the type isn't handled by this
				// pass
				if (itemName.charAt(0) == '\u0001' != isBlock) {
					continue;
				}

				itemName = itemName.substring(1);
				int currId = isBlock ? getMain().iBlockRegistry.getId(itemName)
				...
				if (currId == -1) {
					...
					FMLLog.fine("Fixed %s id mismatch %s: %d (init) -> %d (map).", isBlock ? "block" : "item", itemName,
							currId, newId);
	...
	public static List<String> processIdRematches(Iterable<MissingMapping> missedMappings, boolean isLocalWorld,
		...
		for (MissingMapping remap : missedMappings) {
			...
			if (action == FMLMissingMappingsEvent.Action.REMAP) {
				// block/item re-mapped, finish the registration with the new name/object, but
				// the old id
				int currId, newId;
				String newName;
				...
				if (currId != newId) {
					FMLLog.info("Fixed %s id mismatch %s: %d (init) -> %d (map).",
				...
				// Pulled out specifically so the block doesn't get reassigned a new ID just
				// because it's
				// Item block has gone away
				FMLLog.fine("The ItemBlock %s is no longer present in the game. The residual block will remain",
						remap.name);
			} else {
	...
	// IDs previously allocated in a world, but now unmapped/dangling; prevents the
	// IDs from being reused
	private final Set<Integer> blockedIds;

	private GameData() {
		iBlockRegistry = new FMLControlledNamespacedRegistry<>("minecraft:air", MAX_BLOCK_ID, MIN_BLOCK_ID, Block.class,
				'\u0001');
		iItemRegistry = new FMLControlledNamespacedRegistry<>(null, MAX_ITEM_ID, MIN_ITEM_ID, Item.class, '\u0002');
	...
	int register(Object obj, String name, int idHint) // from FMLControlledNamespacedRegistry.addObject
	{
		...
		if (obj instanceof Block)
			...
			throw new IllegalArgumentException(
					"An invalid registry object is to be added, only instances of Block or Item are allowed.");
	}
	...
	int registerItem(Item item, String name) // from GameRegistry
	{
		...
		if (name.indexOf(':') != -1) {
			FMLLog.bigWarning("Illegal extra prefix %s for name %s, invalid registry invocation/invalid name?",
	...
	private int registerItem(Item item, String name, int idHint) {
		if (item instanceof ItemBlock) // ItemBlock, adjust id and clear the slot already occupied by the corresponding
										// block
		{
			...
			{
				if (idHint < 0 || availabilityMap.get(idHint) || idHint > MAX_BLOCK_ID) // non-suitable id, allocate one
																						// in the block id range, add
																						// would use the item id range
																						// otherwise
				{
					id = availabilityMap.nextClearBit(MIN_BLOCK_ID); // find suitable id here, iItemRegistry would
																		// search from MIN_ITEM_ID
					if (id > MAX_BLOCK_ID)
						throw new RuntimeException(String.format("Invalid id %d - maximum id range exceeded.", id));
					FMLLog.fine("Allocated id %d for ItemBlock %s in the block id range, original id requested: %d.",
							id, name, idHint);
			...
			} else // ItemBlock after its Block
			{
				if (FMLControlledNamespacedRegistry.DEBUG) {
					FMLLog.fine("Found matching Block %s for ItemBlock %s at id %d, original id requested: %d", block,
							item, id, idHint);
				}
	...
	private int registerBlock(Block block, String name, int idHint) {
		...
		if (itemBlock != null) // has ItemBlock, adjust id and clear the slot already occupied by the
								// corresponding item
		{
		...
		if (itemBlock != null) // verify
		{
			if (blockId != idHint)
				throw new IllegalStateException(
						String.format("Block at itemblock id %d insertion failed, got id %d.", idHint, blockId));
	 ...
	 *
	 * The slot must not be occupied by something else than the specified object
	 * within the same type. The same object is permitted for handling duplicate
	 * registrations.
	 *
	 * @param id
	 *            id to free
	 ...
	 *
	 * If no mod id can be determined, minecraft will be assumed. The prefix is
	...
	private void testConsistency() {
		...
		for (int pass = 0; pass < 2; pass++) {
			...
			registry.validateContent(isBlock ? MAX_BLOCK_ID : MAX_ITEM_ID, type, availabilityMap, blockedIds,
					iBlockRegistry);
		}
	...
	void registerSubstitutionAlias(String nameToSubstitute, Type type, Object toReplace)
			throws ExistingSubstitutionException {
		type.getRegistry().addSubstitutionAlias(Loader.instance().activeModContainer().getModId(), nameToSubstitute,
				toReplace);
