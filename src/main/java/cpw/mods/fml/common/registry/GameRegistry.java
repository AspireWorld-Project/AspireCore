...
public class GameRegistry {
	 ...
	 *            a weight to assign to this generator. Heavy weights tend to sink
	 *            to the bottom of list of world generators (i.e. they run later)
	 */
	...
	/**
	 * Callback hook for world gen - if your mod wishes to add extra mod related
	 ...
	 * @param chunkProvider
	 */
	public static void generateWorld(int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
	...
	/**
	 * Register an item with the item registry with a custom name : this allows for
	 * easier server->client resolution
	 *
	 * @param item
	...
	/**
	 * Register the specified Item with a mod specific name : overrides the standard
	 * type based name
	 *
	 * @param item
	 *            The item to register
	 * @param name
	 *            The mod-unique name to register it as - null will remove a custom
	 *            name
	 * @param modId
	 *            deprecated, unused where one mod should "own" all the blocks of
	 *            all the mods, null defaults to the active mod
	 */
	...
	/**
	 * Add a forced persistent substitution alias for the block or item to another
	 * block or item. This will have the effect of using the substituted block or
	 * item instead of the original, where ever it is referenced.
	 *
	 ...
	 * @param object
	 *            a NEW instance that is type compatible with the existing instance
	 * @throws ExistingSubstitutionException
	 *             if someone else has already registered an alias either from or to
	 *             one of the names
	 ...
	 * @param name
	 *            The mod-unique name to register it as, will get prefixed by your
	 *            modid.
	 */
	 ...
	 *            The block to register
	 * @param itemclass
	 *            The item type to register with it : null registers a block without
	 *            associated item.
	 * @param name
	 *            The mod-unique name to register it as, will get prefixed by your
	 *            modid.
	...
	/**
	 * @deprecated Use the registerBlock version without the modId parameter
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Register a block with the world, with the specified item class, block name
	 * and owning modId
	 *
	 * @param block
	 *            The block to register
	 ...
	 *            modid.
	 * @param itemCtorArgs
	 *            Arguments to pass (after the required {@code Block} parameter) to
	 *            the ItemBlock constructor (optional).
	 */
	public static Block registerBlock(Block block, Class<? extends ItemBlock> itemclass, String name,
			Object... itemCtorArgs) {
	 ...
	 * caution! This method allows for you to "rename" the 'id' of the tile entity.
	 *
	 * @param tileEntityClass
	 *            The tileEntity class to register
	 * @param id
	 *            The primary ID, this will be the ID that the tileentity saves as
	 * @param alternatives
	 *            A list of alternative IDs that will also map to this class. These
	 *            will never save, but they will load
	 */
	public static void registerTileEntityWithAlternatives(Class<? extends TileEntity> tileEntityClass, String id,
			String... alternatives) {
		TileEntity.addMapping(tileEntityClass, id);
		Map<String, Class<?>> teMappings = ObfuscationReflectionHelper.getPrivateValue(TileEntity.class, null,
				"field_" + "145855_i", "nameToClassMap");
	...
	/**
	 * Manually register a custom item stack with FML for later tracking. It is
	 * automatically scoped with the active modid
	 *
	 * @param name
	 *            The name to register it under
	 * @param itemStack
	 *            The itemstack to register
	 */
	...
	/**
	 * Lookup an itemstack based on mod and name. It will create "default"
	 * itemstacks from blocks and items if no explicit itemstack is found.
	 *
	 * If it is built from a block, the metadata is by default the "wildcard" value.
	 *
	 * Custom itemstacks can be dumped from minecraft by setting the system property
	 * fml.dumpRegistry to true (-Dfml.dumpRegistry=true on the command line will
	 * work)
	 *
	 * @param modId
	 *            The modid of the stack owner
	 * @param name
	...
	/**
	 * Look up the mod identifier data for a block. Returns null if there is no mod
	 * specified mod identifier data, or it is part of a custom itemstack definition
	 * {@link #registerCustomItemStack}
	 *
	 * Note: uniqueness and persistence is only guaranteed by mods using the game
	 * registry correctly.
	 *
	 * @param block
	 *            to lookup
	...
	/**
	 * Look up the mod identifier data for an item. Returns null if there is no mod
	 * specified mod identifier data, or it is part of a custom itemstack definition
	 * {@link #registerCustomItemStack}
	 *
	 * Note: uniqueness and persistence is only guaranteed by mods using the game
	...
	/**
	 * This will cause runtime injection of public static final fields to occur at
	 * various points where mod blocks and items <em>could</em> be subject to
	 * change. This allows for dynamic substitution to occur.
	 *
	 */
	...
	public @interface ObjectHolder {
		/**
		 * If used on a class, this represents a modid only. If used on a field, it
		 * represents a name, which can be abbreviated or complete. Abbreviated names
		 * derive their modid from an enclosing ObjectHolder at the class level.
		 *
	 ...
	 *
	 * Will return null if the item doesn't exist (because it's not from a loaded
	 * mod for example) Will throw a {@link RuntimeException} if the nbtString is
	 * invalid for use in an {@link ItemStack}
	 *
	 * @param itemName
	 ...
	 *            the stack size
	 * @param nbtString
	 *            an nbt stack as a string, will be processed by {@link JsonToNBT}
	...
	public static ItemStack makeItemStack(String itemName, int meta, int stackSize, String nbtString) {
		...
		if (!Strings.isNullOrEmpty(nbtString)) {
			...
			try {
				...
				FMLLog.getLogger().log(Level.WARN, "Encountered an exception parsing ItemStack NBT string {}",
						nbtString, e);
