...
public class LanguageRegistry {
	...
	public void loadLocalization(String localizationFile, String lang, boolean isXML) {
		...
		if (urlResource != null) {
			...
			if (activeModContainer != null) {
				FMLLog.log(activeModContainer.getModId(), Level.ERROR,
	...
	private void searchZipForLanguages(File source, Side side) throws IOException {
		...
		for (ZipEntry ze : Collections.list(zf.entries())) {
			...
			if (matcher.matches()) {
				...
				// FMLLog.fine("Injecting found translation data for lang %s in zip file %s at
				// %s into language system", lang, source.getName(), ze.getName());
				added.add(lang);
	...
	private void searchDirForLanguages(File source, String path, Side side) throws IOException {
		for (File file : source.listFiles()) {
			...
			if (matcher.matches()) {
				String lang = matcher.group(2);
				FMLLog.fine("Injecting found translation assets for lang %s at %s into language system", lang,
						currPath);
				LanguageRegistry.instance().injectLanguage(lang,
						StringTranslate.parseLangFile(new FileInputStream(file)));
				// Ensure en_US is available to StringTranslate on the server
				if ("en_US".equals(lang) && side == Side.SERVER) {
