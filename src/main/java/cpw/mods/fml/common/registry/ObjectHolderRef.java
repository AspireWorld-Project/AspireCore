...
class ObjectHolderRef {
	...
	ObjectHolderRef(Field field, String injectedObject, boolean extractFromExistingValues) {
		this.field = field;
		isBlock = Block.class.isAssignableFrom(field.getType());
		isItem = Item.class.isAssignableFrom(field.getType());
	...
	public void apply() {
		...
		try {
			...
			FMLLog.log(Level.WARN, e, "Unable to set %s with value %s (%s)", field, thing, injectedObject);
		}
	}
}