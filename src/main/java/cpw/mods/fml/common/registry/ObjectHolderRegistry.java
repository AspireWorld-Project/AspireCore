...
public enum ObjectHolderRegistry {
	...
	public void findObjectHolders(ASMDataTable table) {
		...
		// double pass - get all the class level annotations first, then the field level
		// annotations
		for (ASMData data : allObjectHolders) {
			String className = data.getClassName();
			String annotationTarget = data.getObjectName();
	...
	private void scanTarget(Map<String, String> classModIds, Map<String, Class<?>> classCache, String className,
			String annotationTarget, String value, boolean isClass, boolean extractFromValue) {
		Class<?> clazz;
		...
		if (isClass) {
			...
			if (value.indexOf(':') == -1) {
				...
				if (prefix == null) {
					FMLLog.warning(
							"Found an unqualified ObjectHolder annotation (%s) without a modid context at %s.%s, ignoring",
							value, className, annotationTarget);
