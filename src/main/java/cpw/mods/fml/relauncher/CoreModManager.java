...
public class CoreModManager {
	...
	private static class FMLPluginWrapper implements ITweaker {
		...
		public String toString() {
			return String.format("%s {%s}", name, predepends);
		}
		...
		public void injectIntoClassLoader(LaunchClassLoader classLoader) {
			FMLRelaunchLog.fine("Injecting coremod %s {%s} class transformers", name,
					coreModInstance.getClass().getName());
	...
	private static void discoverCoreMods(File mcDir, LaunchClassLoader classLoader) {
		...
		for (File coreMod : coreModList) {
			...
			if (!modTypes.contains("FML")) {
				FMLRelaunchLog.fine("Adding %s to the list of things to skip. It is not an FML mod,  it has types %s",
	...
	private static FMLPluginWrapper loadCoreMod(LaunchClassLoader classLoader, String coreModClass, File location) {
		...
		try {
			...
			if (!Arrays.asList(rootPlugins).contains(coreModClass)
				...
				FMLRelaunchLog.log(Level.ERROR,
			...
			if (accessTransformerClass != null) {
				FMLRelaunchLog.log(Level.DEBUG, "Added access transformer class %s to enqueued access transformers",
			...
			if (!Lists.newArrayList(rootPlugins).contains(coreModClass)) {
				FMLRelaunchLog.log(Level.ERROR, cnfe, "Coremod %s: Unable to class load the plugin %s", coreModName,
						coreModClass);
			} else {
				FMLRelaunchLog.fine("Skipping root plugin %s", coreModClass);
			}
		} catch (ClassCastException cce) {
			FMLRelaunchLog.log(Level.ERROR, cce, "Coremod %s: The plugin %s is not an implementor of IFMLLoadingPlugin",
					coreModName, coreModClass);
		} catch (InstantiationException ie) {
			FMLRelaunchLog.log(Level.ERROR, ie, "Coremod %s: The plugin class %s was not instantiable", coreModName,
					coreModClass);
		} catch (IllegalAccessException iae) {
			FMLRelaunchLog.log(Level.ERROR, iae, "Coremod %s: The plugin class %s was not accessible", coreModName,
					coreModClass);
		}
		return null;
	}
	...
	private static void sortCoreMods() {
		TopologicalSort.DirectedGraph<FMLPluginWrapper> sortGraph = new TopologicalSort.DirectedGraph<>();
	...
	public static void sortTweakList() {
		@SuppressWarnings("unchecked")
		List<ITweaker> tweakers = (List<ITweaker>) Launch.blackboard.get("Tweaks");
		// Basically a copy of Collections.sort pre 8u20, optimized as we know we're an
		// array list.
		...
		// Basically a copy of Collections.sort, optimized as we know we're an array
		// list.
		// Thanks unhelpful fixer of http://bugs.java.com/view_bug.do?bug_id=8032636
