...
public class FMLLaunchHandler {
	...
	private FMLLaunchHandler(LaunchClassLoader launchLoader, FMLTweaker tweaker) {
		...
		minecraftHome = tweaker.getGameDir();
		...
		classLoader.addTransformerExclusion("cpw.mods.fml.common.asm.transformers.deobf.");
	...
	private void setupHome() {
		...
		FMLRelaunchLog.info("Forge Mod Loader version %s.%s.%s.%s for Minecraft %s loading", FMLInjectionData.major,
				FMLInjectionData.minor, FMLInjectionData.rev, FMLInjectionData.build, FMLInjectionData.mccversion,
				FMLInjectionData.mcpversion);
		FMLRelaunchLog.info("Java is %s, version %s, running on %s:%s:%s, installed at %s",
		...
		try {
			...
			t.printStackTrace();
			FMLRelaunchLog.log(Level.ERROR, t,
					"An error occurred trying to configure the minecraft home at %s for Forge Mod Loader",
