...
public class ModListHelper {
	...
	static void parseModList(File minecraftDirectory) {
		FMLRelaunchLog.fine("Attempting to load commandline specified mods, relative to %s",
	...
	private static void parseListFile(String listFile) {
		...
		try {
			...
			FMLRelaunchLog.log(Level.INFO, e2, "Unable to canonicalize path %s relative to %s", listFile,
					mcDirectory.getAbsolutePath());
			return;
		}
		if (!f.exists()) {
		...
		visitedFiles.add(f);
		// We visit parents before children, so the additionalMods list is sorted from
		// parent to child
