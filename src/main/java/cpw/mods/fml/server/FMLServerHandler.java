 ...
 *
 * The FML entry point is {@link #beginServerLoading(MinecraftServer)} called
 * from {@link net.minecraft.server.dedicated.DedicatedServer}
 *
	...
	public void queryUser(StartupQuery query) throws InterruptedException {
		if (query.getResult() == null) {
			...
			if (!query.isSynchronous())
				return; // no-op until mc does commands in another thread (if ever)
			...
			while (!done && server.isServerRunning()) {
				...
				// rudimentary command processing, check for fml confirm/cancel and stop
				// commands
				synchronized (dedServer.pendingCommandList) {
					for (Iterator<ServerCommand> it = GenericIterableFactory
							.newCastingIterable(dedServer.pendingCommandList, ServerCommand.class).iterator(); it
									.hasNext();) {
