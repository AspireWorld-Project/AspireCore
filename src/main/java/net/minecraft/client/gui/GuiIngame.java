...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float p_73830_1_, boolean p_73830_2_, int p_73830_3_, int p_73830_4_) {
		...
		if (Minecraft.isFancyGraphicsEnabled()) {
			renderVignette(mc.thePlayer.getBrightness(p_73830_1_), k, l);
		} else {
		...
		if (!mc.thePlayer.isPotionActive(Potion.confusion)) {
			float f1 = mc.thePlayer.prevTimeInPortal
					+ (mc.thePlayer.timeInPortal - mc.thePlayer.prevTimeInPortal) * p_73830_1_;
		...
		if (mc.thePlayer.isRidingHorse()) {
			...
			f3 = mc.thePlayer.getHorseJumpPower();
			short1 = 182;
			l1 = (int) (f3 * (short1 + 1));
			i2 = l - 32 + 3;
			...
			mc.getTextureManager().bindTexture(Gui.icons);
			j1 = mc.thePlayer.xpBarCap();

			if (j1 > 0) {
				short1 = 182;
				l1 = (int) (mc.thePlayer.experience * (short1 + 1));
				i2 = l - 32 + 3;
			...
			if (mc.thePlayer.experienceLevel > 0) {
				...
				String s3 = "" + mc.thePlayer.experienceLevel;
		...
		if (mc.gameSettings.heldItemTooltips) {
			...
			if (remainingHighlightTicks > 0 && highlightingItemStack != null) {
				s2 = highlightingItemStack.getDisplayName();
				...
				i2 = (int) (remainingHighlightTicks * 256.0F / 10.0F);
		...
		if (mc.gameSettings.showDebugInfo) {
			...
			long l5 = j5 - k5;
			String s = "Used memory: " + l5 * 100L / i5 + "% (" + l5 / 1024L / 1024L + "MB) of " + i5 / 1024L / 1024L
					+ "MB";
			i3 = 14737632;
			...
			for (String brd : FMLCommonHandler.instance().getBrandings(false)) {
				drawString(fontrenderer, brd, k - fontrenderer.getStringWidth(brd) - 2, offset += 10, 14737632);
			}
			j3 = MathHelper.floor_double(mc.thePlayer.posX);
			...
			drawString(fontrenderer, "f: " + i4 + " (" + Direction.directions[i4] + ") / "
					+ MathHelper.wrapAngleTo180_float(mc.thePlayer.rotationYaw), 2, 88, 14737632);

			if (mc.theWorld != null && mc.theWorld.blockExists(j3, k3, l3)) {
				Chunk chunk = mc.theWorld.getChunkFromBlockCoords(j3, l3);
				drawString(fontrenderer,
						"lc: " + (chunk.getTopFilledSegment() + 15) + " b: "
								+ chunk.getBiomeGenForWorldCoords(j3 & 15, l3 & 15,
										mc.theWorld.getWorldChunkManager()).biomeName
								+ " bl: " + chunk.getSavedLightValue(EnumSkyBlock.Block, j3 & 15, k3, l3 & 15) + " sl: "
								+ chunk.getSavedLightValue(EnumSkyBlock.Sky, j3 & 15, k3, l3 & 15) + " rl: "
								+ chunk.getBlockLightValue(j3 & 15, k3, l3 & 15, 0),
						2, 96, 14737632);
			}
		...
		if (recordPlayingUpFor > 0) {
			...
			f3 = recordPlayingUpFor - p_73830_1_;
		...
		if (mc.gameSettings.keyBindPlayerList.getIsKeyPressed() && (!mc.isIntegratedServerRunning()
				|| mc.thePlayer.sendQueue.playerInfoList.size() > 1 || scoreobjective != null)) {
	...
	protected void func_96136_a(ScoreObjective p_96136_1_, int p_96136_2_, int p_96136_3_, FontRenderer p_96136_4_) {
		...
		if (collection.size() <= 15) {
			...
			for (Iterator iterator = collection.iterator(); iterator
					.hasNext(); k = Math.max(k, p_96136_4_.getStringWidth(s))) {
			...
			while (iterator1.hasNext()) {
				...
				if (l == collection.size()) {
					...
					p_96136_4_.drawString(s3, i2 + k / 2 - p_96136_4_.getStringWidth(s3) / 2,
							i1 - p_96136_4_.FONT_HEIGHT, 553648127);
				}
			}
		}
	}
	...
	protected void func_110327_a(int p_110327_1_, int p_110327_2_) {
		...
		int k = MathHelper.ceiling_float_int(mc.thePlayer.getHealth());
		int l = MathHelper.ceiling_float_int(mc.thePlayer.prevHealth);
		...
		float f2 = f1;
		int i3 = mc.thePlayer.getTotalArmorValue();
		int j3 = -1;

		if (mc.thePlayer.isPotionActive(Potion.regeneration)) {
			j3 = updateCounter % MathHelper.ceiling_float_int(f + 5.0F);
		}
		...
		for (k3 = MathHelper.ceiling_float_int((f + f1) / 2.0F) - 1; k3 >= 0; --k3) {
			...
			i4 = MathHelper.ceiling_float_int((k3 + 1) / 10.0F) - 1;
			j4 = k1 + k3 % 10 * 8;
			k4 = i2 - i4 * k2;
		...
		if (entity == null) {
			...
			for (l3 = 0; l3 < 10; ++l3) {
				...
				if (mc.thePlayer.getFoodStats().getSaturationLevel() <= 0.0F && updateCounter % (i1 * 3 + 1) == 0) {
					k5 = i2 + rand.nextInt(3) - 1;
				}
			...
			k5 = (int) Math.ceil(entitylivingbase.getHealth());
		...
		if (mc.thePlayer.isInsideOfMaterial(Material.water)) {
			l3 = mc.thePlayer.getAir();
			k5 = MathHelper.ceiling_double_int((l3 - 2) * 10.0D / 300.0D);
			i4 = MathHelper.ceiling_double_int(l3 * 10.0D / 300.0D) - k5;
	...
	protected void renderInventorySlot(int p_73832_1_, int p_73832_2_, int p_73832_3_, float p_73832_4_) {
		...
		if (itemstack != null) {
			...
			itemRenderer.renderItemAndEffectIntoGUI(mc.fontRenderer, mc.getTextureManager(), itemstack, p_73832_2_,
					p_73832_3_);
			...
			itemRenderer.renderItemOverlayIntoGUI(mc.fontRenderer, mc.getTextureManager(), itemstack, p_73832_2_,
					p_73832_3_);
		}
	}
	...
	public void updateTick() {
		...
		if (mc.thePlayer != null) {
			...
			} else if (highlightingItemStack != null && itemstack.getItem() == highlightingItemStack.getItem()
					&& ItemStack.areItemStackTagsEqual(itemstack, highlightingItemStack)
					&& (itemstack.isItemStackDamageable()
							|| itemstack.getItemDamage() == highlightingItemStack.getItemDamage())) {
