...
public class EntityFireworkStarterFX extends EntityFX {
	...
	public EntityFireworkStarterFX(World p_i1208_1_, double p_i1208_2_, double p_i1208_4_, double p_i1208_6_,
			double p_i1208_8_, double p_i1208_10_, double p_i1208_12_, EffectRenderer p_i1208_14_,
		...
		if (p_i1208_15_ != null) {
			...
			if (fireworkExplosions.tagCount() == 0) {
				...
				for (int i = 0; i < fireworkExplosions.tagCount(); ++i) {
					NBTTagCompound nbttagcompound1 = fireworkExplosions.getCompoundTagAt(i);
	...
	public void onUpdate() {
		...
		if (fireworkAge % 2 == 0 && fireworkExplosions != null && fireworkAge / 2 < fireworkExplosions.tagCount()) {
			...
			if (b0 == 1) {
				createBall(0.5D, 4, aint, aint1, flag3, flag2);
			} else if (b0 == 2) {
				createShaped(0.5D,
						...
						aint, aint1, flag3, flag2, false);
			} else if (b0 == 3) {
				createShaped(0.5D,
						new double[][] { { 0.0D, 0.2D }, { 0.2D, 0.2D }, { 0.2D, 0.6D }, { 0.6D, 0.6D }, { 0.6D, 0.2D },
								{ 0.2D, 0.2D }, { 0.2D, 0.0D }, { 0.4D, 0.0D }, { 0.4D, -0.6D }, { 0.2D, -0.6D },
								{ 0.2D, -0.4D }, { 0.0D, -0.4D } },
						aint, aint1, flag3, flag2, true);
			} else if (b0 == 4) {
				createBurst(aint, aint1, flag3, flag2);
			} else {
				createBall(0.25D, 2, aint, aint1, flag3, flag2);
			}
			...
			EntityFireworkOverlayFX entityfireworkoverlayfx = new EntityFireworkOverlayFX(worldObj, posX, posY, posZ);
			entityfireworkoverlayfx.setRBGColorF(f, f1, f2);
			theEffectRenderer.addEffect(entityfireworkoverlayfx);
		}
	...
	private boolean func_92037_i() {
		...
		return minecraft == null || minecraft.renderViewEntity == null
				|| minecraft.renderViewEntity.getDistanceSq(posX, posY, posZ) >= 256.0D;
	}
	...
	private void createParticle(double p_92034_1_, double p_92034_3_, double p_92034_5_, double p_92034_7_,
			...
			boolean p_92034_16_) {
		EntityFireworkSparkFX entityfireworksparkfx = new EntityFireworkSparkFX(worldObj, p_92034_1_, p_92034_3_,
				p_92034_5_, p_92034_7_, p_92034_9_, p_92034_11_, theEffectRenderer);
		...
		if (p_92034_14_ != null && p_92034_14_.length > 0) {
			entityfireworksparkfx.setFadeColour(p_92034_14_[rand.nextInt(p_92034_14_.length)]);
		}

		theEffectRenderer.addEffect(entityfireworksparkfx);
	}
		...
		for (int j = -p_92035_3_; j <= p_92035_3_; ++j) {
			for (int k = -p_92035_3_; k <= p_92035_3_; ++k) {
				for (int l = -p_92035_3_; l <= p_92035_3_; ++l) {
					...
					double d7 = MathHelper.sqrt_double(d4 * d4 + d5 * d5 + d6 * d6) / p_92035_1_
							+ rand.nextGaussian() * 0.05D;
					createParticle(d1, d2, d3, d4 / d7, d5 / d7, d6 / d7, p_92035_4_, p_92035_5_, p_92035_6_,
							p_92035_7_);
	...
	private void createShaped(double p_92038_1_, double[][] p_92038_3_, int[] p_92038_4_, int[] p_92038_5_,
		...
		createParticle(posX, posY, posZ, d1 * p_92038_1_, d2 * p_92038_1_, 0.0D, p_92038_4_, p_92038_5_, p_92038_6_,
				p_92038_7_);
			...
			for (int j = 1; j < p_92038_3_.length; ++j) {
				...
				for (double d9 = 0.25D; d9 <= 1.0D; d9 += 0.25D) {
					...
					for (double d13 = -1.0D; d13 <= 1.0D; d13 += 2.0D) {
						createParticle(posX, posY, posZ, d10 * d13, d11, d12 * d13, p_92038_4_, p_92038_5_, p_92038_6_,
								p_92038_7_);
					}
				}
	...
	private void createBurst(int[] p_92036_1_, int[] p_92036_2_, boolean p_92036_3_, boolean p_92036_4_) {
		...
		for (int i = 0; i < 70; ++i) {
			...
			createParticle(posX, posY, posZ, d2, d4, d3, p_92036_1_, p_92036_2_, p_92036_3_, p_92036_4_);
		}
	}
