...
public class RenderGlobal implements IWorldAccess {
	...
	public RenderGlobal(Minecraft p_i1249_1_) {
		mc = p_i1249_1_;
		renderEngine = p_i1249_1_.getTextureManager();
		byte b0 = 34;
		byte b1 = 16;
		...
		if (occlusionEnabled) {
			...
			glOcclusionQueryBase = GLAllocation.createDirectIntBuffer(b0 * b0 * b1);
			glOcclusionQueryBase.clear();
			glOcclusionQueryBase.position(0);
			glOcclusionQueryBase.limit(b0 * b0 * b1);
	...
	public void setWorldAndLoadRenderers(WorldClient p_72732_1_) {
		...
		theWorld = p_72732_1_;
		renderBlocksRg = new RenderBlocks(p_72732_1_);
	...
	public void loadRenderers() {
		if (theWorld != null) {
			...
			if (worldRenderers != null) {
				for (i = 0; i < worldRenderers.length; ++i) {
					worldRenderers[i].stopRendering();
				}
			}

			i = renderDistanceChunks * 2 + 1;
			renderChunksWide = i;
			renderChunksTall = 16;
			renderChunksDeep = i;
			...
			for (l = 0; l < worldRenderersToUpdate.size(); ++l) {
				((WorldRenderer) worldRenderersToUpdate.get(l)).needsUpdate = false;
			}
			...
			for (l = 0; l < renderChunksWide; ++l) {
				for (int i1 = 0; i1 < renderChunksTall; ++i1) {
					for (int j1 = 0; j1 < renderChunksDeep; ++j1) {
						worldRenderers[(j1 * renderChunksTall + i1) * renderChunksWide + l] = new WorldRenderer(
								theWorld, tileEntities, l * 16, i1 * 16, j1 * 16, glRenderListBase + j);

						if (occlusionEnabled) {
							worldRenderers[(j1 * renderChunksTall + i1) * renderChunksWide
									+ l].glOcclusionQuery = glOcclusionQueryBase.get(k);
						}
			...
			if (theWorld != null) {
				...
				if (entitylivingbase != null) {
					markRenderersForNewPosition(MathHelper.floor_double(entitylivingbase.posX),
	...
	public void renderEntities(EntityLivingBase p_147589_1_, ICamera p_147589_2_, float p_147589_3_) {
		...
		if (renderEntitiesStartupCounter > 0) {
			...
			double d0 = p_147589_1_.prevPosX + (p_147589_1_.posX - p_147589_1_.prevPosX) * p_147589_3_;
			double d1 = p_147589_1_.prevPosY + (p_147589_1_.posY - p_147589_1_.prevPosY) * p_147589_3_;
			double d2 = p_147589_1_.prevPosZ + (p_147589_1_.posZ - p_147589_1_.prevPosZ) * p_147589_3_;
			...
			TileEntityRendererDispatcher.instance.cacheActiveRenderInfo(theWorld, mc.getTextureManager(),
					mc.fontRenderer, mc.renderViewEntity, p_147589_3_);
			RenderManager.instance.cacheActiveRenderInfo(theWorld, mc.getTextureManager(), mc.fontRenderer,
					mc.renderViewEntity, mc.pointedEntity, mc.gameSettings, p_147589_3_);
			...
			EntityLivingBase entitylivingbase1 = mc.renderViewEntity;
			double d3 = entitylivingbase1.lastTickPosX
					+ (entitylivingbase1.posX - entitylivingbase1.lastTickPosX) * p_147589_3_;
			double d4 = entitylivingbase1.lastTickPosY
					+ (entitylivingbase1.posY - entitylivingbase1.lastTickPosY) * p_147589_3_;
			double d5 = entitylivingbase1.lastTickPosZ
					+ (entitylivingbase1.posZ - entitylivingbase1.lastTickPosZ) * p_147589_3_;
			...
			for (i = 0; i < theWorld.weatherEffects.size(); ++i) {
				entity = (Entity) theWorld.weatherEffects.get(i);
			...
			for (i = 0; i < list.size(); ++i) {
				...
				boolean flag = entity.isInRangeToRender3d(d0, d1, d2)
						&& (entity.ignoreFrustumCheck || p_147589_2_.isBoundingBoxInFrustum(entity.boundingBox)
				...
				if (flag && (entity != mc.renderViewEntity || mc.gameSettings.thirdPersonView != 0
						|| mc.renderViewEntity.isPlayerSleeping())
						&& theWorld.blockExists(MathHelper.floor_double(entity.posX), 0,
	...
	public String getDebugInfoRenders() {
		return "C: " + renderersBeingRendered + "/" + renderersLoaded + ". F: " + renderersBeingClipped + ", O: "
				+ renderersBeingOccluded + ", E: " + renderersSkippingRenderPass;
	}
	...
	public int sortAndRender(EntityLivingBase p_72719_1_, int p_72719_2_, double p_72719_3_) {
		...
		double d3 = p_72719_1_.posX - prevSortX;
		double d4 = p_72719_1_.posY - prevSortY;
		double d5 = p_72719_1_.posZ - prevSortZ;

		if (prevChunkSortX != p_72719_1_.chunkCoordX || prevChunkSortY != p_72719_1_.chunkCoordY
				|| prevChunkSortZ != p_72719_1_.chunkCoordZ || d3 * d3 + d4 * d4 + d5 * d5 > 16.0D) {
			prevSortX = p_72719_1_.posX;
			prevSortY = p_72719_1_.posY;
			prevSortZ = p_72719_1_.posZ;
			prevChunkSortX = p_72719_1_.chunkCoordX;
			prevChunkSortY = p_72719_1_.chunkCoordY;
			prevChunkSortZ = p_72719_1_.chunkCoordZ;
			markRenderersForNewPosition(MathHelper.floor_double(p_72719_1_.posX),
					MathHelper.floor_double(p_72719_1_.posY), MathHelper.floor_double(p_72719_1_.posZ));
			Arrays.sort(sortedWorldRenderers, new EntitySorter(p_72719_1_));
		}

		double d6 = p_72719_1_.posX - prevRenderSortX;
		double d7 = p_72719_1_.posY - prevRenderSortY;
		double d8 = p_72719_1_.posZ - prevRenderSortZ;
		int k;

		if (d6 * d6 + d7 * d7 + d8 * d8 > 1.0D) {
			prevRenderSortX = p_72719_1_.posX;
			prevRenderSortY = p_72719_1_.posY;
			prevRenderSortZ = p_72719_1_.posZ;

			for (k = 0; k < 27; ++k) {
				sortedWorldRenderers[k].updateRendererSort(p_72719_1_);
			}
		}
		...
		if (occlusionEnabled && mc.gameSettings.advancedOpengl && !mc.gameSettings.anaglyph && p_72719_2_ == 0) {
			byte b0 = 0;
			int l = 16;
			checkOcclusionQueryResult(b0, l);
			...
			k = b1 + renderSortedRenderers(b0, l, p_72719_2_, p_72719_3_);
			...
			do {
				...
				checkOcclusionQueryResult(l1, l);
				theWorld.theProfiler.endSection();
				GL11.glPushMatrix();
				float f9 = 0.0F;
				float f = 0.0F;
				...
				for (int j1 = l1; j1 < l; ++j1) {
					if (sortedWorldRenderers[j1].skipAllRenderPasses()) {
						...
						if (sortedWorldRenderers[j1].isInFrustum
							...
							float f2 = MathHelper
									.sqrt_float(sortedWorldRenderers[j1].distanceToEntitySquared(p_72719_1_));
							int k1 = (int) (1.0F + f2 / 128.0F);

							if (cloudTickCounter % k1 == j1 % k1) {
								WorldRenderer worldrenderer1 = sortedWorldRenderers[j1];
								float f3 = (float) (worldrenderer1.posXMinus - d9);
								float f4 = (float) (worldrenderer1.posYMinus - d1);
								float f5 = (float) (worldrenderer1.posZMinus - d2);
								float f6 = f3 - f9;
								float f7 = f4 - f;
								float f8 = f5 - f1;
				...
				theWorld.theProfiler.endStartSection("render");
				k += renderSortedRenderers(l1, l, p_72719_2_, p_72719_3_);
			} while (l < sortedWorldRenderers.length);
		} else {
			theWorld.theProfiler.endStartSection("render");
			k = b1 + renderSortedRenderers(0, sortedWorldRenderers.length, p_72719_2_, p_72719_3_);
		}
	...
	private void checkOcclusionQueryResult(int p_72720_1_, int p_72720_2_) {
		for (int k = p_72720_1_; k < p_72720_2_; ++k) {
			if (sortedWorldRenderers[k].isWaitingOnOcclusionQuery) {
				occlusionResult.clear();
				ARBOcclusionQuery.glGetQueryObjectuARB(sortedWorldRenderers[k].glOcclusionQuery,
						ARBOcclusionQuery.GL_QUERY_RESULT_AVAILABLE_ARB, occlusionResult);
	...
	private int renderSortedRenderers(int p_72724_1_, int p_72724_2_, int p_72724_3_, double p_72724_4_) {
		...
		if (p_72724_3_ == 1) {
			i1 = sortedWorldRenderers.length - 1 - p_72724_1_;
			j1 = sortedWorldRenderers.length - 1 - p_72724_2_;
			b0 = -1;
		}
		...
		for (int k1 = i1; k1 != j1; k1 += b0) {
			if (p_72724_3_ == 0) {
				...
				if (sortedWorldRenderers[k1].skipRenderPass[p_72724_3_]) {
					++renderersSkippingRenderPass;
				} else if (!sortedWorldRenderers[k1].isInFrustum) {
					++renderersBeingClipped;
				} else if (occlusionEnabled && !sortedWorldRenderers[k1].isVisible) {
			...
			if (!sortedWorldRenderers[k1].skipRenderPass[p_72724_3_] && sortedWorldRenderers[k1].isInFrustum
					&& (!occlusionEnabled || sortedWorldRenderers[k1].isVisible)) {
				int l1 = sortedWorldRenderers[k1].getGLCallListForPass(p_72724_3_);

				if (l1 >= 0) {
					glRenderLists.add(sortedWorldRenderers[k1]);
					++l;
				}
			}
		}
		...
		EntityLivingBase entitylivingbase = mc.renderViewEntity;
		double d3 = entitylivingbase.lastTickPosX
				+ (entitylivingbase.posX - entitylivingbase.lastTickPosX) * p_72724_4_;
		double d1 = entitylivingbase.lastTickPosY
				+ (entitylivingbase.posY - entitylivingbase.lastTickPosY) * p_72724_4_;
		double d2 = entitylivingbase.lastTickPosZ
				+ (entitylivingbase.posZ - entitylivingbase.lastTickPosZ) * p_72724_4_;
		int i2 = 0;
		int j2;
		...
		for (j2 = 0; j2 < glRenderLists.size(); ++j2) {
			WorldRenderer worldrenderer = (WorldRenderer) glRenderLists.get(j2);
			k2 = -1;

			for (l2 = 0; l2 < i2; ++l2) {
				if (allRenderLists[l2].rendersChunk(worldrenderer.posXMinus, worldrenderer.posYMinus,
			...
			if (k2 < 0) {
				k2 = i2++;
				allRenderLists[k2].setupRenderList(worldrenderer.posXMinus, worldrenderer.posYMinus,
						worldrenderer.posZMinus, d3, d1, d2);
			}

			allRenderLists[k2].addGLRenderList(worldrenderer.getGLCallListForPass(p_72724_3_));
		}
		...
		renderAllRenderLists(p_72724_3_, p_72724_4_);
		return l;
	}

	public void renderAllRenderLists(int p_72733_1_, double p_72733_2_) {
		mc.entityRenderer.enableLightmap(p_72733_2_);

		for (int j = 0; j < allRenderLists.length; ++j) {
			allRenderLists[j].callLists();
		}

		mc.entityRenderer.disableLightmap(p_72733_2_);
	}
	...
	public void renderSky(float p_72714_1_) {
		...
		if ((skyProvider = mc.theWorld.provider.getSkyRenderer()) != null) {
			skyProvider.render(p_72714_1_, theWorld, mc);
			return;
		}
		if (mc.theWorld.provider.dimensionId == 1) {
			...
			OpenGlHelper.glBlendFunc(770, 771, 1, 0);
			RenderHelper.disableStandardItemLighting();
			float[] afloat = theWorld.provider.calcSunriseSunsetColors(theWorld.getCelestialAngle(p_72714_1_),
					p_72714_1_);
			float f7;
			float f8;
			float f9;
			...
			if (afloat != null) {
				...
				GL11.glPushMatrix();
				GL11.glRotatef(90.0F, 1.0F, 0.0F, 0.0F);
				GL11.glRotatef(MathHelper.sin(theWorld.getCelestialAngleRadians(p_72714_1_)) < 0.0F ? 180.0F : 0.0F,
						0.0F, 0.0F, 1.0F);
				GL11.glRotatef(90.0F, 0.0F, 0.0F, 1.0F);
				f6 = afloat[0];
				f7 = afloat[1];
			...
			GL11.glPushMatrix();
			f6 = 1.0F - theWorld.getRainStrength(p_72714_1_);
			f7 = 0.0F;
			f8 = 0.0F;
			f9 = 0.0F;
			...
			tessellator1.draw();
			GL11.glDisable(GL11.GL_TEXTURE_2D);
			float f18 = theWorld.getStarBrightness(p_72714_1_) * f6;
	...
	public void renderClouds(float p_72718_1_) {
		...
		if (mc.theWorld.provider.isSurfaceWorld()) {
			if (mc.gameSettings.fancyGraphics) {
				renderCloudsFancy(p_72718_1_);
			} else {
				GL11.glDisable(GL11.GL_CULL_FACE);
				float f1 = (float) (mc.renderViewEntity.lastTickPosY
						+ (mc.renderViewEntity.posY - mc.renderViewEntity.lastTickPosY) * p_72718_1_);
				byte b0 = 32;
				int i = 256 / b0;
				Tessellator tessellator = Tessellator.instance;
				...
				GL11.glEnable(GL11.GL_BLEND);
				OpenGlHelper.glBlendFunc(770, 771, 1, 0);
				Vec3 vec3 = theWorld.getCloudColour(p_72718_1_);
				...
				f5 = 4.8828125E-4F;
				double d2 = cloudTickCounter + p_72718_1_;
				double d0 = mc.renderViewEntity.prevPosX
						+ (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * p_72718_1_
						+ d2 * 0.029999999329447746D;
				double d1 = mc.renderViewEntity.prevPosZ
						+ (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ) * p_72718_1_;
				int j = MathHelper.floor_double(d0 / 2048.0D);
				int k = MathHelper.floor_double(d1 / 2048.0D);
				d0 -= j * 2048;
				d1 -= k * 2048;
				float f8 = theWorld.provider.getCloudHeight() - f1 + 0.33F;
	...
	public void renderCloudsFancy(float p_72736_1_) {
		GL11.glDisable(GL11.GL_CULL_FACE);
		float f1 = (float) (mc.renderViewEntity.lastTickPosY
				+ (mc.renderViewEntity.posY - mc.renderViewEntity.lastTickPosY) * p_72736_1_);
		Tessellator tessellator = Tessellator.instance;
		float f2 = 12.0F;
		float f3 = 4.0F;
		double d0 = cloudTickCounter + p_72736_1_;
		double d1 = (mc.renderViewEntity.prevPosX
				+ (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * p_72736_1_ + d0 * 0.029999999329447746D)
				/ f2;
		double d2 = (mc.renderViewEntity.prevPosZ
				+ (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ) * p_72736_1_) / f2 + 0.33000001311302185D;
		...
		GL11.glEnable(GL11.GL_BLEND);
		OpenGlHelper.glBlendFunc(770, 771, 1, 0);
		Vec3 vec3 = theWorld.getCloudColour(p_72736_1_);
			...
			for (int l = -b1 + 1; l <= b1; ++l) {
				for (int i1 = -b1 + 1; i1 <= b1; ++i1) {
					...
					if (f4 > -f3 - 1.0F) {
						...
						tessellator.addVertexWithUV(f16 + 0.0F, f4 + 0.0F, f17 + b0, (f14 + 0.0F) * f10 + f8,
								(f15 + b0) * f10 + f9);
						tessellator.addVertexWithUV(f16 + b0, f4 + 0.0F, f17 + b0, (f14 + b0) * f10 + f8,
								(f15 + b0) * f10 + f9);
						tessellator.addVertexWithUV(f16 + b0, f4 + 0.0F, f17 + 0.0F, (f14 + b0) * f10 + f8,
								(f15 + 0.0F) * f10 + f9);
						tessellator.addVertexWithUV(f16 + 0.0F, f4 + 0.0F, f17 + 0.0F, (f14 + 0.0F) * f10 + f8,
								(f15 + 0.0F) * f10 + f9);
					}
					...
					if (f4 <= f3 + 1.0F) {
						...
						tessellator.setNormal(0.0F, 1.0F, 0.0F);
						tessellator.addVertexWithUV(f16 + 0.0F, f4 + f3 - f13, f17 + b0, (f14 + 0.0F) * f10 + f8,
								(f15 + b0) * f10 + f9);
						tessellator.addVertexWithUV(f16 + b0, f4 + f3 - f13, f17 + b0, (f14 + b0) * f10 + f8,
								(f15 + b0) * f10 + f9);
						tessellator.addVertexWithUV(f16 + b0, f4 + f3 - f13, f17 + 0.0F, (f14 + b0) * f10 + f8,
								(f15 + 0.0F) * f10 + f9);
						tessellator.addVertexWithUV(f16 + 0.0F, f4 + f3 - f13, f17 + 0.0F, (f14 + 0.0F) * f10 + f8,
								(f15 + 0.0F) * f10 + f9);
					}
					...
					if (l > -1) {
						...
						for (j1 = 0; j1 < b0; ++j1) {
							tessellator.addVertexWithUV(f16 + j1 + 0.0F, f4 + 0.0F, f17 + b0,
									(f14 + j1 + 0.5F) * f10 + f8, (f15 + b0) * f10 + f9);
							tessellator.addVertexWithUV(f16 + j1 + 0.0F, f4 + f3, f17 + b0,
									(f14 + j1 + 0.5F) * f10 + f8, (f15 + b0) * f10 + f9);
							tessellator.addVertexWithUV(f16 + j1 + 0.0F, f4 + f3, f17 + 0.0F,
									(f14 + j1 + 0.5F) * f10 + f8, (f15 + 0.0F) * f10 + f9);
							tessellator.addVertexWithUV(f16 + j1 + 0.0F, f4 + 0.0F, f17 + 0.0F,
									(f14 + j1 + 0.5F) * f10 + f8, (f15 + 0.0F) * f10 + f9);
						}
					}
					...
					if (l <= 1) {
						...
						for (j1 = 0; j1 < b0; ++j1) {
							...
							tessellator.addVertexWithUV(f16 + j1 + 1.0F - f13, f4 + f3, f17 + b0,
									(f14 + j1 + 0.5F) * f10 + f8, (f15 + b0) * f10 + f9);
							tessellator.addVertexWithUV(f16 + j1 + 1.0F - f13, f4 + f3, f17 + 0.0F,
									(f14 + j1 + 0.5F) * f10 + f8, (f15 + 0.0F) * f10 + f9);
					...
					if (i1 > -1) {
						...
						for (j1 = 0; j1 < b0; ++j1) {
							tessellator.addVertexWithUV(f16 + 0.0F, f4 + f3, f17 + j1 + 0.0F, (f14 + 0.0F) * f10 + f8,
									(f15 + j1 + 0.5F) * f10 + f9);
							tessellator.addVertexWithUV(f16 + b0, f4 + f3, f17 + j1 + 0.0F, (f14 + b0) * f10 + f8,
									(f15 + j1 + 0.5F) * f10 + f9);
							tessellator.addVertexWithUV(f16 + b0, f4 + 0.0F, f17 + j1 + 0.0F, (f14 + b0) * f10 + f8,
									(f15 + j1 + 0.5F) * f10 + f9);
							tessellator.addVertexWithUV(f16 + 0.0F, f4 + 0.0F, f17 + j1 + 0.0F, (f14 + 0.0F) * f10 + f8,
									(f15 + j1 + 0.5F) * f10 + f9);
						}
					}
					...
					if (i1 <= 1) {
						...
						for (j1 = 0; j1 < b0; ++j1) {
							tessellator.addVertexWithUV(f16 + 0.0F, f4 + f3, f17 + j1 + 1.0F - f13,
									(f14 + 0.0F) * f10 + f8, (f15 + j1 + 0.5F) * f10 + f9);
							tessellator.addVertexWithUV(f16 + b0, f4 + f3, f17 + j1 + 1.0F - f13, (f14 + b0) * f10 + f8,
									(f15 + j1 + 0.5F) * f10 + f9);
							tessellator.addVertexWithUV(f16 + b0, f4 + 0.0F, f17 + j1 + 1.0F - f13,
									(f14 + b0) * f10 + f8, (f15 + j1 + 0.5F) * f10 + f9);
	...
	public boolean updateRenderers(EntityLivingBase p_72716_1_, boolean p_72716_2_) {
		...
		for (k = 0; k < i; ++k) {
			worldrenderer = (WorldRenderer) worldRenderersToUpdate.get(k);
		...
		for (i1 = worldRenderersToUpdate.size(); k1 != i1; ++k1) {
			WorldRenderer worldrenderer1 = (WorldRenderer) worldRenderersToUpdate.get(k1);
			...
			if (worldrenderer1 != null) {
				...
				if (!flag1) {
					if (l != k1) {
						worldRenderersToUpdate.set(l, worldrenderer1);
					}
	...
	public void drawBlockDamageTexture(Tessellator p_72717_1_, EntityLivingBase p_72717_2_, float p_72717_3_) {
		double d0 = p_72717_2_.lastTickPosX + (p_72717_2_.posX - p_72717_2_.lastTickPosX) * p_72717_3_;
		double d1 = p_72717_2_.lastTickPosY + (p_72717_2_.posY - p_72717_2_.lastTickPosY) * p_72717_3_;
		double d2 = p_72717_2_.lastTickPosZ + (p_72717_2_.posZ - p_72717_2_.lastTickPosZ) * p_72717_3_;
		...
		if (!damagedBlocks.isEmpty()) {
			...
			while (iterator.hasNext()) {
				DestroyBlockProgress destroyblockprogress = (DestroyBlockProgress) iterator.next();
				double d3 = destroyblockprogress.getPartialBlockX() - d0;
				double d4 = destroyblockprogress.getPartialBlockY() - d1;
				double d5 = destroyblockprogress.getPartialBlockZ() - d2;

				if (d3 * d3 + d4 * d4 + d5 * d5 > 1024.0D) {
					iterator.remove();
				} else {
					Block block = theWorld.getBlock(destroyblockprogress.getPartialBlockX(),
							destroyblockprogress.getPartialBlockY(), destroyblockprogress.getPartialBlockZ());

					if (block.getMaterial() != Material.air) {
						renderBlocksRg.renderBlockUsingTexture(block, destroyblockprogress.getPartialBlockX(),
								destroyblockprogress.getPartialBlockY(), destroyblockprogress.getPartialBlockZ(),
								destroyBlockIcons[destroyblockprogress.getPartialBlockDamage()]);
					}
				}
			}
	...
	public void drawSelectionBox(EntityPlayer p_72731_1_, MovingObjectPosition p_72731_2_, int p_72731_3_,
		...
		if (p_72731_3_ == 0 && p_72731_2_.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
			...
			GL11.glDepthMask(false);
			float f1 = 0.002F;
			Block block = theWorld.getBlock(p_72731_2_.blockX, p_72731_2_.blockY, p_72731_2_.blockZ);

			if (block.getMaterial() != Material.air) {
				block.setBlockBoundsBasedOnState(theWorld, p_72731_2_.blockX, p_72731_2_.blockY, p_72731_2_.blockZ);
				double d0 = p_72731_1_.lastTickPosX + (p_72731_1_.posX - p_72731_1_.lastTickPosX) * p_72731_4_;
				double d1 = p_72731_1_.lastTickPosY + (p_72731_1_.posY - p_72731_1_.lastTickPosY) * p_72731_4_;
				double d2 = p_72731_1_.lastTickPosZ + (p_72731_1_.posZ - p_72731_1_.lastTickPosZ) * p_72731_4_;
				drawOutlinedBoundingBox(block.getSelectedBoundingBoxFromPool(theWorld, p_72731_2_.blockX,
						p_72731_2_.blockY, p_72731_2_.blockZ).expand(f1, f1, f1).getOffsetBoundingBox(-d0, -d1, -d2),
						-1);
			}
	...
	public void markBlockForUpdate(int p_147586_1_, int p_147586_2_, int p_147586_3_) {
		markBlocksForUpdate(p_147586_1_ - 1, p_147586_2_ - 1, p_147586_3_ - 1, p_147586_1_ + 1, p_147586_2_ + 1,
				p_147586_3_ + 1);
	}

	@Override
	public void markBlockForRenderUpdate(int p_147588_1_, int p_147588_2_, int p_147588_3_) {
		markBlocksForUpdate(p_147588_1_ - 1, p_147588_2_ - 1, p_147588_3_ - 1, p_147588_1_ + 1, p_147588_2_ + 1,
				p_147588_3_ + 1);
	}
	...
	public void markBlockRangeForRenderUpdate(int p_147585_1_, int p_147585_2_, int p_147585_3_, int p_147585_4_,
		...
		markBlocksForUpdate(p_147585_1_ - 1, p_147585_2_ - 1, p_147585_3_ - 1, p_147585_4_ + 1, p_147585_5_ + 1,
				p_147585_6_ + 1);
	}
	...
	public void clipRenderersByFrustum(ICamera p_72729_1_, float p_72729_2_) {
		for (int i = 0; i < worldRenderers.length; ++i) {
			if (!worldRenderers[i].skipAllRenderPasses()
					&& (!worldRenderers[i].isInFrustum || (i + frustumCheckOffset & 15) == 0)) {
				worldRenderers[i].updateInFrustum(p_72729_1_);
			}
		}
	...
	public void playRecord(String p_72702_1_, int p_72702_2_, int p_72702_3_, int p_72702_4_) {
		...
		ISound isound = (ISound) mapSoundPositions.get(chunkcoordinates);
		...
		if (p_72702_1_ != null) {
			...
			PositionedSoundRecord positionedsoundrecord = PositionedSoundRecord.func_147675_a(resource, p_72702_2_,
					p_72702_3_, p_72702_4_);
			mapSoundPositions.put(chunkcoordinates, positionedsoundrecord);
	...
	@Override
	public void spawnParticle(String p_72708_1_, final double p_72708_2_, final double p_72708_4_,
			final double p_72708_6_, double p_72708_8_, double p_72708_10_, double p_72708_12_) {
		try {
	...
	public EntityFX doSpawnParticle(String p_72726_1_, double p_72726_2_, double p_72726_4_, double p_72726_6_,
		...
		if (mc != null && mc.renderViewEntity != null && mc.effectRenderer != null) {
			...
			if (p_72726_1_.equals("hugeexplosion")) {
				mc.effectRenderer.addEffect(entityfx = new EntityHugeExplodeFX(theWorld, p_72726_2_, p_72726_4_,
						p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_));
			} else if (p_72726_1_.equals("largeexplode")) {
				mc.effectRenderer.addEffect(entityfx = new EntityLargeExplodeFX(renderEngine, theWorld, p_72726_2_,
						p_72726_4_, p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_));
			} else if (p_72726_1_.equals("fireworksSpark")) {
				mc.effectRenderer.addEffect(entityfx = new EntityFireworkSparkFX(theWorld, p_72726_2_, p_72726_4_,
						p_72726_6_, p_72726_8_, p_72726_10_, p_72726_12_, mc.effectRenderer));
			}
			...
			if (entityfx != null)
				...
				if (d6 * d6 + d7 * d7 + d8 * d8 > d9 * d9)
					...
					if (p_72726_1_.equals("bubble")) {
						...
						entityfx = new EntityFootStepFX(renderEngine, theWorld, p_72726_2_, p_72726_4_, p_72726_6_);
						...
						if (p_72726_1_.startsWith("iconcrack_")) {
							...
							if (p_72726_1_.startsWith("blockcrack_")) {
								...
								k = Integer.parseInt(astring[2]);
								entityfx = new EntityDiggingFX(theWorld, p_72726_2_, p_72726_4_, p_72726_6_, p_72726_8_,
										p_72726_10_, p_72726_12_, block, k).applyRenderColor(k);
	...
	public void broadcastSound(int p_82746_1_, int p_82746_2_, int p_82746_3_, int p_82746_4_, int p_82746_5_) {
		...
		case 1018:
			if (mc.renderViewEntity != null) {
				double d0 = p_82746_2_ - mc.renderViewEntity.posX;
				double d1 = p_82746_3_ - mc.renderViewEntity.posY;
				double d2 = p_82746_4_ - mc.renderViewEntity.posZ;
	...
	public void playAuxSFX(EntityPlayer p_72706_1_, int p_72706_2_, int p_72706_3_, int p_72706_4_, int p_72706_5_,
		...
		case 1003:
			if (Math.random() < 0.5D) {
				theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "random.door_open", 1.0F,
						theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			} else {
				theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "random.door_close", 1.0F,
						theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			}

			break;
		case 1004:
			theWorld.playSound(p_72706_3_ + 0.5F, p_72706_4_ + 0.5F, p_72706_5_ + 0.5F, "random.fizz", 0.5F,
		...
		case 1005:
			...
			break;
		case 1007:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.ghast.charge", 10.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1008:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.ghast.fireball", 10.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1009:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.ghast.fireball", 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1010:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.zombie.wood", 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1011:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.zombie.metal", 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1012:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.zombie.woodbreak", 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1014:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.wither.shoot", 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1015:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.bat.takeoff", 0.05F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1016:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.zombie.infect", 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1017:
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "mob.zombie.unfect", 2.0F,
					(random.nextFloat() - random.nextFloat()) * 0.2F + 1.0F, false);
			break;
		case 1020:
			theWorld.playSound(p_72706_3_ + 0.5F, p_72706_4_ + 0.5F, p_72706_5_ + 0.5F, "random.anvil_break", 1.0F,
					theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1021:
			theWorld.playSound(p_72706_3_ + 0.5F, p_72706_4_ + 0.5F, p_72706_5_ + 0.5F, "random.anvil_use", 1.0F,
					theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 1022:
			theWorld.playSound(p_72706_3_ + 0.5F, p_72706_4_ + 0.5F, p_72706_5_ + 0.5F, "random.anvil_land", 0.3F,
					theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 2000:
			int j2 = p_72706_6_ % 3 - 1;
			int j1 = p_72706_6_ / 3 % 3 - 1;
			d1 = p_72706_3_ + j2 * 0.6D + 0.5D;
			d2 = p_72706_4_ + 0.5D;
			double d9 = p_72706_5_ + j1 * 0.6D + 0.5D;

			for (int k2 = 0; k2 < 10; ++k2) {
				double d11 = random.nextDouble() * 0.2D + 0.01D;
				double d12 = d1 + j2 * 0.01D + (random.nextDouble() - 0.5D) * j1 * 0.5D;
				d4 = d2 + (random.nextDouble() - 0.5D) * 0.5D;
				d13 = d9 + j1 * 0.01D + (random.nextDouble() - 0.5D) * j2 * 0.5D;
				d5 = j2 * d11 + random.nextGaussian() * 0.01D;
				d6 = -0.03D + random.nextGaussian() * 0.01D;
				d7 = j1 * d11 + random.nextGaussian() * 0.01D;
				spawnParticle("smoke", d12, d4, d13, d5, d6, d7);
			}
			...
			if (block.getMaterial() != Material.air) {
				mc.getSoundHandler()
						.playSound(new PositionedSoundRecord(new ResourceLocation(block.stepSound.getBreakSound()),
								...
								p_72706_3_ + 0.5F, p_72706_4_ + 0.5F, p_72706_5_ + 0.5F));
			}

			mc.effectRenderer.addBlockDestroyEffects(p_72706_3_, p_72706_4_, p_72706_5_, block, p_72706_6_ >> 12 & 255);
			break;
		case 2002:
			d0 = p_72706_3_;
			d1 = p_72706_4_;
			...
			for (k1 = 0; k1 < 8; ++k1) {
				spawnParticle(s, d0, d1, d2, random.nextGaussian() * 0.15D, random.nextDouble() * 0.2D,
			...
			for (l2 = 0; l2 < 100; ++l2) {
				...
				d7 = Math.sin(d13) * d4;
				EntityFX entityfx = doSpawnParticle(s1, d0 + d5 * 0.1D, d1 + 0.3D, d2 + d7 * 0.1D, d5, d6, d7);
			...
			theWorld.playSound(p_72706_3_ + 0.5D, p_72706_4_ + 0.5D, p_72706_5_ + 0.5D, "game.potion.smash", 1.0F,
					theWorld.rand.nextFloat() * 0.1F + 0.9F, false);
			break;
		case 2003:
			d0 = p_72706_3_ + 0.5D;
			...
			for (double d10 = 0.0D; d10 < Math.PI * 2D; d10 += 0.15707963267948966D) {
				spawnParticle("portal", d0 + Math.cos(d10) * 5.0D, d1 - 0.4D, d2 + Math.sin(d10) * 5.0D,
						Math.cos(d10) * -5.0D, 0.0D, Math.sin(d10) * -5.0D);
				spawnParticle("portal", d0 + Math.cos(d10) * 5.0D, d1 - 0.4D, d2 + Math.sin(d10) * 5.0D,
						Math.cos(d10) * -7.0D, 0.0D, Math.sin(d10) * -7.0D);
			}

			return;
		case 2004:
			for (l2 = 0; l2 < 20; ++l2) {
				d4 = p_72706_3_ + 0.5D + (theWorld.rand.nextFloat() - 0.5D) * 2.0D;
				d13 = p_72706_4_ + 0.5D + (theWorld.rand.nextFloat() - 0.5D) * 2.0D;
				d5 = p_72706_5_ + 0.5D + (theWorld.rand.nextFloat() - 0.5D) * 2.0D;
				theWorld.spawnParticle("smoke", d4, d13, d5, 0.0D, 0.0D, 0.0D);
				theWorld.spawnParticle("flame", d4, d13, d5, 0.0D, 0.0D, 0.0D);
			}

			return;
		case 2005:
			ItemDye.func_150918_a(theWorld, p_72706_3_, p_72706_4_, p_72706_5_, p_72706_6_);
			break;
		case 2006:
			block = theWorld.getBlock(p_72706_3_, p_72706_4_, p_72706_5_);

			if (block.getMaterial() != Material.air) {
				double d3 = Math.min(0.2F + p_72706_6_ / 15.0F, 10.0F);
				...
				for (int i2 = 0; i2 < l1; ++i2) {
					...
					d5 = MathHelper.randomFloatClamp(random, 0.75F, 1.0F);
					d6 = 0.20000000298023224D + d3 / 100.0D;
					d7 = MathHelper.cos(f3) * 0.2F * d5 * d5 * (d3 + 0.2D);
					double d8 = MathHelper.sin(f3) * 0.2F * d5 * d5 * (d3 + 0.2D);
					theWorld.spawnParticle(
							"blockdust_" + Block.getIdFromBlock(block) + "_"
									+ theWorld.getBlockMetadata(p_72706_3_, p_72706_4_, p_72706_5_),
							p_72706_3_ + 0.5F, p_72706_4_ + 1.0F, p_72706_5_ + 0.5F, d7, d6, d8);
				}
			}
		}
	}
	...
	public void destroyBlockPartially(int p_147587_1_, int p_147587_2_, int p_147587_3_, int p_147587_4_,
		...
		if (p_147587_5_ >= 0 && p_147587_5_ < 10) {
			DestroyBlockProgress destroyblockprogress = (DestroyBlockProgress) damagedBlocks
			...
			if (destroyblockprogress == null || destroyblockprogress.getPartialBlockX() != p_147587_2_
				...
				damagedBlocks.put(Integer.valueOf(p_147587_1_), destroyblockprogress);
			}
	...
	public void registerDestroyBlockIcons(IIconRegister p_94140_1_) {
		...
		for (int i = 0; i < destroyBlockIcons.length; ++i) {
			destroyBlockIcons[i] = p_94140_1_.registerIcon("destroy_stage_" + i);
		}
	}
}