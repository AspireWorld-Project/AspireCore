...
public class TextureMap extends AbstractTexture implements ITickableTextureObject, IIconRegister {
	...
	public void loadTexture(IResourceManager p_110551_1_) throws IOException {
		initMissingImage();
		deleteGlTexture();
		loadTextureAtlas(p_110551_1_);
	}

	public void loadTextureAtlas(IResourceManager p_110571_1_) {
		registerIcons(); // Re-gather list of Icons, allows for addition/removal of blocks/items after
							// this map was initially constructed.
		...
		cpw.mods.fml.common.ProgressManager.ProgressBar bar = cpw.mods.fml.common.ProgressManager
				.push("Texture Loading", skipFirst ? 0 : mapRegisteredSprites.size());
		...
		while (!skipFirst && iterator.hasNext()) {
			...
			try {
				...
				textureatlassprite.loadSprite(abufferedimage, animationmetadatasection, anisotropicFiltering > 1.0F);
			} catch (RuntimeException runtimeexception) {
				// logger.error("Unable to parse metadata from " + resourcelocation1,
				// runtimeexception);
				cpw.mods.fml.client.FMLClientHandler.instance().trackBrokenTexture(resourcelocation1,
		...
		bar = cpw.mods.fml.common.ProgressManager.push("Mipmap generation",
				skipFirst ? 0 : mapRegisteredSprites.size());
		...
		while (iterator2.hasNext()) {
			...
			try {
				TextureUtil.uploadTextureMipmap(textureatlassprite.getFrameTextureData(0),
						textureatlassprite.getIconWidth(), textureatlassprite.getIconHeight(),
						textureatlassprite.getOriginX(), textureatlassprite.getOriginY(), false, false);
	...
	public TextureAtlasSprite getAtlasSprite(String p_110572_1_) {
		TextureAtlasSprite textureatlassprite = (TextureAtlasSprite) mapUploadedSprites.get(p_110572_1_);
	...
	public void updateAnimations() {
		...
		Iterator iterator = listAnimatedSprites.iterator();
	...
	public IIcon registerIcon(String p_94245_1_) {
		...
		{
			Object object = mapRegisteredSprites.get(p_94245_1_);
			...
			if (object == null) {
				...
				mapRegisteredSprites.put(p_94245_1_, object);
			}
	...
	public void setAnisotropicFiltering(int p_147632_1_) {
		anisotropicFiltering = p_147632_1_;
	}
	...
	// ===================================================================================================
	/**
	 * Grabs the registered entry for the specified name, returning null if there
	 * was not a entry. Opposed to registerIcon, this will not instantiate the
	 * entry, useful to test if a mapping exists.
	 *
	 * @param name
	...
	/**
	 * Adds a texture registry entry to this map for the specified name if one does
	 * not already exist. Returns false if the map already contains a entry for the
	 * specified name.
	 *
	 * @param name
	 *            Entry name
