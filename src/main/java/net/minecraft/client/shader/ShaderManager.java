...
public class ShaderManager {
	...
	public ShaderManager(IResourceManager p_i45087_1_, String p_i45087_2_) throws JsonException {
		...
		try {
			...
			if (jsonarray1 != null) {
				int j = 0;
				field_148015_q = Lists.newArrayListWithCapacity(jsonarray1.size());
				field_148014_r = Lists.newArrayListWithCapacity(jsonarray1.size());

				for (iterator1 = jsonarray1.iterator(); iterator1.hasNext(); ++j) {
					JsonElement jsonelement1 = (JsonElement) iterator1.next();

					try {
						field_148014_r.add(JsonUtils.getJsonElementStringValue(jsonelement1, "attribute"));
			...
			field_148006_l = ShaderLinkHelper.getStaticShaderLinkHelper().func_148078_c();
			ShaderLinkHelper.getStaticShaderLinkHelper().func_148075_b(this);
			func_147990_i();

			if (field_148014_r != null) {
				iterator1 = field_148014_r.iterator();

				while (iterator1.hasNext()) {
					String s3 = (String) iterator1.next();
					int l = OpenGlHelper.func_153164_b(field_148006_l, s3);
					field_148015_q.add(Integer.valueOf(l));
				}
			}
	...
	public void func_147993_b() {
		...
		for (int i = 0; i < field_148010_h.size(); ++i) {
			if (field_147997_f.get(field_147998_g.get(i)) != null) {
	...
	public void func_147995_c() {
		...
		for (int i = 0; i < field_148010_h.size(); ++i) {
			if (field_147997_f.get(field_147998_g.get(i)) != null) {
				...
				Object object = field_147997_f.get(field_147998_g.get(i));
				int j = -1;
				...
				if (j != -1) {
					...
					OpenGlHelper.func_153163_f(
							OpenGlHelper.func_153194_a(field_148006_l, (CharSequence) field_147998_g.get(i)), i);
				}
			}
		}

		Iterator iterator = field_148011_i.iterator();
	...
	public ShaderUniform func_147991_a(String p_147991_1_) {
		return field_148009_k.containsKey(p_147991_1_) ? (ShaderUniform) field_148009_k.get(p_147991_1_) : null;
	}

	public ShaderUniform func_147984_b(String p_147984_1_) {
		return field_148009_k.containsKey(p_147984_1_) ? (ShaderUniform) field_148009_k.get(p_147984_1_)
	...
	private void func_147990_i() {
		...
		for (int j = 0; i < field_147998_g.size(); ++j) {
			s = (String) field_147998_g.get(i);
			k = OpenGlHelper.func_153194_a(field_148006_l, s);

			if (k == -1) {
				logger.warn("Shader " + field_148007_m + "could not find sampler named " + s
						+ " in the specified shader program.");
				field_147997_f.remove(s);
				field_147998_g.remove(j);
				--j;
			} else {
				field_148010_h.add(Integer.valueOf(k));
			}
		...
		while (iterator.hasNext()) {
			...
			if (k == -1) {
				...
				field_148008_j.add(Integer.valueOf(k));
				shaderuniform.func_148084_b(k);
				field_148009_k.put(s, shaderuniform);
			}
		}
	}
	...
	private void func_147987_b(JsonElement p_147987_1_) throws JsonException {
		...
		if (jsonarray.size() != j && jsonarray.size() > 1)
			...
			field_148011_i.add(shaderuniform);
		}
	}
