	...
	public static EntityPlayerMP[] matchPlayers(ICommandSender p_82380_0_, String p_82380_1_) {
		...
		if (matcher.matches()) {
			...
			if (!s1.equals("p") && !s1.equals("a")) {
				if (s1.equals("r")) {
					list = MinecraftServer.getServer().getConfigurationManager().findPlayers(chunkcoordinates, i, j, 0,
							j1, k, l, map1, s2, s3, world);
					Collections.shuffle(list);
					list = list.subList(0, Math.min(i1, list.size()));
					return list.isEmpty() ? new EntityPlayerMP[0]
							: (EntityPlayerMP[]) list.toArray(new EntityPlayerMP[list.size()]);
				} else
					return null;
			} else {
				list = MinecraftServer.getServer().getConfigurationManager().findPlayers(chunkcoordinates, i, j, i1, j1,
						k, l, map1, s2, s3, world);
				return list.isEmpty() ? new EntityPlayerMP[0]
						: (EntityPlayerMP[]) list.toArray(new EntityPlayerMP[list.size()]);
			}
		} else
