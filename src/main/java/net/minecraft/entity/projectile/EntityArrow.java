...
public class EntityArrow extends Entity implements IProjectile {
	...
	public void onUpdate() {
		...
		if (block.getMaterial() != Material.air) {
			...
			AxisAlignedBB axisalignedbb = block.getCollisionBoundingBoxFromPool(worldObj, field_145791_d,
		...
		if (inGround) {
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					if (field_145790_g.getMaterial() != Material.air) {
						field_145790_g.onEntityCollidedWithBlock(worldObj, field_145791_d, field_145792_e,
								field_145789_f, this);
					}
				}
			}
			...
			if (getIsCritical()) {
				for (i = 0; i < 4; ++i) {
					worldObj.spawnParticle("crit", posX + motionX * i / 4.0D, posY + motionY * i / 4.0D,
							posZ + motionZ * i / 4.0D, -motionX, -motionY + 0.2D, -motionZ);
				}
			}
			...
			if (isInWater()) {
				for (int l = 0; l < 4; ++l) {
					f4 = 0.25F;
					worldObj.spawnParticle("bubble", posX - motionX * f4, posY - motionY * f4, posZ - motionZ * f4,
