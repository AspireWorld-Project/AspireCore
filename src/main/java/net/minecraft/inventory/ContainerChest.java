...
public class ContainerChest extends Container {
	...
	public ContainerChest(IInventory p_i1806_1_, IInventory p_i1806_2_) {
		...
		notifyInventory = !(p_i1806_1_ instanceof InventoryPlayer
				&& ((InventoryPlayer) p_i1806_1_).player.isEntityPlayerMP()
				&& ((EntityPlayerMP) ((InventoryPlayer) p_i1806_1_).player).isHidden());
		if (notifyInventory) {
		...
		for (j = 0; j < numRows; ++j) {
			for (k = 0; k < 9; ++k) {
				addSlotToContainer(new Slot(p_i1806_2_, k + j * 9, 8 + k * 18, 18 + j * 18));
			}
		}

		for (j = 0; j < 3; ++j) {
			for (k = 0; k < 9; ++k) {
				addSlotToContainer(new Slot(p_i1806_1_, k + j * 9 + 9, 8 + k * 18, 103 + j * 18 + i));
			}
		}

		for (j = 0; j < 9; ++j) {
			addSlotToContainer(new Slot(p_i1806_1_, j, 8 + j * 18, 161 + i));
		}
	}
