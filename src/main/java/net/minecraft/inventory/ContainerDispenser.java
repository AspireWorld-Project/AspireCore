...
public class ContainerDispenser extends Container {
	...
	public ContainerDispenser(IInventory p_i1825_1_, TileEntityDispenser p_i1825_2_) {
		tileDispenser = p_i1825_2_;
		int i;
		int j;

		for (i = 0; i < 3; ++i) {
			for (j = 0; j < 3; ++j) {
				addSlotToContainer(new Slot(p_i1825_2_, j + i * 3, 62 + j * 18, 17 + i * 18));
			}
		}

		for (i = 0; i < 3; ++i) {
			for (j = 0; j < 9; ++j) {
				addSlotToContainer(new Slot(p_i1825_1_, j + i * 9 + 9, 8 + j * 18, 84 + i * 18));
			}
		}

		for (i = 0; i < 9; ++i) {
			addSlotToContainer(new Slot(p_i1825_1_, i, 8 + i * 18, 142));
		}
	}

	@Override
	public boolean canInteractWith(EntityPlayer p_75145_1_) {
		return tileDispenser.isUseableByPlayer(p_75145_1_);
	}
