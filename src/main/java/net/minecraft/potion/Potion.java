...
public class Potion {
	...
	public static final Potion poison = new Potion(19, true, 5149489).setPotionName("potion.poison").setIconIndex(6, 0)
			.setEffectiveness(0.25D);
	public static final Potion wither = new Potion(20, true, 3484199).setPotionName("potion.wither").setIconIndex(1, 2)
			.setEffectiveness(0.25D);
		...
		if (id == regeneration.id) {
			if (p_76394_1_.getHealth() < p_76394_1_.getMaxHealth()) {
				if (id == Potion.regeneration.id) {
					p_76394_1_.heal(1.0F, EntityRegainHealthEvent.RegainReason.MAGIC_REGEN);
				} else {
					p_76394_1_.heal(1.0F, EntityRegainHealthEvent.RegainReason.MAGIC);
				}
			}
		} else if (id == poison.id) {
			if (p_76394_1_.getHealth() > 1.0F) {
				p_76394_1_.attackEntityFrom(CraftEventFactory.POISON, 1.0F);
			}
			...
			if (id == Potion.regeneration.id) {
				p_76394_1_.heal(Math.max(4 << p_76394_2_, 0), EntityRegainHealthEvent.RegainReason.MAGIC_REGEN);
			} else {
				p_76394_1_.heal(Math.max(4 << p_76394_2_, 0), EntityRegainHealthEvent.RegainReason.MAGIC);
			}
		}
	}
	...
	public Potion func_111184_a(IAttribute p_111184_1_, String p_111184_2_, double p_111184_3_, int p_111184_5_) {
		AttributeModifier attributemodifier = new AttributeModifier(UUID.fromString(p_111184_2_), getName(),
	...
	public void removeAttributesModifiersFromEntity(EntityLivingBase p_111187_1_, BaseAttributeMap p_111187_2_,
			int p_111187_3_) {
		Iterator iterator = field_111188_I.entrySet().iterator();
	...
	public void applyAttributesModifiersToEntity(EntityLivingBase p_111185_1_, BaseAttributeMap p_111185_2_,
		...
		while (iterator.hasNext()) {
			...
			if (iattributeinstance != null) {
				...
				iattributeinstance
						.applyModifier(new AttributeModifier(attributemodifier.getID(), getName() + " " + p_111185_3_,
								func_111183_a(p_111185_3_, attributemodifier), attributemodifier.getOperation()));
			}
		}
	}
	...
	/**
	 * If the standard PotionEffect text (name and duration) should be drawn when
	 * this potion is active.
	 *
	 * @param effect
	 *            the active PotionEffect
	...
	public void applyInstantEffect(EntityLivingBase p_76402_1_, EntityLivingBase p_76402_2_, int p_76402_3_,
			double p_76402_4_, EntityPotion potion) {
		// CraftBukkit end
		int j;

		if ((id != Potion.heal.id || p_76402_2_.isEntityUndead())
				&& (id != Potion.harm.id || !p_76402_2_.isEntityUndead())) {
			if (id == Potion.harm.id && !p_76402_2_.isEntityUndead()
					|| id == Potion.heal.id && p_76402_2_.isEntityUndead()) {
				j = (int) (p_76402_4_ * (6 << p_76402_3_) + 0.5D);

				if (p_76402_1_ == null) {
					p_76402_2_.attackEntityFrom(DamageSource.magic, j);
				} else {
					// CraftBukkit - The "damager" needs to be the potion
					p_76402_2_.attackEntityFrom(
							DamageSource.causeIndirectMagicDamage(potion != null ? potion : p_76402_2_, p_76402_1_), j);
				}
			}
		} else {
			j = (int) (p_76402_4_ * (4 << p_76402_3_) + 0.5D);
			p_76402_2_.heal(j, EntityRegainHealthEvent.RegainReason.MAGIC); // CraftBukkit
		}
	}
}