...
public class StatisticsFile extends StatFileWriter {
					...
					if (((JsonElement) entry.getValue()).isJsonPrimitive()
						...
						if (jsonobject1.has("progress") && statbase.func_150954_l() != null) {
							try {
								...
								logger.warn("Invalid statistic progress in " + field_150887_d, throwable);
							}
						}
					}
	...
	public void func_150876_a(EntityPlayerMP p_150876_1_) {
		...
		if (field_150886_g || i - field_150885_f > 300) {
			field_150885_f = i;
			Iterator iterator = func_150878_c().iterator();

			while (iterator.hasNext()) {
				StatBase statbase = (StatBase) iterator.next();
				hashmap.put(statbase, Integer.valueOf(writeStat(statbase)));
			}
		}
	...
	public void func_150884_b(EntityPlayerMP p_150884_1_) {
		...
		while (iterator.hasNext()) {
			...
			if (hasAchievementUnlocked(achievement)) {
				hashmap.put(achievement, Integer.valueOf(writeStat(achievement)));
