	...
	protected void decode(ChannelHandlerContext p_decode_1_, ByteBuf p_decode_2_, List p_decode_3_) throws IOException {
		...
		if (i != 0) {
			...
			Packet packet = Packet
					.generatePacket((BiMap) p_decode_1_.channel().attr(NetworkManager.attrKeyReceivable).get(), j);
			...
			if (packet == null)
				...
				if (packetbuffer.readableBytes() > 0)
					throw new IOException("Packet was larger than I expected, found " + packetbuffer.readableBytes()
							+ " bytes extra whilst reading packet " + j);
				else {
					p_decode_3_.add(packet);
					field_152499_c.func_152469_a(j, i);

					if (logger.isDebugEnabled()) {
						logger.debug(field_150799_b, " IN: [{}:{}] {}[{}]",
                                p_decode_1_.channel().attr(NetworkManager.attrKeyConnectionState).get(),
                                Integer.valueOf(j), packet.getClass().getName(), packet.serialize());
					}
				}
			}
		}
