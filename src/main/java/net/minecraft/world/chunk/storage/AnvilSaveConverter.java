...
public class AnvilSaveConverter extends SaveFormatOld {
			...
			for (int j = 0; j < i; ++j) {
				...
				if (file1.isDirectory()) {
					...
					if (worldinfo != null
						...
						long k = 0L;
						arraylist.add(new SaveFormatComparator(s, s1, worldinfo.getLastTimePlayed(), k,
	...
	public boolean convertMapFormat(String p_75805_1_, IProgressUpdate p_75805_2_) {
		...
		File file1 = new File(savesDirectory, p_75805_1_);
		File file2 = new File(file1, "DIM-1");
		File file3 = new File(file1, "DIM1");
		logger.info("Scanning folders...");
		addRegionFilesToCollection(file1, arraylist);

		if (file2.exists()) {
			addRegionFilesToCollection(file2, arraylist1);
		}

		if (file3.exists()) {
			addRegionFilesToCollection(file3, arraylist2);
		}

		int i = arraylist.size() + arraylist1.size() + arraylist2.size();
		logger.info("Total conversion count is " + i);
		WorldInfo worldinfo = getWorldInfo(p_75805_1_);
		Object object = null;
		...
		convertFile(new File(file1, "region"), arraylist, (WorldChunkManager) object, 0, i, p_75805_2_);
		convertFile(new File(file2, "region"), arraylist1, new WorldChunkManagerHell(BiomeGenBase.hell, 0.0F),
				arraylist.size(), i, p_75805_2_);
		convertFile(new File(file3, "region"), arraylist2, new WorldChunkManagerHell(BiomeGenBase.sky, 0.0F),
				arraylist.size() + arraylist1.size(), i, p_75805_2_);
		...
		ISaveHandler isavehandler = getSaveLoader(p_75805_1_, false);
		isavehandler.saveWorldInfo(worldinfo);
		return true;
	}

	private void createFile(String p_75809_1_) {
		File file1 = new File(savesDirectory, p_75809_1_);
	...
	private void convertFile(File p_75813_1_, Iterable p_75813_2_, WorldChunkManager p_75813_3_, int p_75813_4_,
		...
		while (iterator.hasNext()) {
			...
			convertChunks(p_75813_1_, file2, p_75813_3_, p_75813_4_, p_75813_5_, p_75813_6_);
			++p_75813_4_;
			int k = (int) Math.round(100.0D * p_75813_4_ / p_75813_5_);
	...
	private void convertChunks(File p_75811_1_, File p_75811_2_, WorldChunkManager p_75811_3_, int p_75811_4_,
		...
		try {
			...
			for (int k = 0; k < 32; ++k) {
				...
				l = (int) Math.round(100.0D * (p_75811_4_ * 1024) / (p_75811_5_ * 1024));
				int i1 = (int) Math.round(100.0D * ((k + 1) * 32 + p_75811_4_ * 1024) / (p_75811_5_ * 1024));
