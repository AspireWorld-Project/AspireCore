...
public class ExtendedBlockStorage {
		...
		for (int i = 0; i < 16; ++i) {
			for (int j = 0; j < 16; ++j) {
				for (int k = 0; k < 16; ++k) {
					...
					Block block = getBlockByExtId(k, i, j);
	...
	private static void logDeprecation() {
		log.warn(
				"Called deprecated method in ExtendedBlockStorage. It may have no effect intended by the modder or lead to performance issues",
				new Throwable());
	}
