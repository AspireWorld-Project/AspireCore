...
public class ForgeHooksClient {
	...
	public static boolean renderEntityItem(EntityItem entity, ItemStack item, float bobing, float rotation,
		...
		if (is3D || block != null && RenderBlocks.renderItemIn3d(block.getRenderType())) {
			...
			for (int j = 0; j < count; j++) {
				GL11.glPushMatrix();
				if (j > 0) {
					GL11.glTranslatef((random.nextFloat() * 2.0F - 1.0F) * 0.2F / scale,
	...
	public static boolean renderInventoryItem(RenderBlocks renderBlocks, TextureManager engine, ItemStack item,
			boolean inColor, float zLevel, float x, float y) {
	...
	public static void orientBedCamera(Minecraft mc, EntityLivingBase entity) {
		...
		if (block != null && block.isBed(mc.theWorld, x, y, z, entity)) {
			GL11.glRotatef(block.getBedDirection(mc.theWorld, x, y, z) * 90, 0.0F, 1.0F, 0.0F);
		}
	}

	public static boolean onDrawBlockHighlight(RenderGlobal context, EntityPlayer player, MovingObjectPosition target,
			int subID, ItemStack currentItem, float partialTicks) {
		return MinecraftForge.EVENT_BUS
				.post(new DrawBlockHighlightEvent(context, player, target, subID, currentItem, partialTicks));
	}
	...
	/**
	 * This is added for Optifine's convenience. And to explode if a ModMaker is
	 * developing.
	 *
	 * @param texture
	 */
				...
				if (Tessellator.class.getPackage().getName().startsWith("net.minecraft.")) {
					...
					if (mc.ingameGUI != null) {
						mc.ingameGUI.getChatGUI().printChatMessage(
								new ChatComponentTranslation("forge.texture.preload.warning", texture));
					}
				}
			}
		}
	...
	public static ModelBiped getArmorModel(EntityLivingBase entityLiving, ItemStack itemStack, int slotID,
			ModelBiped _default) {
	...
	// This properly moves the domain, if provided, to the front of the string
	...
	public static void renderMainMenu(GuiMainMenu gui, FontRenderer font, int width, int height) {
		...
		// case AHEAD: line = "Using non-recommended Forge build, issues may arise."};
		// break;
		case OUTDATED:
		case BETA_OUTDATED:
		...
		if (line != null) {
			// if we have a line, render it in the bottom right, above Mojang's copyright
			// line
	...
	public static ISound playSound(SoundManager manager, ISound sound) {
		...
		PlaySoundEvent17 e = new PlaySoundEvent17(manager, sound,
				accessor == null ? null : accessor.getSoundCategory());
	...
	public static void onPreRenderWorld(WorldRenderer worldRenderer, int pass) {
		if (worldRendererRB != null) {
			...
			MinecraftForge.EVENT_BUS.post(new RenderWorldEvent.Pre(worldRenderer,
					(ChunkCache) worldRendererRB.blockAccess, worldRendererRB, pass));
		}
	}

	public static void onPostRenderWorld(WorldRenderer worldRenderer, int pass) {
		if (worldRendererRB != null) {
			MinecraftForge.EVENT_BUS.post(new RenderWorldEvent.Post(worldRenderer,
					(ChunkCache) worldRendererRB.blockAccess, worldRendererRB, pass));
			worldRenderPass = -1;
		}
	}
