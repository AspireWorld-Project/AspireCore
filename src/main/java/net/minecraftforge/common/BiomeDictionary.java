...
import static net.minecraft.world.biome.BiomeGenBase.extremeHillsPlus;
...
import static net.minecraft.world.biome.BiomeGenBase.hell;
...
import static net.minecraft.world.biome.BiomeGenBase.jungleEdge;
import static net.minecraft.world.biome.BiomeGenBase.jungleHills;
import static net.minecraft.world.biome.BiomeGenBase.megaTaiga;
import static net.minecraft.world.biome.BiomeGenBase.megaTaigaHills;
...
public class BiomeDictionary {
	public enum Type {
		 ...
		 */
		SAVANNA, CONIFEROUS, JUNGLE,

		/* Tags specifying the nature of a biome */
		SPOOKY, DEAD, LUSH, NETHER, END, MUSHROOM, MAGICAL,
		...
		/**
		 * Retrieves a Type value by name, if one does not exist already it creates one.
		 * This can be used as interm measure for modders to add there own category of
		 * Biome.
		 *
		 * There are NO naming conventions besides: MUST be all upper case (enforced by
		 * name.toUpper()) NO Special characters. {Unenforced, just don't be a pain, if
		 * it becomes a issue I WILL make this RTE with no worry about backwards
		 * compatibility}
		 *
	 ...
	 * @param type
	 *            the type to register the biome as
	 ...
	 * @return returns true if the biome is registered as being of type type, false
	 *         otherwise
	 */
	...
	/**
	 * Loops through the biome list and automatically adds tags to any biome that
	 * does not have any This is called by Forge at postinit time. It will
	 * additionally dispatch any deferred decorator creation events.
	 *
	 ...
	 * This method is called automatically if a biome has not been registered with
	 * any tags, And another method requests information about it
	 *
	 * @param biome
	 *            the biome to be considered
	 */
	public static void makeBestGuess(BiomeGenBase biome) {
		...
		if (biome.isHighHumidity() && biome.rootHeight < 0.0F && biome.heightVariation <= 0.3F
				&& biome.heightVariation >= 0.0F) {
