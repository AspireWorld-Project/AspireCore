...
public class ChestGenHooks {
	...
	/**
	 * Generates an array of items based on the input min/max count. If the stack
	 * can not hold the total amount, it will be split into stacks of size 1.
	 *
	 * @param rand
	...
	/**
	 * Removes all items that match the input item stack, Only metadata and item ID
	 * are checked. If the input item has a metadata of -1, all metadatas will
	 * match.
	 *
	 * @param item
	 *            The item to check
	...
	public void removeItem(ItemStack item) {
		...
		while (itr.hasNext()) {
			...
			if (item.isItemEqual(cont.theItemId) || item.getItemDamage() == OreDictionary.WILDCARD_VALUE
					&& item.getItem() == cont.theItemId.getItem()) {
				itr.remove();
			}
		}
	}
	...
	/**
	 * Returns a single ItemStack from the possible items in this registry, Useful
	 * if you just want a quick and dirty random Item.
	 *
	 * @param rand
	 *            A Random Number gen
	...
	public ItemStack getOneItem(Random rand) {
		...
		ItemStack[] stacks = ChestGenHooks.generateStacks(rand, item.theItemId, item.theMinimumChanceToGenerateItem,
