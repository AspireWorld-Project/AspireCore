...
public class DimensionManager {
	 ...
	 * registered to this provider type. If the return size is greater then 0, it is
	 * required that the caller either change those dimensions's registered type, or
	 * replace this type before the world is attempted to load, else the loader will
	 * throw an exception.
	 *
	 * @param id
	 *            The provider type ID to unreigster
	 * @return An array containing all dimension IDs still registered to this
	 *         provider type.
	 */
	...
	public static void registerDimension(int id, int providerType) {
		if (!providers.containsKey(providerType))
			throw new IllegalArgumentException(String.format(
					"Failed to register dimension for id %d, provider type %d does not exist", id, providerType));
	...
	/**
	 * For unregistering a dimension when the save is changed (disconnected from a
	 * server or loaded a new save
	 */
	...
	public static Integer[] getIDs() {
		return worlds.keySet().toArray(new Integer[worlds.size()]); // Only loaded dims, since usually used to cycle
																	// through loaded worlds
	}
	...
	public static void setWorld(int id, WorldServer world) {
		if (world != null) {
			...
			FMLLog.info("Loading dimension %d (%s) (%s)", id, world.getWorldInfo().getWorldName(),
					world.func_73046_m());
		} else {
			worlds.remove(id);
	...
	public static void initDimension(int dim) {
		...
		WorldServer world = dim == 0 ? overworld
				: new WorldServerMulti(mcServer, savehandler, overworld.getWorldInfo().getWorldName(), dim,
						worldSettings, overworld, mcServer.theProfiler);
	...
	/**
	 * Not public API: used internally to get dimensions that should load at server
	 * startup
	 */
	...
	public static WorldProvider createProviderFor(int dim) {
		try {
			...
			FMLCommonHandler.instance().getFMLLogger().log(Level.ERROR,
					String.format("An error occured trying to create an instance of WorldProvider %d (%s)", dim,
							providers.get(getProviderType(dim)).getSimpleName()),
					e);
	...
	/*
	 * To be called by the server at the appropriate time, do not call from mod
	 * code.
	 */
	...
	/**
	 * Return the current root directory for the world save. Accesses getSaveHandler
	 * from the overworld
	 *
	...
	public static File getCurrentSaveRootDirectory() {
		if (DimensionManager.getWorld(0) != null)
			...
			String name = srv.isSinglePlayer() ? srv.getFolderName()
					: srv.getMultiWorld().getNameByID(0) != null ? srv.getMultiWorld().getNameByID(0) : "world";
