 ...
 *
 * The basic principle is a ticket based system. 1. Mods register a callback
 * {@link #setForcedChunkLoadingCallback(Object, LoadingCallback)} 2. Mods ask
 * for a ticket {@link #requestTicket(Object, World, Type)} and then hold on to
 * that ticket. 3. Mods request chunks to stay loaded
 * {@link #forceChunk(Ticket, ChunkCoordIntPair)} or remove chunks from force
 * loading {@link #unforceChunk(Ticket, ChunkCoordIntPair)}. 4. When a world
 * unloads, the tickets associated with that world are saved by the chunk
 * manager. 5. When a world loads, saved tickets are offered to the mods
 * associated with the tickets. The {@link Ticket#getModData()} that is set by
 * the mod should be used to re-register chunks to stay loaded (and maybe take
 * other actions).
 *
 * The chunkloading is configurable at runtime. The file
 * "config/forgeChunkLoading.cfg" contains both default configuration for
 * chunkloading, and a sample individual mod specific override section.
 *
 * @author cpw
 *
 */
public class ForgeChunkManager {
	...
	public interface LoadingCallback {
		 ...
		 * re-register the chunks associated with those tickets. The list supplied here
		 * is truncated to length prior to use. Tickets unwanted by the mod must be
		 * disposed of manually unless the mod is an OrderedLoadingCallback instance in
		 * which case, they will have been disposed of by the earlier callback.
		 *
		 * @param tickets
		 *            The tickets to re-register. The list is immutable and cannot be
	 ...
	 * LoadingCallback to provide access to additional behaviour. Specifically, this
	 * callback will fire prior to Forge dropping excess tickets. Tickets in the
	 * returned list are presumed ordered and excess will be truncated from the
	 * returned list. This allows the mod to control not only if they actually
	 * <em>want</em> a ticket but also their preferred ticket ordering.
	 *
	 * @author cpw
	 *
	 */
	public interface OrderedLoadingCallback extends LoadingCallback {
		/**
		 * Called back when tickets are loaded from the world to allow the mod to decide
		 * if it wants the ticket still, and prioritise overflow based on the ticket
		 * count. WARNING: You cannot force chunks in this callback, it is strictly for
		 * allowing the mod to be more selective in which tickets it wishes to preserve
		 * in an overflow situation
		 *
		 * @param tickets
		 *            The tickets that you will want to select from. The list is
		 *            immutable and cannot be manipulated directly. Copy it first.
		 * @param world
		 *            The world
		 * @param maxTicketCount
		 *            The maximum number of tickets that will be allowed.
		 * @return A list of the tickets this mod wishes to continue using. This list
	...
	public interface PlayerOrderedLoadingCallback extends LoadingCallback {
		/**
		 * Called back when tickets are loaded from the world to allow the mod to decide
		 * if it wants the ticket still. This is for player bound tickets rather than
		 * mod bound tickets. It is here so mods can decide they want to dump all player
		 * tickets
		 *
		 * WARNING: You cannot force chunks in this callback, it is strictly for
		 * allowing the mod to be more selective in which tickets it wishes to preserve
		 *
		 * @param tickets
		 *            The tickets that you will want to select from. The list is
	...
	public static class Ticket {
		...
		Ticket(String modId, Type type, World world) {
			...
			ticketType = type;
			this.world = world;
			maxDepth = getMaxChunkDepthFor(modId);
			requestedChunks = Sets.newLinkedHashSet();
		}
		...
		/**
		 * The chunk list depth can be manipulated up to the maximal grant allowed for
		 * the mod. This value is configurable. Once the maximum is reached, the least
		 * recently forced chunk, by original registration time, is removed from the
		 * forced chunk list.
		 *
		 * @param depth
		...
		public void setChunkListDepth(int depth) {
			if (depth > getMaxChunkDepthFor(modId) || depth <= 0 && getMaxChunkDepthFor(modId) > 0) {
				FMLLog.warning("The mod %s tried to modify the chunk ticket depth to: %d, its allowed maximum is: %d",
						modId, depth, getMaxChunkDepthFor(modId));
			} else {
				maxDepth = depth;
			}
		}
		...
		/**
		 * Gets the current max depth for this ticket. Should be the same as
		 * getMaxChunkListDepth() unless setChunkListDepth has been called.
		 *
		...
		/**
		 * Bind the entity to the ticket for {@link Type#ENTITY} type tickets. Other
		...
		/**
		 * Retrieve the {@link NBTTagCompound} that stores mod specific data for the
		 * chunk ticket. Example data to store would be a TileEntity or Block location.
		 * This is persisted with the ticket and provided to the {@link LoadingCallback}
		 * for the mod. It is recommended to use this to recover useful state
		 * information for the forced chunks.
		 *
		 * @return The custom compound tag for mods to store additional chunkloading
		 *         data
		 */
	...
	/**
	 * Allows dynamically loading world mods to test if there are chunk tickets in
	 * the world Mods that add dynamically generated worlds (like Mystcraft) should
	 * call this method to determine if the world should be loaded during server
	 * starting.
	 *
	 * @param chunkDir
	 *            The chunk directory to test: should be equivalent to
	 *            {@link WorldServer#getChunkSaveLocation()}
	 ...
	 *
	 * @param mod
	 *            The mod instance registering the callback
	 * @param callback
	...
	public static void setForcedChunkLoadingCallback(Object mod, LoadingCallback callback) {
		...
		if (container == null) {
			FMLLog.warning("Unable to register a callback for an unknown mod %s (%s : %x)", mod,
					mod.getClass().getName(), System.identityHashCode(mod));
			return;
		}
	 ...
	 *
	 * @param mod
	 *            The mod that will own the tickets
	 * @param world
	 *            The world
	...
	public static Ticket requestPlayerTicket(Object mod, String player, World world, Type type) {
		...
		if (mc == null) {
			FMLLog.log(Level.ERROR, "Failed to locate the container for mod instance %s (%s : %x)", mod,
					mod.getClass().getName(), System.identityHashCode(mod));
			return null;
		}
		if (playerTickets.get(player).size() > playerTicketLength) {
			FMLLog.warning("Unable to assign further chunkloading tickets to player %s (on behalf of mod %s)", player,
					mc.getModId());
			return null;
		}
	 ...
	 *
	 * @param mod
	 *            The mod requesting a ticket
	 * @param world
	 *            The world in which it is requesting the ticket
	 * @param type
	 *            The type of ticket
	 * @return A ticket with which to register chunks for loading, or null if no
	 *         further tickets are available
	 */
	...
	/**
	 * Release the ticket back to the system. This will also unforce any chunks held
	 * by the ticket so that they can be unloaded and/or stop ticking.
	 *
	 * @param ticket
	...
	/**
	 * Force the supplied chunk coordinate to be loaded by the supplied ticket. If
	 * the ticket's {@link Ticket#maxDepth} is exceeded, the least recently
	 * registered chunk is unforced and may be unloaded. It is safe to force the
	 * chunk several times for a ticket, it will not generate duplication or change
	 * the ordering.
	 *
	 * @param ticket
	 *            The ticket registering the chunk
	 * @param chunk
	 *            The chunk to force
	 */
	public static void forceChunk(Ticket ticket, ChunkCoordIntPair chunk) {
		...
		if (ticket.isPlayerTicket() ? !playerTickets.containsValue(ticket)
				: !tickets.get(ticket.world).containsEntry(ticket.modId, ticket)) {
			FMLLog.severe("The mod %s attempted to force load a chunk with an invalid ticket. This is not permitted.",
					ticket.modId);
			return;
		}
		...
		ImmutableSetMultimap<ChunkCoordIntPair, Ticket> newMap = ImmutableSetMultimap
				.<ChunkCoordIntPair, Ticket>builder().putAll(forcedChunks.get(ticket.world)).put(chunk, ticket).build();
	...
	/**
	 * Reorganize the internal chunk list so that the chunk supplied is at the *end*
	 * of the list This helps if you wish to guarantee a certain "automatic unload
	 * ordering" for the chunks in the ticket list
	 *
	 * @param ticket
	 *            The ticket holding the chunk list
	 * @param chunk
	 *            The chunk you wish to push to the end (so that it would be
	 *            unloaded last)
	 */
	 ...
	 *
	 * @param ticket
	 *            The ticket holding the chunk
	 * @param chunk
	 *            The chunk to unforce
	 */
	...
	public static void syncConfigDefaults() {
		// By adding a property order list we are defining the order that the properties
		// will appear both in the config file and on the GUIs.
		...
		config.addCustomCategoryComment("Forge",
		...
		for (String mod : config.getCategoryNames()) {
			...
			config.get(mod, "maximumTicketCount", 200).setLanguageKey("forge.configgui.maximumTicketCount")
					.setMinValue(0);
			config.get(mod, "maximumChunksPerTicket", 25).setLanguageKey("forge.configgui.maximumChunksPerTicket")
					.setMinValue(0);
		}
