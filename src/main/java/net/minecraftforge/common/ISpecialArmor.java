...
public interface ISpecialArmor {
	 ...
	 *
	 * Armor will higher priority will have damage applied to them before lower
	 * priority ones. If there are multiple pieces of armor with the same priority,
	 * damage will be distributed between them based on there absorption ratio.
	 *
	 * @param player
	 ...
	 * durability and stack size. If the stack is depleted it will be cleaned up
	 * automatically.
	 *
	 * @param entity
	 *            The entity wearing the armor
	...
	class ArmorProperties implements Comparable<ArmorProperties> {
		...
		public int Slot = 0;
		private static final boolean DEBUG = false; // Only enable this if you wish to be spamed with debugging
													// information.
		 ...
		 */
		public static float ApplyArmor(EntityLivingBase entity, ItemStack[] inventory, DamageSource source,
				double damage) {
			if (DEBUG) {
		...
		public String toString() {
			return String.format("%d, %d, %f, %d", Priority, AbsorbMax, AbsorbRatio,
