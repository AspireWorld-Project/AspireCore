 ...
 * right slots of a anvil. If the event is canceled, vanilla behavior will not
 * run, and the output will be set to null. If the event is not canceled, but
 * the output is not null, it will set the output and not run vanilla behavior.
 * if the output is null, and the event is not canceled, vanilla behavior will
 * execute.
 */
@Cancelable
public class AnvilUpdateEvent extends Event {
	...
	public int materialCost; // The number of items from the right slot to be consumed during the repair.
