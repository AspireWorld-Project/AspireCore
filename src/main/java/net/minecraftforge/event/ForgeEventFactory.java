...
public class ForgeEventFactory {
	...
	public static PlaceEvent onPlayerBlockPlace(EntityPlayer player, BlockSnapshot blockSnapshot,
			ForgeDirection direction) {
		Block placedAgainst = blockSnapshot.world.getBlock(blockSnapshot.x + direction.getOpposite().offsetX,
				blockSnapshot.y + direction.getOpposite().offsetY, blockSnapshot.z + direction.getOpposite().offsetZ);
	...
	public static float getBreakSpeed(EntityPlayer player, Block block, int metadata, float original, int x, int y,
			int z) {
	...
	public static List<BiomeGenBase.SpawnListEntry> getPotentialSpawns(WorldServer world, EnumCreatureType type, int x,
			int y, int z, List<BiomeGenBase.SpawnListEntry> oldList) {
	...
	public static int getMaxSpawnPackSize(EntityLiving entity) {
		...
		return maxCanSpawnEvent.getResult() == Result.ALLOW ? maxCanSpawnEvent.maxPackSize
	...
	public static float fireBlockHarvesting(ArrayList<ItemStack> drops, World world, Block block, int x, int y, int z,
			int meta, int fortune, float dropChance, boolean silkTouch, EntityPlayer player) {
		BlockEvent.HarvestDropsEvent event = new BlockEvent.HarvestDropsEvent(x, y, z, world, block, meta, fortune,
				dropChance, drops, player, silkTouch);
		MinecraftForge.EVENT_BUS.post(event);
		return event.dropChance;
	}

	public static ItemTooltipEvent onItemTooltip(ItemStack itemStack, EntityPlayer entityPlayer, List<String> toolTip,
	...
	public static void firePlayerSavingEvent(EntityPlayer player, File playerDirectory, String uuidString) {
		MinecraftForge.EVENT_BUS.postWithProfile(player.worldObj.theProfiler,
				new PlayerEvent.SaveToFile(player, playerDirectory, uuidString));
	}
	...
	public static void firePlayerLoadingEvent(EntityPlayer player, IPlayerFileData playerFileData, String uuidString) {
		...
		File dir = ObfuscationReflectionHelper.getPrivateValue(SaveHandler.class, sh, "playersDirectory",
				"field_" + "75771_c");
	...
	public static void onExplosionDetonate(World world, Explosion explosion, List<Entity> list, double diameter) {
		// Filter entities to only those who are effected, to prevent modders from
		// seeing more then will be hurt.
		/*
		 * Enable this if we get issues with modders looping to much. Iterator<Entity>
		 * itr = list.iterator(); while (itr.hasNext()) { Entity e = itr.next(); double
		 * dist = e.getDistance(explosion.explosionX, explosion.explosionY,
		 * explosion.explosionZ) / diameter; if (dist > 1.0F) itr.remove(); }
		 */
