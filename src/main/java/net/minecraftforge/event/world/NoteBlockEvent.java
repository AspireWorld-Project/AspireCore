...
public class NoteBlockEvent extends BlockEvent {
	...
	/**
	 * get the vanilla note-id, which contains information about both Note and
	 * Octave. Most modders should not need this.
	 *
	...
	public void setNote(Note note, Octave octave) {
		Preconditions.checkArgument(octave != Octave.HIGH || note == Note.F_SHARP,
				"Octave.HIGH is only valid for Note.F_SHARP!");
		noteId = note.ordinal() + octave.ordinal() * 12;
	}

	/**
	 * Fired when a Noteblock plays it's note. You can override the note and
	 * instrument Canceling this event will stop the note from playing.
	 */
	@Cancelable
	...
	/**
	 * Fired when a Noteblock is changed. You can adjust the note it will change to
	 * via {@link #setNote(Note, Octave)}. Canceling this event will not change the
	 * note and also stop the Noteblock from playing it's note.
	 */
	@Cancelable
	public static class Change extends NoteBlockEvent {
		...
		public Change(World world, int x, int y, int z, int meta, int oldNote, int newNote) {
			...
			oldOctave = Octave.fromId(oldNote);
		}
	}
	...
	/**
	 * Information about the pitch of a Noteblock note. For altered notes such as
	...
	/**
	 * Describes the Octave of a Note being played by a Noteblock. Together with
