 ...
 *
 * It is highly recommended that you extend this class or one of the
 * Forge-provided child classes.
 *
...
public abstract class BlockFluidBase extends Block implements IFluidBlock {
	...
	/**
	 * This is the fluid used in the constructor. Use this reference to configure
	 * things like icons for your block. It might not be active in the registry, so
	 * do NOT expose it.
	 */
	...
	public BlockFluidBase(Fluid fluid, Material material) {
		...
		maxScaledLight = fluid.luminosity;
		tickRate = fluid.viscosity / 200;
		densityDir = fluid.density > 0 ? -1 : 1;
		fluid.setBlock(this);
	...
	/**
	 * Returns true if the block at (x, y, z) is displaceable. Does not displace the
	 * block.
	 */
	 ...
	 * Never used...?
	 *
	 * @Override public float getBlockBrightness(World world, int x, int y, int z) {
	 * float lightThis = world.getLightBrightness(x, y, z); float lightUp =
	 * world.getLightBrightness(x, y + 1, z); return lightThis > lightUp ? lightThis
	 * : lightUp; }
	 */
	...
	public Vec3 getFlowVector(IBlockAccess world, int x, int y, int z) {
		...
		if (world.getBlock(x, y + 1, z) == this) {
			boolean flag = isBlockSolid(world, x, y, z - 1, 2) || isBlockSolid(world, x, y, z + 1, 3)
					|| isBlockSolid(world, x - 1, y, z, 4) || isBlockSolid(world, x + 1, y, z, 5)
					|| isBlockSolid(world, x, y + 1, z - 1, 2) || isBlockSolid(world, x, y + 1, z + 1, 3)
					|| isBlockSolid(world, x - 1, y + 1, z, 4) || isBlockSolid(world, x + 1, y + 1, z, 5);
