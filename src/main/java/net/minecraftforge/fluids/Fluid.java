 ...
 *
 * This class is a fluid (liquid or gas) equivalent to "Item." It describes the
 * nature of a fluid and contains its general properties.
 *
 ...
 *
 * Fluid implementations are not required to actively use these properties, nor
 * are objects interfacing with fluids required to make use of them, but it is
 * encouraged.
 *
 * The default values can be used as a reference point for mods adding fluids
 * such as oil or heavy water.
 *
 * @author King Lemming
 *
 */
public class Fluid {
	...
	/**
	 * Density of the fluid - completely arbitrary; negative density indicates that
	 * the fluid is lighter than air.
	 *
	 ...
	 * that the fluid is hotter than air.
	 *
	 * Default value is approximately the real-life room temperature of water in
	 * degrees Kelvin.
	 */
	 ...
	 * (x10^-3).
	 *
	 * Higher viscosity means that a fluid flows more slowly, like molasses. Lower
	 * viscosity means that a fluid flows more quickly, like helium.
	 *
	 */
	 ...
	 *
	 * The default value of null should remain for any Fluid without a Block
	 * implementation.
	 */
	...
	public Fluid setBlock(Block block) {
		if (this.block == null || this.block == block) {
			...
			FMLLog.warning("A mod has attempted to assign Block " + block + " to the Fluid '" + fluidName
					+ "' but this Fluid has already been linked to the Block " + this.block
	...
	public final Fluid setIcons(IIcon stillIcon, IIcon flowingIcon) {
		return setStillIcon(stillIcon).setFlowingIcon(flowingIcon);
	}

	public final Fluid setIcons(IIcon commonIcon) {
		return setStillIcon(commonIcon).setFlowingIcon(commonIcon);
	}
