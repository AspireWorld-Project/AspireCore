 ...
 * and things that have ID/metadata mappings.
 *
...
public abstract class FluidContainerRegistry {
	...
	// the local maps are not guaranteed to have the same internal generic
	// structure,
	...
	private static class ContainerKey {
		...
		public boolean equals(Object o) {
			...
			if (container.getItem() != ck.container.getItem())
				return false;
			if (container.getItemDamage() != ck.container.getItemDamage())
				return false;
	 ...
	 *
	 * @param stack
	 *            FluidStack containing the type and amount of the fluid stored in
	 *            the item.
	 ...
	 * @return True if container was successfully registered; false if it already
	 *         is.
	 */
	...
	/**
	 * Register a new fluid containing item. The item is assumed to hold 1000 mB of
	 * fluid. Also registers the Fluid if possible.
	 *
	 * @param fluid
	 ...
	 *            ItemStack representing the container when it is empty.
	 * @return True if container was successfully registered; false if it already
	 *         is, or an invalid parameter was passed.
	 */
	...
	/**
	 * Register a new fluid containing item that does not have an empty container.
	 * The item is assumed to hold 1000 mB of fluid. Also registers the Fluid if
	 * possible.
	 *
	 * @param fluid
	 *            Fluid type that is stored in the item.
	...
	public static boolean registerFluidContainer(FluidContainerData data) {
		...
		if (data.fluid == null || data.fluid.getFluid() == null) {
			FMLLog.bigWarning(
					"Invalid registration attempt for a fluid container item %s has occurred. The registration has been denied to prevent crashes. The mod responsible for the registration needs to correct this.",
					data.filledContainer.getItem().getUnlocalizedName(data.filledContainer));
			return false;
		}
	 ...
	 * @param container
	 *            The full container.
	 * @return The containers capacity, or 0 if the ItemStack does not represent a
	 ...
	 * @return The containers capacity, or 0 if the ItemStack does not represent a
	 *         registered container or the FluidStack is not registered with the
	 *         empty container.
	 */
	...
	/**
	 * Wrapper class for the registry entries. Ensures that none of the attempted
	 * registrations contain null references unless permitted.
	 */
