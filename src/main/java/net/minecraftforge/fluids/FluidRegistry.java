...
public abstract class FluidRegistry {
	...
	// the globally unique fluid map - only used to associate non-defaults during
	// world/server loading
	...
	}.setBlock(Blocks.lava).setLuminosity(15).setDensity(3000).setViscosity(6000).setTemperature(1300)
	...
	/**
	 * Called by Forge to prepare the ID map for server -> client sync. Modders, DO
	 * NOT call this.
	 */
	...
	/**
	 * Called by forge to load default fluid IDs from the world or from server ->
	 * client for syncing DO NOT call this and expect useful behaviour.
	 *
	 * @param newfluidIDs
	 */
	private static void loadFluidDefaults(BiMap<Fluid, Integer> localFluidIDs, Set<String> defaultNames) {
		...
		for (String defaultName : defaultNames) {
			...
			FMLLog.getLogger().log(Level.DEBUG, "The fluid {} has been selected as the default fluid for {}",
					defaultName, fluid.getName());
	...
	/**
	 * Register a new Fluid. If a fluid with the same name already exists,
	 * registration the alternative fluid is tracked in case it is the default in
	 * another place
	 *
	 * @param fluid
	 *            The fluid to register.
	 * @return True if the fluid was registered as the current default fluid, false
	 *         if it was only registered as an alternative
	 */
	...
	/**
	 * Does the supplied fluid have an entry for it's name (whether or not the fluid
	 * itself is default)
	 *
	 * @param fluid
	 *            the fluid we're testing
	...
	/**
	 * Returns a read-only map containing Fluid IDs and their associated Fluids. In
	 * 1.8.3, this will change to just 'getRegisteredFluidIDs'
	 */
	...
	public static String getDefaultFluidName(Fluid key) {
		...
		if (Strings.isNullOrEmpty(name)) {
			FMLLog.getLogger().log(Level.ERROR,
					...
					key.getClass().getName(), key.getName());
	...
	public static void validateFluidRegistry() {
		...
		if (!illegalFluids.isEmpty()) {
			FMLLog.getLogger().log(Level.FATAL,
					"The fluid registry is corrupted. Something has inserted a fluid without registering it");
