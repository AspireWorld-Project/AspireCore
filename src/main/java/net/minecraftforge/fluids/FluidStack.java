 ...
 * {@link #isFluidStackIdentical(FluidStack)} to determine if FluidID, Amount
 * and NBT Tag are all equal.
 *
...
public class FluidStack {
	/**
	 * This field will be removed in 1.8. It may be incorrect after a world is
	 * loaded. Code should always use {@link #getFluid()} instead. That will always
	...
	public FluidStack(Fluid fluid, int amount) {
		if (fluid == null) {
			...
			FMLLog.bigWarning("Failed attempt to create a FluidStack for an unregistered Fluid %s (type %s)",
					fluid.getName(), fluid.getClass().getName());
			throw new IllegalArgumentException("Cannot create a fluidstack from an unregistered fluid");
		}
		fluidDelegate = FluidRegistry.makeDelegate(fluid);
		this.amount = amount;
		this.fluid = fluid;
	}
	...
	/**
	 * Determines if the FluidIDs and NBT Tags are equal. This does not check
	 * amounts.
	 *
	 * @param other
	...
	/**
	 * Determines if the NBT Tags are equal. Useful if the FluidIDs are known to be
	 * equal.
	 */
	 ...
	 *
	 * @param other
	 * @return true if this FluidStack contains the other FluidStack (same fluid and
	 *         >= amount)
	 */
	...
	/**
	 * Determines if the FluidIDs and NBT Tags are equal compared to a registered
	 * container ItemStack. This does not check amounts.
	 *
	 * @param other
	...
	/**
	 * Default equality comparison for a FluidStack. Same functionality as
	 * isFluidEqual().
	 *
