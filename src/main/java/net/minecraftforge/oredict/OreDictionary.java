...
public class OreDictionary {
	...
	/**
	 * Minecraft changed from -1 to Short.MAX_VALUE in 1.5 release for the "block
	 * wildcard". Use this in case it changes again.
	 */
	...
	public static void initVanillaEntries() {
		...
		ItemStack[] exclusions = new ItemStack[] { new ItemStack(Blocks.lapis_block), new ItemStack(Items.cookie),
				...
				new ItemStack(Blocks.stone_stairs), new ItemStack(Blocks.cobblestone_wall),
				new ItemStack(Blocks.oak_stairs), new ItemStack(Blocks.spruce_stairs),
				new ItemStack(Blocks.birch_stairs), new ItemStack(Blocks.jungle_stairs),
				new ItemStack(Blocks.acacia_stairs), new ItemStack(Blocks.dark_oak_stairs),
	...
	/**
	 * Gets the integer ID for the specified ore name. If the name does not have a
	 * ID it assigns it a new one.
	 *
	 * @param name
	...
	/**
	 * Gets the integer ID for the specified item stack. If the item stack is not
	 * linked to any ore, this will return -1 and no new entry will be created.
	 *
	 * @param stack
	...
	public static int getOreID(ItemStack stack) {
		...
		// HACK: use the registry name's ID. It is unique and it knows about
		// substitutions. Fallback to a -1 value (what Item.getIDForItem would have
		// returned) in the case where the registry is not aware of the item yet
		// IT should be noted that -1 will fail the gate further down, if an entry
		// already exists with value -1 for this name. This is what is broken and being
		// warned about.
		// APPARENTLY it's quite common to do this. OreDictionary should be considered
		// alongside Recipes - you can't make them properly until you've registered with
		// the game.
		String registryName = stack.getItem().delegate.name();
		int id;
		if (registryName == null) {
			FMLLog.log(Level.DEBUG,
					"Attempted to find the oreIDs for an unregistered object (%s). This won't work very well.", stack);
			return -1;
		} else {
		...
		if (ids == null || ids.size() == 0) {
			ids = stackToId.get(id | stack.getItemDamage() + 1 << 16); // Mow the Meta specific one, +1 so that meta
																		// 0 is significant
		}
	...
	/**
	 * Gets all the integer ID for the ores that the specified item stakc is
	 * registered to. If the item stack is not linked to any ore, this will return
	 * an empty array and no new entry will be created.
	 *
	 * @param stack
	 *            The item stack of the ore.
	 ...
	 *
	 * The returned List is unmodifiable, but will be updated if a new ore is
	 * registered using registerOre
	 *
	 * @param name
	...
	/**
	 * Retrieves the List of items that are registered to this ore type at this
	 * instant. If the flag is TRUE, then it will create the list as empty if it did
	 * not exist.
	 *
	 * This option should be used by modders who are doing blanket scans in
	 * postInit. It greatly reduces clutter in the OreDictionary is the responsible
	 * and proper way to use the dictionary in a large number of cases.
	 *
	 * The other function above is utilized in OreRecipe and is required for the
	 * operation of that code.
	 *
	 * @param name
	 *            The ore name, directly calls getOreID if the flag is TRUE
	 * @param alwaysCreateEntry
	 *            Flag - should a new entry be created if empty
	 * @return An arraylist containing ItemStacks registered for this ore
	 */
	...
	/**
	 * Returns whether or not an oreName exists in the dictionary. This function can
	 * be used to safely query the Ore Dictionary without adding needless clutter to
	 ...
	 * Creates the list as empty if it did not exist.
	 *
	 * Warning: In 1.8, the return value will become a immutible list, and this
	 * function WILL NOT create the entry if the ID doesn't exist, IDs are intended
	 * to be internal OreDictionary things and modders should not ever code them in.
	 *
	 * @param id
	...
	{
		return getOres(id.intValue());
	}

	private static ArrayList<ItemStack> getOres(int id) // TODO: change to ImmutibleList<ItemStack> in 1.8, also make
														// private
	{
	...
	// Convenience functions that make for cleaner code mod side. They all drill
	// down to registerOre(String, int, ItemStack)
	...
	/**
	 * Registers a ore item into the dictionary. Raises the registerOre function in
		...
		if (registryName == null) {
			FMLLog.bigWarning(
					"A broken ore dictionary registration with name %s has occurred. It adds an item (type: %s) which is currently unknown to the game registry. This dictionary item can only support a single value when"
							...
							+ Loader.instance().activeModContainer().getName() + " report it to them!",
					name, ore.getItem().getClass());
			hash = -1;
		} else {
			...
			for (ItemStack ore : ores) {
				...
				if (name == null) {
					FMLLog.log(Level.DEBUG,
							...
							getOreName(id), ore.getItem().getClass());
					hash = -1;
				} else {
				...
				ids.add(id);
				// System.out.println(id + " " + getOreName(id) + " " +
				// Integer.toHexString(hash) + " " + ore);
			}
		}
	}

	// Pulled from Collections.UnmodifiableList, as we need to explicitly subclass
	// ArrayList for backward compatibility.
