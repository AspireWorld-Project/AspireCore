...
public class RecipeSorter implements Comparator<IRecipe> {
	...
	private RecipeSorter() {
		...
		register("minecraft:mapextending", RecipesMapExtending.class, SHAPED,
	...
	public int compare(IRecipe r1, IRecipe r2) {
		...
		if (c1 == SHAPELESS && c2 == SHAPED)
			return 1;
		if (c1 == SHAPED && c2 == SHAPELESS)
			return -1;
	...
	private static int getPriority(IRecipe recipe) {
		...
		if (ret == null) {
			if (!warned.contains(cls)) {
				FMLLog.info("  Unknown recipe class! %s Modder please refer to %s", cls.getName(),
