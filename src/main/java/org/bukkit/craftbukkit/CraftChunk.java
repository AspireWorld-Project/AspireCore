...
public class CraftChunk implements Chunk {
	...
	public BlockState[] getTileEntities() {
		...
		for (Object obj : chunk.chunkTileEntityMap.keySet().toArray()) {
			...
			entities[index++] = worldServer.getWorld().getBlockAt(position.chunkPosX + (chunk.xPosition << 4),
					position.chunkPosY, position.chunkPosZ + (chunk.zPosition << 4)).getState();
		}
		return entities;
	}
	...
	public ChunkSnapshot getChunkSnapshot(boolean includeMaxBlockY, boolean includeBiome,
		...
		for (int i = 0; i < cs.length; i++) {
			if (cs[i] == null) { /* Section is empty? */
				...
				if (true /* cs[i].getBlockMSBArray() != null */) { /* If we've got extended IDs */
	...
	private static float[] getTemperatures(net.minecraft.world.biome.WorldChunkManager chunkmanager, int chunkX,
			int chunkZ) {
		net.minecraft.world.biome.BiomeGenBase[] biomes = chunkmanager.getBiomesForGeneration(null, chunkX, chunkZ, 16,
				16);
