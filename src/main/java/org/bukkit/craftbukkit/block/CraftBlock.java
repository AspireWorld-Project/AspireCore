...
public class CraftBlock implements Block {
	...
	@Override
	public byte getData() {
		return (byte) chunk.getHandle().getBlockMetadata(x & 0xF, y & 0xFF, z & 0xF);
	}
	...
	public boolean setTypeIdAndData(final int type, final byte data, final boolean applyPhysics) {
		if (applyPhysics)
			return chunk.getHandle().worldObj.setBlockSilently(x, y, z, getNMSBlock(type), data, 3);
		else {
			boolean success = chunk.getHandle().worldObj.setBlockSilently(x, y, z, getNMSBlock(type), data, 2);
			if (success) {
	...
	public int getTypeId() {
		return CraftMagicNumbers.getId(chunk.getHandle().getBlock(x & 0xF, y & 0xFF, z & 0xF));
	}
	...
	public byte getLightFromSky() {
		return (byte) chunk.getHandle().getSavedLightValue(EnumSkyBlock.Sky, x & 0xF, y & 0xFF, z & 0xF);
	}

	@Override
	public byte getLightFromBlocks() {
		return (byte) chunk.getHandle().getSavedLightValue(EnumSkyBlock.Block, x & 0xF, y & 0xFF, z & 0xF);
	}
	...
	public BlockFace getFace(final Block block) {
		...
		for (BlockFace face : values) {
			if (getX() + face.getModX() == block.getX() && getY() + face.getModY() == block.getY()
	...
	/**
	 * Notch uses a 0-5 to mean DOWN, UP, NORTH, SOUTH, WEST, EAST in that order all
	 * over. This method is convenience to convert for us.
	 *
	...
	public BlockState getState() {
		...
		if (material == null) {
			TileEntity te = ((CraftWorld) getWorld()).getHandle().getTileEntity(getX(), getY(), getZ());
			if (te != null && te instanceof IInventory)
				// In order to allow plugins to properly grab the container location, we must
				// pass a class that extends CraftBlockState and implements InventoryHolder.
	...
	public int getBlockPower(BlockFace face) {
		...
		if ((face == BlockFace.DOWN || face == BlockFace.SELF) && world.getIndirectPowerOutput(x, y - 1, z, 0)) {
			power = func_150178_a(world, x, y - 1, z, power);
		}
		...
		if ((face == BlockFace.EAST || face == BlockFace.SELF) && world.getIndirectPowerOutput(x + 1, y, z, 2)) {
			power = func_150178_a(world, x + 1, y, z, power);
		}
		...
		if ((face == BlockFace.NORTH || face == BlockFace.SELF) && world.getIndirectPowerOutput(x, y, z - 1, 4)) {
			power = func_150178_a(world, x, y, z - 1, power);
		}
	...
	private static int func_150178_a(net.minecraft.world.World p_150178_1_, int p_150178_2_, int p_150178_3_,
			int p_150178_4_, int p_150178_5_) {
		if (p_150178_1_.getBlock(p_150178_2_, p_150178_3_, p_150178_4_) != Blocks.redstone_wire)
			return p_150178_5_;
		else {
	...
	public boolean isEmpty() {
		// Cauldron start - support custom air blocks (Railcraft player aura tracking
		// block)
	...
	private boolean itemCausesDrops(ItemStack item) {
		...
		return block != null
				&& (block.getMaterial().isToolNotRequired() || itemType != null && itemType.func_150897_b(block));
	}
			...
			for (int i = 0; i < count; ++i) {
				...
				if (item != null) {
					...
					if (Blocks.skull == block) {
						net.minecraft.item.ItemStack nmsStack = new net.minecraft.item.ItemStack(item, 1,
								block.getDamageValue(chunk.getHandle().worldObj, x, y, z));
						TileEntitySkull tileentityskull = (TileEntitySkull) chunk.getHandle().worldObj.getTileEntity(x,
								y, z);
						...
						drops.add(new ItemStack(org.bukkit.craftbukkit.util.CraftMagicNumbers.getMaterial(item), 1,
								(short) block.damageDropped(data)));
					}
				}
			}
		}
	...
	public static void initMappings() { // Cauldron - initializer to initMappings() method; called in CraftServer
										// BIOME_MAPPING = new Biome[BiomeGenBase.biomeList.length]; // Cauldron - move
										// up
										// BIOMEBASE_MAPPING = new BiomeGenBase[Biome.values().length]; // Cauldron -
										// move up
		...
		// add support for mod biomes
		for (int i = 0; i < BIOME_MAPPING.length; i++) {
			if (BiomeGenBase.getBiome(i) != null && BIOME_MAPPING[i] == null) {
				String name = BiomeGenBase.getBiome(i).biomeName;
				BiomeGenBase.getBiome(i);

				FMLLog.fine("Adding biome mapping " + BiomeGenBase.getBiome(i).biomeID + " " + name + " at BiomeBase["
						+ i + "]");
	...
	// Cauldron start - if cauldron.dump-materials is true, dump all materials with
	...
	public static void dumpMaterials() {
		...
		// FMLLog.info("To disable these dumps, set cauldron.dump-materials to false in
		// bukkit.yml.");
		// }
	}
	// Cauldron end
