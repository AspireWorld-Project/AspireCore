...
public class CraftSimpleCommandMap extends SimpleCommandMap {
	...
	public boolean dispatch(CommandSender sender, String commandLine) throws CommandException {
		...
		if (target == null)
			return false;
		try {
			// Cauldron start - if command is a mod command, check permissions and route
			// through vanilla
			if (target instanceof ModCustomCommand) {
				...
				if (sender instanceof ConsoleCommandSender) {
					FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager()
							.executeCommand(vanillaConsoleSender, commandLine);
				} else {
					FMLCommonHandler.instance().getMinecraftServerInstance().getCommandManager()
							.executeCommand(((CraftPlayer) sender).getHandle(), commandLine);
				}
			} else {
				// Cauldron end
