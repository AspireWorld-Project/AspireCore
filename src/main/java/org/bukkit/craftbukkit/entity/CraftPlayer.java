...
public class CraftPlayer extends CraftHumanEntity implements Player {
	...
	public CraftPlayer(CraftServer server, net.minecraft.entity.player.EntityPlayerMP entity) {
		...
		entity.setBukkitDisplayName(entity.getCommandSenderName());
	}
	...
	public void sendRawMessage(String message) {
		...
		for (net.minecraft.util.IChatComponent component : CraftChatMessage.fromString(message)) {
			getHandle().playerNetServerHandler
					.sendPacket(new net.minecraft.network.play.server.S02PacketChat(component));
		}
	}
	...
	public String getOverridenDisplayName() {
		return getHandle().getBukkitDisplayName();
	}
	...
	public void kickPlayer(String message) {
		if (Thread.currentThread() != net.minecraft.server.MinecraftServer.getServer().getServerThread())
	...
	public void playNote(Location loc, byte instrument, byte note) {
		...
		getHandle().playerNetServerHandler.sendPacket(new net.minecraft.network.play.server.S29PacketSoundEffect(
				"note." + instrumentName, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), 3.0f, note));
	}
	...
	public void playNote(Location loc, Instrument instrument, Note note) {
		...
		getHandle().playerNetServerHandler.sendPacket(new net.minecraft.network.play.server.S29PacketSoundEffect(
				"note." + instrumentName, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), 3.0f, note.getId()));
	}
	...
	public void playSound(Location loc, String sound, float volume, float pitch) {
		if (loc == null || sound == null || getHandle().playerNetServerHandler == null)
			return;
		...
		net.minecraft.network.play.server.S29PacketSoundEffect packet = new net.minecraft.network.play.server.S29PacketSoundEffect(
	...
	public void playEffect(Location loc, Effect effect, int data) {
		...
		net.minecraft.network.play.server.S28PacketEffect packet = new net.minecraft.network.play.server.S28PacketEffect(
	...
	public void sendBlockChange(Location loc, int material, byte data) {
		...
		S23PacketBlockChange packet = new S23PacketBlockChange(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(),
	...
	public void sendSignChange(Location loc, String[] lines) {
		...
		getHandle().playerNetServerHandler
				.sendPacket(new S33PacketUpdateSign(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), astring));
	}
	...
	public void sendMap(MapView map) {
		...
		for (int x = 0; x < 128; ++x) {
			...
			net.minecraft.network.play.server.S34PacketMaps packet = new net.minecraft.network.play.server.S34PacketMaps(
					map.getId(), bytes);
			getHandle().playerNetServerHandler.sendPacket(packet);
		}
	}
	...
	public boolean isDisconnected() {
		...
		return entity.playerNetServerHandler == null
				|| !entity.playerNetServerHandler.netManager.channel().config().isAutoRead();
	}
	...
	public boolean teleport(Location location, PlayerTeleportEvent.TeleportCause cause) {
		...
		if (fromWorld == toWorld) {
			...
			getHandle().transferToDimension(toWorld.provider.dimensionId);
			getHandle().setPositionAndUpdate(location.getX(), location.getY(), location.getZ());
		}
		return true;
	}
	...
	public void loadData() {
		getHandle().readFromNBT(
				server.getHandle().getDataLoader().getDataProvider().loadPlayer(getHandle().getGameProfile()));
	}
	...
	@Override
	public void setSleepingIgnored(boolean isSleeping) { // TODO
		// getHandle().fauxSleeping = isSleeping;
		// ((CraftWorld) getWorld()).getHandle().checkSleepStatus();
	}

	@Override
	public boolean isSleepingIgnored() {// TODO
	...
	@Override
	public void setPlayerTime(long time, boolean relative) {// TODO
		// getHandle().timeOffset = time;
		// getHandle().relativeTime = relative;
	}

	@Override
	public long getPlayerTimeOffset() {// TODO
		return 0;// return getHandle().timeOffset;
	}

	@Override
	public long getPlayerTime() {// TODO
		return getHandle().worldObj.getWorldTime();// return getHandle().getPlayerTime();
	}

	@Override
	public boolean isPlayerTimeRelative() {// TODO
	...
	@Override
	public WeatherType getPlayerWeather() {// TODO
		return getHandle().worldObj.getWorldInfo().isRaining() ? WeatherType.DOWNFALL : WeatherType.CLEAR;// return
																											// getHandle().getPlayerWeather();
	}

	@Override
	public void resetPlayerWeather() {// TODO
	...
	public void setGameMode(GameMode mode) {
		...
		if (mode != getGameMode()) {
			...
			getHandle().theItemInWorldManager
					.setGameType(net.minecraft.world.WorldSettings.GameType.getByID(mode.getValue()));
			getHandle().fallDistance = 0;
			getHandle().playerNetServerHandler
					.sendPacket(new net.minecraft.network.play.server.S2BPacketChangeGameState(3, mode.getValue()));
		}
	}
	...
	public float getExhaustion() {
		return getHandle().getFoodStats().getFoodExhaustionLevel();
	}

	@Override
	public void setExhaustion(float value) {
		getHandle().getFoodStats().setFoodExhaustionLevel(value);
	}

	@Override
	public float getSaturation() {
		return getHandle().getFoodStats().getFoodSaturationLevel();
	}
	...
	public Location getBedSpawnLocation() {
		...
		if (world != null && bed != null) {
			bed = net.minecraft.entity.player.EntityPlayer.verifyRespawnCoordinates(((CraftWorld) world).getHandle(),
					bed, getHandle().isSpawnForced(spawnDim));
			if (bed != null)
	...
	@Override
	public void setBedSpawnLocation(Location location, boolean override) { // TODO
		// if (location == null) {
		// getHandle().setSpawnChunk(null, override);
		// } else {
		// getHandle().setSpawnChunk(new
		// net.minecraft.util.ChunkCoordinates(location.getBlockX(),
	...
	@Override
	public void hidePlayer(Player player) {// TODO
		...
		// ((net.minecraft.world.WorldServer) entity.worldObj).theEntityTracker;
		// net.minecraft.entity.player.EntityPlayerMP other = ((CraftPlayer)
		// player).getHandle();
		// net.minecraft.entity.EntityTrackerEntry entry =
		// (net.minecraft.entity.EntityTrackerEntry)
		// tracker.trackedEntityIDs.lookup(other.getEntityId());
		// if (entry != null) {
		// entry.removePlayerFromTracker(getHandle());
		// }
		//
		// //remove the hidden player from this player user list
		// getHandle().playerNetServerHandler.sendPacket(new
		// net.minecraft.network.play.server.S38PacketPlayerListItem(player.getPlayerListName(),
		// false, 9999));
	}

	@Override
	public void showPlayer(Player player) {// TODO
		...
		//
		// getHandle().playerNetServerHandler.sendPacket(new
		// S38PacketPlayerListItem(player.getPlayerListName(), false, 9999));
	}
	...
	public int hashCode() {
		if (hash == 0 || hash == 485) {
			hash = 97 * 5 + (getName() != null ? getName().toLowerCase().hashCode() : 0);
		}
		return hash;
	}
	...
	public void sendSupportedChannels() {
		...
		if (!listening.isEmpty()) {
			...
			getHandle().playerNetServerHandler.sendPacket(
					new net.minecraft.network.play.server.S3FPacketCustomPayload("REGISTER", stream.toByteArray()));
		}
	}
	...
	@Override
	public boolean setWindowProperty(Property prop, int value) { // TODO
	...
	public int getNoDamageTicks() {
		if (getHandle().getField_147101_bU() > 0)
			return Math.max(getHandle().getField_147101_bU(), getHandle().hurtResistantTime);
		else
	...
	public void setWalkSpeed(float value) {
		validateSpeed(value);
		net.minecraft.entity.player.EntityPlayerMP player = getHandle();
		player.capabilities.setWalkSpeed(Math.max(value, 0.0001f) / 2f); // Spigot
		player.sendPlayerAbilities();
	}
	...
	public void updateScaledHealth() {
		net.minecraft.entity.ai.attributes.ServersideAttributeMap attributemapserver = (net.minecraft.entity.ai.attributes.ServersideAttributeMap) getHandle()
				.getAttributeMap();
		...
		getHandle().playerNetServerHandler
				.sendPacket(new net.minecraft.network.play.server.S06PacketUpdateHealth(getScaledHealth(),
						getHandle().getFoodStats().getFoodLevel(), getHandle().getFoodStats().getSaturationLevel()));
		getHandle().playerNetServerHandler.sendPacket(
				new net.minecraft.network.play.server.S20PacketEntityProperties(getHandle().getEntityId(), set));

		set.clear();
		getHandle().setMaxHealthCache(getMaxHealth());
	}
	...
	public void injectScaledMaxHealth(Collection collection, boolean force) {
		...
		for (Object genericInstance : collection) {
			net.minecraft.entity.ai.attributes.IAttribute attribute = ((net.minecraft.entity.ai.attributes.IAttributeInstance) genericInstance)
					.getAttribute();
		...
		collection.add(new net.minecraft.entity.ai.attributes.ModifiableAttributeInstance(getHandle().getAttributeMap(),
				new net.minecraft.entity.ai.attributes.RangedAttribute("generic.maxHealth",
						scaledHealth ? healthScale : getMaxHealth(), 0.0D, Float.MAX_VALUE).setDescription("Max Health")
								.setShouldWatch(true)));
	}
	...
	private final Player.Spigot spigot = new Player.Spigot() {
		...
		public boolean getCollidesWithEntities() {
			return getHandle().isCollidesWithEntities();
		}

		@Override
		public void setCollidesWithEntities(boolean collides) {
			getHandle().setCollidesWithEntities(collides);
		...
		public String getLocale() {
			return getHandle().getTranslator();
		}
	};
