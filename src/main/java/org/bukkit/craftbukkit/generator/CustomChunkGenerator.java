...
public class CustomChunkGenerator extends InternalChunkGenerator {
	...
	public net.minecraft.world.chunk.Chunk provideChunk(int x, int z) {
		...
		if (xbtypes != null) {
			chunk = new net.minecraft.world.chunk.Chunk(world, x, z);
			...
			for (int sec = 0; sec < scnt; sec++) {
				...
				// Build chunk section
				csect[sec] = newExtendedBlockStorage(sec << 4, true, secBlkID, secExtBlkID);
			}
			...
			if (btypes != null) {
				...
				for (int sec = 0; sec < scnt; sec++) {
					...
					csect[sec] = newExtendedBlockStorage(sec << 4, true, btypes[sec], null);
				}
				...
				chunk = new net.minecraft.world.chunk.Chunk(world, x, z); // Create empty chunk
							...
							for (int cz = 0; cz < 16; cz++) {
								...
								if (blk != 0) { // If non-empty
									if (cs == null) { // If no section yet, get one
										cs = csect[sec] = new net.minecraft.world.chunk.storage.ExtendedBlockStorage(
												sec << 4, true);
	...
	private static net.minecraft.world.chunk.storage.ExtendedBlockStorage newExtendedBlockStorage(int y, boolean flag,
			byte[] blkIds, byte[] extBlkIds) {
		net.minecraft.world.chunk.storage.ExtendedBlockStorage ebs = new net.minecraft.world.chunk.storage.ExtendedBlockStorage(
				y, flag);
		ebs.getSlot().setLSB(blkIds);
		if (extBlkIds != null) {
			ebs.getSlot().setMSB(extBlkIds);
		}
		return ebs;
	}
	...
	@Override
	public net.minecraft.world.ChunkPosition func_147416_a(net.minecraft.world.World world, String type, int x, int y,
			int z) {
		return "Stronghold".equals(type) && strongholdGen != null ? strongholdGen.func_151545_a(world, x, y, z) : null;
	}
