...
public class SimpleHelpMap implements HelpMap {
	...
	public SimpleHelpMap(CraftServer server) {
		helpTopics = new TreeMap<>(HelpTopicComparator.topicNameComparatorInstance()); // Using a
																						// TreeMap
																						// for
																						// its
																						// explicit
		...
		defaultTopic = new IndexHelpTopic("Index", null, null, Collections2.filter(helpTopics.values(), indexFilter),
			...
			if (pluginName != null) {
				...
				if (topic != null) {
					if (!pluginIndexes.containsKey(pluginName)) {
						pluginIndexes.put(pluginName, new TreeSet<>(HelpTopicComparator.helpTopicComparatorInstance())); // keep
																															// things
																															// in
																															// topic
																															// order
