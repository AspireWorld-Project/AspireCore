...
public final class CraftItemStack extends ItemStack {
	...
	public static ItemStack asBukkitCopy(net.minecraft.item.ItemStack original) {
		...
		// Cauldron start - return non-strictly-Bukkit stack in order to preserve full
		// NBT tags
		return asCraftMirror(copyNMSStack(original, original.stackSize));

		/*
		 * ItemStack stack = new ItemStack(original.itemID, original.stackSize, (short)
		 * original.getItemDamage()); if (hasItemMeta(original)) {
		 * stack.setItemMeta(getItemMeta(original)); // Cauldron - TODO: wrap arbitrary
		 * mod NBT in ItemMeta } return stack;
		 */
		// Cauldron end
	}
	...
	static Map<Enchantment, Integer> getEnchantments(net.minecraft.item.ItemStack item) {
		net.minecraft.nbt.NBTTagList list = item != null && item.isItemEnchanted() ? item.getEnchantmentTagList()
				: null;
	...
	public boolean isSimilar(ItemStack stack) {
		...
		if (!(that.getTypeId() == getTypeId() && getDurability() == that.getDurability()))
			return false;
		return hasItemMeta() ? that.hasItemMeta() && handle.stackTagCompound.equals(that.handle.stackTagCompound)
				: !that.hasItemMeta();
	}
