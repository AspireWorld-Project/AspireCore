...
class CraftMetaBook extends CraftMetaItem implements BookMeta {
	...
	CraftMetaBook(net.minecraft.nbt.NBTTagCompound tag) {
		...
		if (tag.hasKey(BOOK_TITLE.NBT)) {
			title = tag.getString(BOOK_TITLE.NBT);
		}

		if (tag.hasKey(BOOK_AUTHOR.NBT)) {
			author = tag.getString(BOOK_AUTHOR.NBT);
		}
	...
	public void setPage(final int page, final String text) {
		...
		pages.set(page - 1,
				text == null ? "" : text.length() > MAX_PAGE_LENGTH ? text.substring(0, MAX_PAGE_LENGTH) : text);
	}
	...
	boolean equalsCommon(CraftMetaItem meta) {
		...
		if (meta instanceof CraftMetaBook) {
			...
			return (hasTitle() ? that.hasTitle() && title.equals(that.title) : !that.hasTitle())
					&& (hasAuthor() ? that.hasAuthor() && author.equals(that.author) : !that.hasAuthor())
					&& (hasPages() ? that.hasPages() && pages.equals(that.pages) : !that.hasPages());
		}
		return true;
	}
