...
final class CraftCriteria {
	...
	static {
		...
		for (Map.Entry<?, ?> entry : ((Map<?, ?>) net.minecraft.scoreboard.IScoreObjectiveCriteria.field_96643_a)
				.entrySet()) {
			String name = entry.getKey().toString();
			net.minecraft.scoreboard.IScoreObjectiveCriteria criteria = (net.minecraft.scoreboard.IScoreObjectiveCriteria) entry
					.getValue();
			if (!criteria.func_96636_a().equals(name))
				throw new AssertionError(
						"Unexpected entry " + name + " to criteria " + criteria + "(" + criteria.func_96636_a() + ")");
	...
	private CraftCriteria(net.minecraft.scoreboard.IScoreObjectiveCriteria criteria) {
		...
		bukkitName = criteria.func_96636_a();
	}
