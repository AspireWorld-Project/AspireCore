...
public class AutoUpdater {
	...
	private final List<String> onUpdate = new ArrayList<>();
	private final List<String> onBroken = new ArrayList<>();
		...
		new Thread() {
			...
			public void run() {
				current = service.getArtifact(currentSlug,
						"information about this CraftBukkit version; perhaps you are running a custom one?");
				...
				if (isUpdateAvailable()) {
					if (current.isBroken() && onBroken.contains(WARN_CONSOLE)) {
						...
						log.severe(
								"Your version of CraftBukkit is known to be broken. It is strongly advised that you update to a more recent version ASAP.");
						...
						log.severe("Newer version " + latest.getVersion() + " (build #" + latest.getBuildNumber()
								+ ") was released on " + latest.getCreated() + ".");
						...
						log.warning("Your version of CraftBukkit is out of date. Version " + latest.getVersion()
								+ " (build #" + latest.getBuildNumber() + ") was released on " + latest.getCreated()
								+ ".");
					...
					log.severe(
							"Unfortunately, there is not yet a newer version suitable for your server. We would advise you wait an hour or two, or try out a dev build.");
					log.severe("----- ------------------- -----");
					...
					if (prefChan != null && current.getChannel().getPriority() < prefChan.getPriority()) {
						...
						log.info("It appears that you're running a " + current.getChannel().getName()
								+ ", when you've specified in bukkit.yml that you prefer to run " + prefChan.getName()
								+ "s.");
						log.info("If you would like to be kept informed about new " + current.getChannel().getName()
								+ " releases, it is recommended that you change 'preferred-channel' in your bukkit.yml to '"
								+ current.getChannel().getSlug() + "'.");
						log.info(
								"With that set, you will be told whenever a new version is available for download, so that you can always keep up to date and secure with the latest fixes.");
						log.info(
								"If you would like to disable this warning, simply set 'suggest-channels' to false in bukkit.yml.");
						log.info("----- ------------------- -----");
					}
				}
			}
