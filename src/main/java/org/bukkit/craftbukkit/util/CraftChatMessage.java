...
public final class CraftChatMessage {
	private static class StringMessage {
		...
		private final List<net.minecraft.util.IChatComponent> list = new ArrayList<>();
		private net.minecraft.util.IChatComponent currentChatComponent = new net.minecraft.util.ChatComponentTranslation(
				""); // Cauldron - ChatComponentText -> ChatComponentTranslation. fixes forge
						// compatibility
		...
		private StringMessage(String message) {
			...
			while (matcher.find()) {
				...
				case 3:
					...
					modifier.setChatClickEvent(
							new net.minecraft.event.ClickEvent(net.minecraft.event.ClickEvent.Action.OPEN_URL, match)); // Should
																														// be
																														// setChatClickable
		...
		private void appendNewComponent(int index) {
			if (index <= currentIndex)
				return;
			net.minecraft.util.IChatComponent addition = new net.minecraft.util.ChatComponentText(
					message.substring(currentIndex, index)).setChatStyle(modifier);
			currentIndex = index;
