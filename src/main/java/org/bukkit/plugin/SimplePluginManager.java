...
public final class SimplePluginManager implements PluginManager {
		...
		for (File file : directory.listFiles()) {
			...
			try {
				...
				if (name.equalsIgnoreCase("bukkit") || name.equalsIgnoreCase("minecraft")
						...
						|| name.equalsIgnoreCase("spigot") || name.equalsIgnoreCase("forge")
						|| name.equalsIgnoreCase("cauldron") || name.equalsIgnoreCase("mcpc")) {
	...
	public synchronized Plugin loadInjectedPlugin(String pkg, PluginDescriptionFile description) {
		Plugin result = null;
		for (Pattern filter : fileAssociations.keySet()) {
			Matcher match = filter.matcher(".jar");
			if (match.find()) {
				JavaPluginLoader loader = (JavaPluginLoader) fileAssociations.get(filter);
				result = loader.loadInjectedPlugin(pkg, description);
				break;
			}
		}
