...
public final class JavaPluginLoader implements PluginLoader {
	...
	private final Map<Method, Class<?>> handlerCache = new HashMap<>();
	...
	public Plugin loadInjectedPlugin(String pkg, PluginDescriptionFile description) {
		PluginClassLoader loader;
		try {
			loader = new InjectedPluginClassLoader(this, (LaunchClassLoader) getClass().getClassLoader(), pkg,
					description);
	...
	public Map<Class<? extends Event>, Set<RegisteredListener>> createRegisteredListeners(Listener listener,
		...
		server.getPluginManager().useTimings();
		...
		for (final Method method : methods) {
			...
			// eventSet.add(new TimedRegisteredListener(listener, executor, eh.priority(),
	...
	public InheritanceMap getGlobalInheritanceMap() {
		if (globalInheritanceMap == null) {
			...
			relocationsCurrent.put("net.minecraft.server",
					"net.minecraft.server." + PluginClassLoader.getNativeVersion());
						...
						.loadMappings(
								new BufferedReader(
										new InputStreamReader(this.getClass().getClassLoader()
												.getResourceAsStream("mappings/" + PluginClassLoader.getNativeVersion()
														+ "/cb2numpkg.srg"))),
								new MavenShade(relocationsCurrent), null, false);
			} catch (IOException ex) {
				ex.printStackTrace();
			...
			BufferedReader reader = new BufferedReader(new InputStreamReader(this.getClass().getClassLoader()
					.getResourceAsStream("mappings/" + PluginClassLoader.getNativeVersion() + "/nms.inheritmap")));
	...
	private EventExecutor makeASMEventExecutor(final Class<? extends Event> eventClass, Listener listener,
			Method method) {
		Class<?> cls = handlerCache.get(method);
		if (cls == null) {
			cls = ClassGenUtils.makeOneArgMethodDelegate(listener.getClass(), getUniqueHandlerName(method), method,
					IEventHandler.class);
	...
	private String getUniqueHandlerName(Method method) {
		return String.format("%s_%s_%s_%s", getClass().getName(), method.getDeclaringClass().getSimpleName(),
