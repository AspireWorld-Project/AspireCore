...
public class UMCommandMap extends SimpleCommandMap {
	...
	public boolean dispatch(CommandSender sender, String cmdLine) throws CommandException {
		return MinecraftServer.getServer().getCommandManager().executeCommand(BukkitUtil.toVanillaCommandSender(sender),
				cmdLine) != 0;
	}
	...
	public Command getCommand(String name) {
		IExtendedCommand cmd = ((CommandHandler) MinecraftServer.getServer().getCommandManager()).getRegistry()
				.get(name);
	...
	private static class BukkitCommand implements IExtendedCommand {
		...
		public String getCommandUsage(ICommandSender sender) {
			return bukkitCommand.getUsage() == null ? null
					: bukkitCommand.getUsage().replace("<command>", bukkitCommand.getLabel()).trim();
		}
