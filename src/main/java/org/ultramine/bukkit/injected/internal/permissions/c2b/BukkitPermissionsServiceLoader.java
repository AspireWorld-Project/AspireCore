...
public class BukkitPermissionsServiceLoader implements ServiceProviderLoader<Permissions> {
	...
	public void load(ServiceDelegate<Permissions> service) {
		@Nullable
		RegisteredServiceProvider<Permission> regPerms = server.getServicesManager().getRegistration(Permission.class);
		@Nullable
		RegisteredServiceProvider<Chat> regChat = server.getServicesManager().getRegistration(Chat.class);
		if (regPerms == null)
			return;
