...
public class BukkitEnumHelper {
	...
	public static World.Environment addBukkitEnvironment(int id, String name) {
		return EnumHelper.addEnum(World.Environment.class, name, new Class[] { Integer.TYPE },
				new Object[] { Integer.valueOf(id) });
	}

	public static WorldType addBukkitWorldType(String name) {
		WorldType worldType = EnumHelper.addEnum(WorldType.class, name, new Class[] { String.class },
				new Object[] { name });
	...
	public static EntityType addBukkitEntityType(String name, Class<? extends org.bukkit.entity.Entity> clazz,
			int typeId, boolean independent) {
		String entityType = name.replace("-", "_").toUpperCase();
		EntityType bukkitType = EnumHelper.addEnum(EntityType.class, entityType,
				new Class[] { String.class, Class.class, Integer.TYPE, Boolean.TYPE },
				new Object[] { name, clazz, typeId, independent });
	...
	// return EnumHelper.addEnum(org.bukkit.event.inventory.InventoryType.class, id,
	// new Class[]{Integer.TYPE, String.class}, new Object[]{size, id});
	// }
	// catch (Throwable e)
	// {
	// //if
	// (MinecraftServer.getServer().tileEntityConfig.enableTEInventoryWarning.getValue())
	// //{
	// log.warn("Could not create inventory type " + tileentity.getClass().getName()
	// + " Exception: " + e.toString());
	// log.warn("Could not determine default inventory size for type " +
	// tileentity.getClass().getName() + " using size of 9");
	// //}
	// return EnumHelper.addEnum(org.bukkit.event.inventory.InventoryType.class, id,
	// new Class[]{Integer.TYPE, String.class}, new Object[]{9, id});
	// }
	// }
}
