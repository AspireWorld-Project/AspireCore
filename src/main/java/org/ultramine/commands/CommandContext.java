...
public class CommandContext {
	...
	private CommandContext(IExtendedCommand command, ICommandSender sender, String[] args) {
		...
		argumentMap = new HashMap<>(args.length);
		actionName = "";
		lastArgumentNum = args.length - 1;
	}
	...
	public void sendMessage(ICommandSender to, EnumChatFormatting tplColor, EnumChatFormatting argsColor, String msg,
			Object... args) {
	...
	public void handleException(@Nullable Throwable throwable) {
		...
		if (throwable instanceof WrongUsageException) {
			...
			ChatComponentTranslation msg = new ChatComponentTranslation("commands.generic.usage",
					new ChatComponentTranslation(cmdEx.getMessage(), cmdEx.getErrorOjbects()));
	...
	public class Argument {
		...
		public Map<String, List<String>> asFlags(String... allowedArr) {
			Set<String> allowed = allowedArr.length == 0 ? null : new HashSet<>(Arrays.asList(allowedArr));
			Map<String, List<String>> map = new HashMap<>();
			String curFlag = null;
			List<String> curList = new ArrayList<>(0);
			for (String s : asStringArray()) {
				if (s.startsWith("-")) {
					if (curFlag != null) {
						map.put(curFlag, curList);
						curList = new ArrayList<>(0);
					}
