...
public class CommandRegistry {
	...
	private final Map<String, IExtendedCommand> commandMap = (Map) new MapWrapper<String, Object>(
			Maps.transformValues(nameInfos, NameInfo::getCurrent)) {
		@Override
	...
	private Iterable<String> getDefaultNames(IExtendedCommand command) {
		...
		return Iterables.concat(Collections.singletonList(command.getCommandName()), aliases);
	}

	private Iterable<String> getAllNames(IExtendedCommand command) {
		Iterable<String> names = getDefaultNames(command);
		return Iterables.concat(names, Iterables.transform(names, TranslitTable::translitENRU));
	}
	...
	public IExtendedCommand registerVanillaCommand(ICommand command) {
		...
		IExtendedCommand exCommand = new VanillaCommandWrapper(command,
				active != null ? active.getModId().toLowerCase() : "vanilla");
	...
	public void registerCommands(Class<?> cls) {
		...
		for (Method method : cls.getMethods()) {
			if (method.isAnnotationPresent(Command.class) && Modifier.isStatic(method.getModifiers())) {
				...
				HandlerBasedCommand.Builder builder = new HandlerBasedCommand.Builder(data.name(), data.group(),
						new MethodBasedCommandHandler(method)).setAliases(data.aliases())
								.setPermissions(data.permissions()).setUsableFromServer(data.isUsableFromServer());
	...
	public List<String> filterPossibleCommandsNames(ICommandSender sender, String filter) {
		...
		for (Map.Entry<String, IExtendedCommand> entry : commandMap.entrySet()) {
			if (CommandBase.doesStringStartWith(filter, entry.getKey())
	...
	public List<IExtendedCommand> getPossibleCommands(ICommandSender sender) {
		List<IExtendedCommand> result = new ArrayList<>();
