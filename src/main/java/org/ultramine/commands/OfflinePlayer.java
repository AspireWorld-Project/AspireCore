...
public class OfflinePlayer {
	...
	public OfflinePlayer(MinecraftServer server, PlayerData profile) {
		mgr = server.getConfigurationManager();
		data = profile;
	}
	...
	public void saveFakePlayer(EntityPlayerMP player) {
		...
		if (!player.getGameProfile().equals(data.getProfile()))
			throw new RuntimeException("Player not this! this:" + data.getProfile().getName() + " other:"
					+ player.getGameProfile().getName());
		if (getIfOnline() != null)
			throw new RuntimeException(
					"Can't save FakePlayer: real player is online! username:" + data.getProfile().getName());
