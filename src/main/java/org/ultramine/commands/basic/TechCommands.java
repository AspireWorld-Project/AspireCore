...
public class TechCommands {
	...
	@Command(name = "id", group = "technical", permissions = { "command.technical.id" }, syntax = { "<%id>" })
	...
	@Command(name = "uptime", aliases = { "ticks", "lagometer" }, group = "technical", permissions = {
			"command.technical.uptime" })
	public static void uptime(CommandContext ctx) {
		...
		ctx.sendMessage(DARK_GREEN, "command.uptime.msg.up", String.format("%dd %dh %dm %ds", uptime / (60 * 60 * 24),
				uptime / (60 * 60) % 24, uptime / 60 % 60, uptime % 60));
		ctx.sendMessage(load > 100 ? RED : DARK_GREEN, "command.uptime.msg.load", Integer.toString(load).concat("%"),
				peakloadcomp);
		ctx.sendMessage(tps < 15 ? RED : DARK_GREEN, "command.uptime.msg.tps", tps,
				Integer.toString((int) (tps / 20 * 100)).concat("%"));
	}

	@Command(name = "debuginfo", group = "technical", permissions = { "command.technical.debuginfo" }, syntax = { "",
			"[chunk]", "<world>", "<player>" })
	public static void debuginfo(CommandContext ctx) {
		if (ctx.getAction().equals("chunk")) {
			...
			WorldServer world = ctx.contains("world") ? ctx.get("world").asWorld()
					: ctx.getSenderAsPlayer().getServerForPlayer();
			ctx.sendMessage("World: %s, Dimension: %s", world.getWorldInfo().getWorldName(),
			...
			ctx.sendMessage("Location: [%s](%s, %s, %s)", player.dimension, MathHelper.floor_double(player.posX),
	...
	@Command(name = "memstat", group = "technical", permissions = { "command.technical.memstat" })
	...
	@Command(name = "multiworld", aliases = { "mw", "mv" }, group = "technical", permissions = {
			"command.technical.multiworld" }, syntax = { "[list]", "[import] <%file>", "[import] <%file> <%path>",
	...
	public static void multiworld(CommandContext ctx) {
		...
		if (ctx.getAction().equals("list")) {
			...
			for (WorldDescriptor desc : mw.getAllDescs()) {
				ctx.sendMessage(GOLD, "    - [%s](%s) - %s", desc.getDimension(), desc.getName(),
						worldStateColor(desc.getState()));
			}
			return;
		...
		if (ctx.getAction().equals("load")) {
			...
			Teleporter.tpNow(ctx.getSenderAsPlayer(), desc.getDimension(), world.getWorldInfo().getSpawnX(),
					world.getWorldInfo().getSpawnY(), world.getWorldInfo().getSpawnZ());
		}
	}
	...
	private static ChatComponentText worldStateColor(WorldState state) {
		...
		comp.getChatStyle().setColor(state == UNREGISTERED ? DARK_RED
				: state == HELD ? RED : state == AVAILABLE ? YELLOW : state == LOADED ? DARK_GREEN : WHITE);
		return comp;
	}
	...
	@Command(name = "countentity", group = "technical", permissions = { "command.technical.countentity" }, syntax = {
			"<int%radius>" })
	public static void countentity(CommandContext ctx) {
		...
		for (Entity ent : GenericIterableFactory.newCastingIterable(player.worldObj.loadedEntityList, Entity.class)) {
			if (!ent.isDead && ent.posX > minX && ent.posX < maxX && ent.posY > minY && ent.posY < maxY
					&& ent.posZ > minZ && ent.posZ < maxZ) {
				count++;
	...
	@Command(name = "clearentity", group = "technical", permissions = { "command.technical.clearentity" }, syntax = {
			"<int%radius>", "[all mobs items] <int%radius>" })
	...
	@Command(name = "startlags", group = "technical", permissions = { "command.technical.startlags" }, syntax = {
			"<int%percent>", "[stop]" })
	...
	public static class LagGenerator {
		...
		public LagGenerator(int percent) {
	...
	@Command(name = "restart", group = "technical", permissions = { "command.technical.restart" }, syntax = { "[abort]",
			"<time>" })
	...
	@Command(name = "javagc", group = "technical", permissions = { "command.technical.javagc" }, syntax = { "" })
	public static void javagc(CommandContext ctx) {
		System.gc();
		ctx.sendMessage("command.javagc.success");
	}

	@Command(name = "chunkgc", group = "technical", permissions = { "command.technical.chunkgc" }, syntax = { "" })
	...
	@Command(name = "chunkdebug", group = "technical", permissions = { "command.technical.chunkdebug" }, syntax = { "",
			"[start stop]", "[top]", "[top] <list average peak>", "[top] <list average peak> <%count>" })
		...
		if (ctx.getAction().equals("start")) {
			...
			if (!ChunkProfiler.instance().isEnabled()) {
				...
				FMLCommonHandler.instance().bus()
						.register(new ChunkDebugDelayedDisplay(ctx, act2.startsWith("p"), count));
			} else {
	...
	private static void printChunkDebugResults(CommandContext ctx, boolean peak, int count) {
		...
		for (int i = 0; i < Math.min(count, results.length); i++) {
			...
			ctx.sendMessage(GOLD, "    - [%s](%s, %s) -> %s%% (%s%%)", chunk.getDimension(), chunk.getChunkX() << 4,
					chunk.getChunkZ() << 4, chunk.getAverage() / 5000 / 100d, chunk.getPeak() / 5000 / 100d);
		}
	}
	...
	@SideOnly(Side.SERVER)
	@Command(name = "backup", group = "technical", permissions = { "command.technical.backup" }, syntax = {
			"[make now]", "[make now] <%world>...", "[list]", "[apply] <number>", "[apply] <number> <flags>..." })
	public static void backup(CommandContext ctx) {
		...
		if (ctx.getAction().equals("make") || ctx.getAction().equals("now")) {
			Collection<String> worlds = ctx.contains("world")
					? mw.resolveSaveDirs(Arrays.asList(ctx.get("world").asStringArray()))
					: mw.getDirsForBackup();
			...
			Map<String, List<String>> flags = ctx.contains("flags")
					? ctx.get("flags").asFlags("worlds", "noplayers", "temp", "restart")
					: new HashMap<>();
			...
			if (flags.containsKey("restart")) {
				for (EntityPlayerMP player : GenericIterableFactory.newCastingIterable(
						ctx.getServer().getConfigurationManager().playerEntityList, EntityPlayerMP.class)) {
					player.playerNetServerHandler.kickPlayerFromServer(
							"\u00a75Выполняется бэкап мира\n\u00a7dВы сможете войти через несколько минут");
				}
			}
	...
	@Command(name = "recipecache", group = "technical", permissions = { "command.technical.recipecache" }, syntax = {
			"[clear]" })
	...
	@SideOnly(Side.SERVER)
	@Command(name = "reloadcfg", group = "technical", permissions = { "command.technical.reloadcfg" }, syntax = { "" })
	...
	@Command(name = "entitylist", group = "technical", permissions = { "command.technical.entitylist" }, syntax = { "",
			"<world>" })
	...
	private static void printEntityInfo(CommandContext ctx, Entity ent) {
		ctx.sendMessage("[%s](%s, %s, %s) ID: %s, Class: %s, Entity: %s", ent.worldObj.provider.dimensionId, ent.posX,
				ent.posY, ent.posZ, ent.getEntityId(), ent.getClass(), ent);
	}

	@Command(name = "killentity", group = "technical", permissions = { "command.technical.killentity" }, syntax = {
			"<%id>" })
