...
public class VanillaCommands {
	...
	@Command(name = "help", group = "player", aliases = { "?" }, permissions = { "command.vanilla.help" }, syntax = {
			"", "<page>" })
	public static void help(CommandContext ctx) {
		@SuppressWarnings("unchecked")
		List<IExtendedCommand> cmds = ctx.getServer().getCommandManager().getPossibleCommands(ctx.getSender());
		int pages = cmds.size() / 10 + (cmds.size() % 10 == 0 ? 0 : 1);
		int page = ctx.contains("page")
				? ctx.get("page").asString().equals("all") ? -1 : ctx.get("page").asInt(1, pages) - 1
				: 0;
		...
		for (int i = start; i < limit; i++) {
			...
			ChatComponentTranslation usage = new ChatComponentTranslation(
					usageS != null ? usageS : '/' + cmd.getCommandName());
			usage.getChatStyle().setChatClickEvent(
					new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/" + cmd.getCommandName() + " "));
	...
	@Command(name = "tp", group = "vanilla", aliases = { "tppos", "tpposp", "tpto" }, permissions = {
			"command.vanilla.tp" }, syntax = { "<player%dst>", "<player%target> <player%dst>", "<%x> <%y> <%z>",
					"<world> <%x> <%y> <%z>", "<player%target> <%x> <%y> <%z>",
					"<player%target> <world> <%x> <%y> <%z>" })
	public static void tp(CommandContext context) {
		EntityPlayerMP target = context.contains("target") ? context.get("target").asPlayer()
		...
		if (context.contains("dst")) {
			...
			WorldServer world = context.contains("world") ? context.get("world").asWorld()
					: target.getServerForPlayer();
			...
			context.sendMessage("command.tp.success.coordinate", target.func_145748_c_(),
					world.getWorldInfo().getWorldName(), x, y, z);
		}
	}

	@Command(name = "difficulty", group = "vanilla", permissions = { "command.vanilla.difficulty" }, syntax = {
			"<list peaceful easy normal hard % difficulty>", "<world> <list peaceful easy normal hard % difficulty>" })
	...
	private static void sendMessage(ICommandSender from, ICommandSender to, String message) {
		...
		from.addChatMessage(new ChatComponentTranslation("command.msg.display.outgoing", to.func_145748_c_(), msg)
				.setChatStyle(new ChatStyle().setColor(GOLD)));
		to.addChatMessage(new ChatComponentTranslation("command.msg.display.incoming", from.func_145748_c_(), msg)
				.setChatStyle(new ChatStyle().setColor(GOLD)));

		if (from instanceof EntityPlayerMP) {
			((EntityPlayerMP) from).getData().core().setLastMessagedPlayer(to.getCommandSenderName());
		}
		if (to instanceof EntityPlayerMP) {
			((EntityPlayerMP) to).getData().core().setLastMessagedPlayer(from.getCommandSenderName());
		}
	}

	@Command(name = "msg", aliases = { "tell", "t", "w" }, group = "vanilla", permissions = {
			"command.vanilla.msg" }, syntax = { "<player> <%msg>..." })
	public static void msg(CommandContext ctx) {
		ICommandSender to = ctx.get("player").asString().equalsIgnoreCase("server") ? MinecraftServer.getServer()
				: ctx.get("player").asPlayer();
		sendMessage(ctx.getSender(), to, ctx.get("msg").asString());
	}

	@Command(name = "reply", aliases = { "r" }, group = "vanilla", permissions = { "command.vanilla.reply" }, syntax = {
			"<%msg>..." })
	public static void reply(CommandContext ctx) {
		...
		ICommandSender to = name.equalsIgnoreCase("server") ? MinecraftServer.getServer()
				: MinecraftServer.getServer().getConfigurationManager().func_152612_a(name);
		sendMessage(ctx.getSender(), to, ctx.get("msg").asString());
	}

	@Command(name = "weather", group = "vanilla", permissions = { "command.vanilla.weather" }, syntax = {
			"[clear sun rain thunder]", "[clear sun rain thunder] <nextchange>", "<world> [clear sun rain thunder]",
			"<world> [clear sun rain thunder] <nextchange>" })
	public static void weather(CommandContext ctx) {
		WorldServer world = ctx.contains("world") ? ctx.get("world").asWorld()
				: ctx.getSenderAsPlayer().getServerForPlayer();
		WorldInfo wi = world.getWorldInfo();
		int nextchange = ctx.contains("nextchange") ? ctx.get("nextchange").asInt(1, 1000000) * 20
				: (300 + world.rand.nextInt(600)) * 20;
	...
	@Command(name = "time", group = "vanilla", permissions = { "command.vanilla.time" }, syntax = {
			"<list day night %worldtime>", "[set add] <list day night %worldtime>",
			"<world> <list day night %worldtime>", "<world> [set add] <list day night %worldtime>", })
	...
	@Command(name = "gamerule", group = "vanilla", permissions = { "command.vanilla.gamerule" }, syntax = { "", "<key>",
			"<world> <key>", "<key> <value>", "<world> <key> <value>" })
	public static void gamerule(CommandContext ctx) {
		GameRules rules = (ctx.contains("world") ? ctx.get("world").asWorld()
				: ctx.getSenderAsPlayer().getServerForPlayer()).getGameRules();
