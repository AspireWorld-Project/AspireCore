...
public class DefaultCompleters {
	...
	public static List<String> item(String val, String[] args) {
		Iterable it = Iterables.concat(Item.itemRegistry.getKeys(),
				((FMLControlledNamespacedRegistry) Item.itemRegistry).getAliases().keySet());
	...
	public static List<String> block(String val, String[] args) {
		return filterCollection(val, Iterables.concat(Block.blockRegistry.getKeys(),
				((FMLControlledNamespacedRegistry) Block.blockRegistry).getAliases().keySet()));
	}
	...
	public static List<String> warp(String val, String[] args) {
		return filterCollection(val,
				MinecraftServer.getServer().getConfigurationManager().getDataLoader().getWarps().keySet());
	}
