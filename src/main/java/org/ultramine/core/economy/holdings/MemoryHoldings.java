...
public class MemoryHoldings implements Holdings {
	private static final AtomicLongFieldUpdater<MemoryHoldings> BALANCE_FIELD_UPDATER = AtomicLongFieldUpdater
			.newUpdater(MemoryHoldings.class, "balance_field");
	...
	public MemoryHoldings(@Nonnull Account account, @Nonnull Currency currency) {
		...
		factor = Math.pow(10, currency.getFractionalDigits());
	}
	...
	@Override
	public double withdraw(double amount, @Nullable String comment)
	...
	public double computeBalance(DoubleUnaryOperator operation, @Nullable String comment) {
		long ret = BALANCE_FIELD_UPDATER.updateAndGet(this,
				balance -> floor(operation.applyAsDouble(balance / factor) * factor));
		onHoldingsBalanceChange();
		return ret / factor;
	}
	...
	protected static long ceiling(double arg) {
		long i = (long) arg;
		return arg > i ? Math.addExact(i, 1) : i;
	}

	protected static long floor(double arg) {
		long i = (long) arg;
		return arg < i ? Math.subtractExact(i, 1) : i;
	}
