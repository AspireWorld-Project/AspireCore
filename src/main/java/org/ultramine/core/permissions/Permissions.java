...
public interface Permissions {
	...
	@Nonnull
	String getMeta(String world, String player, String key);
	...
	default boolean has(World world, GameProfile player, String permission) {
		return has(MinecraftServer.getServer().getMultiWorld().getDescByID(world.provider.dimensionId), player,
				permission);
	}

	default boolean has(ICommandSender player, String permission) {
		return !(player instanceof EntityPlayerMP)
				|| has(player.getEntityWorld(), ((EntityPlayerMP) player).getGameProfile(), permission);
	}
	...
	default @Nonnull String getMeta(World world, GameProfile player, String permission) {
		return getMeta(MinecraftServer.getServer().getMultiWorld().getDescByID(world.provider.dimensionId), player,
				permission);
	}
