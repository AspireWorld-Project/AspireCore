...
public abstract class ServiceSwitchEvent extends Event {
	...
	public ServiceSwitchEvent(Class<?> serviceClass, ServiceDelegate<?> delegate,
			ServiceProviderLoader<?> oldProviderLoader, @Nonnull ServiceProviderLoader<?> newProviderLoader) {
	...
	public static class Pre extends ServiceSwitchEvent {
		public Pre(Class<?> serviceClass, ServiceDelegate<?> delegate, ServiceProviderLoader<?> oldProvider,
				@Nonnull ServiceProviderLoader<?> newProvider) {
			super(serviceClass, delegate, oldProvider, newProvider);
		}
	}

	public static class Post extends ServiceSwitchEvent {
		public Post(Class<?> serviceClass, ServiceDelegate<?> delegate, ServiceProviderLoader<?> oldProvider,
				@Nonnull ServiceProviderLoader<?> newProvider) {
			super(serviceClass, delegate, oldProvider, newProvider);
		}
	}
