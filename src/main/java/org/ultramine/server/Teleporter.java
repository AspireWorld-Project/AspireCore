...
public class Teleporter {
	...
	private static final LinkedList<Teleporter> teleporters = new LinkedList<>();

	public static void tpNow(EntityPlayerMP target, EntityPlayerMP dst) {
		if (target == null || dst == null)
			return;
		doTeleportation(target, dst.worldObj.provider.dimensionId, dst.posX, dst.posY, dst.posZ, dst.rotationYaw,
				dst.rotationPitch);
	}
	...
	public static void tpNow(EntityPlayerMP target, double x, double y, double z) {
		if (target == null)
			return;
		doTeleportation(target, target.worldObj.provider.dimensionId, x, y, z, target.rotationYaw,
	...
	public static void tpLater(EntityPlayerMP target, WarpLocation dst) {
		...
		if (timeto > 0 && !target.hasPermission("admin.abilities.skipteleportcooldown")) {
			target.addChatMessage(new ChatComponentTranslation("ultramine.teleporter.fail.cooldownd", timeto / 1000)
	...
	private static void doTeleportation(EntityPlayerMP player, int dimension, double x, double y, double z, float yaw,
			float pitch) {
		if (player.ridingEntity != null) {
			player.addChatMessage(new ChatComponentTranslation("ultramine.teleporter.fail.riding")
					.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
			return;
		}
	...
	private Teleporter(EntityPlayerMP target, WarpLocation dst) {
		...
		target.addChatMessage(new ChatComponentTranslation("ultramine.teleporter.delay", delay)
	...
	public void cancel() {
		...
		target.getData().core().setTeleporter(null);
		target.addChatMessage(new ChatComponentTranslation("ultramine.teleporter.canceled")
				.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
