...
public class WorldBorder {
	...
	public ChunkPosition correctPosition(int x, int z) {
		...
		for (OneBorder brd : borders) {
			int nmin = Math.min(Math.abs(brd.minX - x), Math.abs(brd.maxX - x))
					* Math.min(Math.abs(brd.minZ - z), Math.abs(brd.maxZ - z));
			if (nmin < min) {
				min = nmin;
				found = brd;
			}
	...
	private static class OneBorder {
		...
		public OneBorder(int x, int z, int radius, boolean round) {
			...
			definiteSquare = (int) Math.sqrt(.5 * radiusSquared);
		}
		...
		public ChunkPosition correctPosition(int xLoc, int zLoc) {
			if (!round) {
				...
				double dT = Math.sqrt((dX * dX + dZ * dZ) / radiusSquared); // distance of the transformed point from
																			// the center
