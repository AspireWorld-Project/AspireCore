...
public class ComputeFramesClassWriter extends ClassWriter {
	private static final LaunchClassLoader CLASSLOADER = (LaunchClassLoader) ComputeFramesClassWriter.class
			.getClassLoader();
	...
	protected String getCommonSuperClass(String type1, String type2) {
		...
		if (node1 != null && (node1.getAccess() & Opcodes.ACC_INTERFACE) != 0
				|| node2 != null && (node2.getAccess() & Opcodes.ACC_INTERFACE) != 0)
			return JAVA_LANG_OBJECT;
	...
	private static ClassReader getClassNode(String name) {
		try {
			byte[] classBytes = ClassPatchManager.INSTANCE.getPatchedResource(
					UltraminePlugin.isObfEnv ? FMLDeobfuscatingRemapper.INSTANCE.unmap(name) : name,
