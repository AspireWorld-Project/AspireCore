...
public class UMTBatchTransformer implements IClassTransformer {
	...
	public byte[] transform(String name, String transformedName, byte[] basicClass) {
		...
		int flags = 0;
		for (IUMClassTransformer transformer : specialTransformers.getOrDefault(transformedName,
		...
		// Computing frames even if we did not changed class to fix other mod changes of
		// 1.7 & 1.8 classes
	...
	public enum TransformResult {
		...
		MODIFIED,
		/**
		 * ClassNode modified and stack changed, stack max (and frames, of class version
		 * > 1.6) will be recounted
		 */
		MODIFIED_STACK
	}
	...
	public interface IUMClassTransformer {
		@Nonnull
		TransformResult transform(String name, String transformedName, ClassReader classReader, ClassNode classNode);
	}
}
