...
public class ServiceInjectionTransformer implements IUMClassTransformer {
		...
		if (fieldsToInject.size() > 0) {
			for (FieldNode field : fieldsToInject) {
				if ((field.access & Opcodes.ACC_STATIC) == 0)
					throw new RuntimeException("Service injection for non-static fields is not supported: "
							+ classNode.name + "#" + field.name);
				if ((field.access & Opcodes.ACC_FINAL) != 0)
					throw new RuntimeException("Service injection for final fields is not supported: " + classNode.name
							+ "#" + field.name);
			}
	...
	private static InsnList buildInjectorFor(String owner, FieldNode field) {
		...
		list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, SBA_CLASS, "provideService",
