			...
			for (ListIterator<AbstractInsnNode> it = m.instructions.iterator(); it.hasNext();) {
				...
				if (insnNode.getType() == AbstractInsnNode.METHOD_INSN) {
					...
					if (MATH_TYPE.equals(mi.owner) && ATAN2_NAME.equals(mi.name) && ATAN2_DESC.equals(mi.desc)
						...
						log.trace("Method {}.{}{}: Replacing INVOKESTATIC Math.atan2 with INVOKESTATIC TrigMath.atan2",
								name, m.name, m.desc);
						it.remove();
					...
					if (MATH_TYPE.equals(mi.owner) && ATAN_NAME.equals(mi.name) && ATAN_DESC.equals(mi.desc)
							&& mi.getOpcode() == Opcodes.INVOKESTATIC) {
						log.trace("Method {}.{}{}: Replacing INVOKESTATIC Math.atan with INVOKESTATIC TrigMath.atan",
								name, m.name, m.desc);
						it.remove();
