...
public class UMConsoleLayout extends AbstractStringLayout {
	...
	/**
	 * simplifies redundant sequence, for example &4&5&5_&r&6&5_ will be simplified
	 * to &5__
	 */
	private static void simplifyControlSequences(StringBuilder sb, String msg) {
		...
		for (int i = 0, s = msg.length(); i < s; i++) {
			...
			if (c == '\u00a7') {
				...
				if (!isValidCode(code)
						|| i + 2 < s && msg.charAt(i + 1) == '\u00a7' && isResetCode(msg.charAt(i + 2))) {
					continue;
				}
				if (code != prevCode) {
					control(sb, code);
	...
	public static UMConsoleLayout createLayout(@PluginAttribute("charset") final String charsetName) {
		...
		return new UMConsoleLayout(Charsets.getSupportedCharset(
				overrideCharset != null && !overrideCharset.isEmpty() ? overrideCharset : charsetName));
	}
