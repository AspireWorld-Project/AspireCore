...
public class ChunkGC {
	...
	public void onTick() {
		...
		if (chunkCount > chunkLimit && /* timePassed > MIN_GC_INTERVAL && */ unloadQueueSize < MAX_UNLOAD_QUEUE_SIZE
				&& (minChunkDiff == 0 || chunkDiff > minChunkDiff)) {
	...
	public void forceCollect() {
		...
		world.func_73046_m().getTickCounter();
		lastChunkCount = provider.chunkMap.size() - provider.unloadQueue.size();
	}
	...
	private List<Chunk> findChunksForUnload() {
		...
		List<Chunk> unbound = new ArrayList<>(Math.max(2, all.size() - boundChunks));
		for (Chunk chunk : all) {
	...
	private static class ChunkComparator implements Comparator<Chunk> {
		...
		public int compare(Chunk c1, Chunk c2) {
			float c = (float) Math.max(curTime - c1.getLoadTime(), _10_MINUTES)
					/ (float) Math.max(curTime - c1.getUnbindTime(), 1)
					- (float) Math.max(curTime - c2.getLoadTime(), _10_MINUTES)
