...
public class UnsafeChunkAlloc implements ChunkAllocService {
	...
	private static final int SLOT_LIMIT = Integer.parseInt(
			System.getProperty("org.ultramine.chunk.alloc.offheap.memlimit", "6")) * (1024 * 1024 * 1024 / SLOT_SIZE);
	// We delaying freeing not only for caching, but to be sure that nobody access
	// this memory from other threads through data races
	private static final int SLOT_FREE_DELAY = 5000;
	private static final boolean USE_8_LAYOUT = System.getProperty("org.ultramine.chunk.alloc.layout", "7").equals("8"); // false
																															// by
																															// default

	private final LongFunction<MemSlot> slotFactory = USE_8_LAYOUT ? pointer -> new Unsafe8MemSlot(this, pointer)
