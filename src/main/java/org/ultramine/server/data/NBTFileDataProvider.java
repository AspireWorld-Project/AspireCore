...
public class NBTFileDataProvider implements IDataProvider {
	...
	public NBTTagCompound loadPlayer(int dim, GameProfile player) {
		return loadPlayer((SaveHandler) mgr.getServerInstance().getMultiWorld().getWorldByID(dim).getSaveHandler(),
				player);
	}
	...
	public void savePlayer(int dim, GameProfile player, NBTTagCompound nbt) {
		savePlayer((SaveHandler) mgr.getServerInstance().getMultiWorld().getWorldByID(dim).getSaveHandler(), player,
				nbt);
	}
