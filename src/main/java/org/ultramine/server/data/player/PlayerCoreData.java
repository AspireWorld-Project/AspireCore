...
public class PlayerCoreData extends PlayerDataExtension {
	...
	private final Map<String, WarpLocation> homes = new HashMap<>();
	...
	public PlayerCoreData(PlayerData data) {
		super(data);
		account = new PlayerAccountHolder(data);
	}
	...
	public void mute(long time) {
		if (time == Long.MAX_VALUE) {
			unmuteTime = Long.MAX_VALUE;
		} else {
			unmuteTime = System.currentTimeMillis() + time;
		}
	}
		...
		public UMIntegratedPlayerHoldings getHoldings(Currency currency) {
			...
			if (val instanceof UMIntegratedPlayerHoldings) {
				...
				if (holdings.getCurrency() != currency) {
					UMIntegratedPlayerHoldings newHoldings = new UMIntegratedPlayerHoldings(holdings.getAccount(),
							holdings.getCurrency(), data);
