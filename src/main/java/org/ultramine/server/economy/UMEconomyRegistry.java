...
public class UMEconomyRegistry implements EconomyRegistry {
	...
	@Nonnull
	@Override
	public UndoableValue<Currency> registerCurrency(@Nonnull String id, @Nonnull String name,
			@Nonnull String pluralName, @Nonnull String sign, int fractionalDigits, int priority) {
	...
	private UndoableValue<Currency> registerCurrencyInternal(@Nonnull String id, @Nonnull String name,
			@Nonnull String pluralName, @Nonnull String sign, int fractionalDigits, int priority,
		...
		Undoable undoService = services.register(DefaultCurrencyService.class,
				new DefaultCurrencyServiceProvider(currency), priority);
		return UndoableValue.of(currency, Undoable.ofAll(undoService,
				() -> currencies.computeIfPresent(id, (s, currency1) -> currency1 == currency ? null : currency1)));
	}
		...
		startPlayerBalances.compute(currency, (s, currency1) -> {
			if (currency1 != null)
				throw new IllegalStateException(
						"Start player balance for currency with id: " + s + " is already registered");
			return balance;
		});
