...
public class ServerWorldEventProxy extends WorldEventProxy {
	...
	public GameProfile getObjectOwner() {
		...
		if (type == WorldUpdateObjectType.BLOCK_EVENT || type == WorldUpdateObjectType.BLOCK_PENDING
				|| type == WorldUpdateObjectType.BLOCK_RANDOM) {
	...
	public boolean canChangeBlock(int x, int y, int z, Block block, int meta, int flags) {
		...
		if (block != Blocks.air) {
			...
			if (type == WorldUpdateObjectType.TILEE_ENTITY) {
				...
				if (tile.xCoord == x && tile.yCoord == y && tile.zCoord == z)
					return true;
			} else if (type == WorldUpdateObjectType.BLOCK_RANDOM || type == WorldUpdateObjectType.BLOCK_PENDING
					|| type == WorldUpdateObjectType.BLOCK_EVENT) {
		...
		boolean isNeighborItself = isNeighbor
				&& neighborStack.get(neighborStack.size() - 1) == ChunkHash.blockCoordToHash(x, y, z);

		return !MinecraftForge.EVENT_BUS
				.post(new SetBlockEvent(world, x, y, z, block, meta, object, isNeighbor, isNeighborItself));
	}
}
