...
public class UMEventHandler {
	...
	public void checkChatPermission(ServerChatEvent e) {
		...
		if (data.isMuted()) {
			...
			if (data.getUnmuteTime() != Long.MAX_VALUE) {
				...
				e.player.addChatMessage(new ChatComponentTranslation("ultramine.ability.chat.muted.forever")
						.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
			}
		} else if (e.player.isHidden()) {
			e.setCanceled(true);
			e.player.addChatMessage(new ChatComponentTranslation("ultramine.hidden.chat")
					.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
		}
	}
	...
	public void onServerCommand(CommandEvent e) {
		if (e.sender instanceof EntityPlayerMP) {
			...
			if (player.playerNetServerHandler != null && player.getData() != null
					&& player.getData().core().isCommandsMuted()) {
				e.setCanceled(true);
				if (player.getData().core().getUnmuteTime() != Long.MAX_VALUE) {
					player.addChatMessage(new ChatComponentTranslation("ultramine.ability.command.muted",
							BasicTypeFormatter.formatTime(
									player.getData().core().getUnmuteTime() - System.currentTimeMillis(), true))
											.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
				} else {
					player.addChatMessage(new ChatComponentTranslation("ultramine.ability.command.muted.forever")
							.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
				}
			}
		}
	...
	public void onServerTickServer(TickEvent.ServerTickEvent e) {
		if (e.phase == TickEvent.Phase.START) {
			...
			if (cfg.enabled && server.getTickCounter() % (cfg.intervalSeconds * 20) == 0) {
				...
				ChatComponentTranslation full = new ChatComponentTranslation("ultramine.autobroadcast.debugmsg",
						loadcomp, peakloadcomp, tpscomp, server.getConfigurationManager().playerEntityList.size());
				full.getChatStyle().setColor(YELLOW);

				server.addChatMessage(full);
				for (EntityPlayerMP player : GenericIterableFactory
						.newCastingIterable(server.getConfigurationManager().playerEntityList, EntityPlayerMP.class))
			...
			if (msgcfg.enabled && server.getTickCounter() % (msgcfg.intervalSeconds * 20) == 0) {
				if (msgcfg.messages.length != 0) {
					if (msgcfg.showAllMessages) {
						...
						broadcastMessage(msgcfg.messages[server.getTickCounter()
								% (msgcfg.intervalSeconds * 20 * msgcfg.messages.length)
								/ (msgcfg.intervalSeconds * 20)]);
					}
				}
			}
		}
	...
	public void onPlayerTickServer(TickEvent.PlayerTickEvent e) {
		if (e.phase == TickEvent.Phase.END && e.side.isServer()) {
			...
			if (!player.getServerForPlayer().getBorder().isInsideBorder(x, z)) {
				...
				player.playerNetServerHandler.setPlayerLocation(pos.chunkPosX, player.lastTickPosY, pos.chunkPosZ,
	...
	public void onPlayerClone(PlayerEvent.Clone e) {
		if (e.entityPlayer.isEntityPlayerMP()) {
			...
			((EntityPlayerMP) e.entityPlayer).setStatisticsFile(
					MinecraftServer.getServer().getConfigurationManager().func_152602_a(e.entityPlayer));
		}
	}
	...
	public void onPlayerChangedDimension(PlayerChangedDimensionEvent e) {
		MinecraftServer.getServer().getConfigurationManager().getDataLoader()
				.handlePlayerDimensionChange((EntityPlayerMP) e.player, e.fromDim, e.toDim);
	}
