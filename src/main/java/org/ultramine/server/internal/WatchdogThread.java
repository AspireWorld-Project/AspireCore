...
public class WatchdogThread extends Thread {
	...
	public void run() {
		while (!stopping) {
			//
			if (lastTick != 0 && System.currentTimeMillis() > lastTick
					+ ConfigurationHandler.getServerConfig().settings.watchdogThread.timeout * 1000) {
	...
	private static void displayThreadInfo(ThreadInfo thread) {
		if (thread.getThreadState() != State.WAITING) {
			...
			log.log(Level.FATAL, "\tPID: " + thread.getThreadId() + " | Suspended: " + thread.isSuspended()
