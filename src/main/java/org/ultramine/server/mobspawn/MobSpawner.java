...
public abstract class MobSpawner {
	...
	/**
	 * @return true, если больше не следует предпринимать попыток спавнить мобов в
	 *         данном чанке
	 */
	...
	protected boolean isApplicableForSpawn(EnumCreatureType type, int x, int y, int z) {
		...
		return !block.isNormalCube()
				&& (block.getMaterial() == type.getCreatureMaterial()
						|| block == Blocks.snow_layer && type == EnumCreatureType.monster)
