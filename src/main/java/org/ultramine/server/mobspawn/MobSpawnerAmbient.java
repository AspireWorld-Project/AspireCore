...
public class MobSpawnerAmbient extends MobSpawner {
	...
	protected void rebuildList() {
		for (EntityPlayerMP player : GenericIterableFactory.newCastingIterable(world.playerEntities,
			...
			for (int i = 0; i < 12; i++) {
				...
				if (world.countEntitiesByType(type, x >> 4, z >> 4, set.localCheckRadius) < getLocalLimit()
