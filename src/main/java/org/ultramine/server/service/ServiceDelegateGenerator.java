...
public class ServiceDelegateGenerator {
	...
	private static final String NotImplementedServiceProvider_INTERNAL_NAME = Type
			.getInternalName(NotResolvedServiceProvider.class);
	...
	public static byte[] makeServiceDelegate(String name, Class<?> iface) {
		...
		cw.visit(V1_5, ACC_PUBLIC | ACC_SUPER, thisClassInternalName, null, "java/lang/Object",
				new String[] { ifaceInternalName, ServiceDelegate_INTERNAL_NAME });
		...
		for (Method method : iface.getDeclaredMethods()) {
			...
			mv.visitMethodInsn(INVOKEINTERFACE, ifaceInternalName, method.getName(), Type.getMethodDescriptor(method),
					true);
	...
	private static int loadInsnForType(Class<?> cls) {
		if (cls == boolean.class || cls == byte.class || cls == short.class || cls == int.class)
			return ILOAD;
		if (cls == long.class)
			return LLOAD;
		if (cls == float.class)
