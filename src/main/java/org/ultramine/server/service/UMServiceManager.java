...
public class UMServiceManager implements ServiceManager {
	...
	private @Nonnull <T> ServiceWrapper<T> getOrCreateService(Class<T> serviceClass1) {
		return ((Map<Class<T>, ServiceWrapper<T>>) (Object) services).computeIfAbsent(serviceClass1, serviceClass -> {
			...
			try {
				delegate = ServiceDelegateGenerator
						.makeServiceDelegate(getClass(), serviceClass.getSimpleName() + "_delegate", serviceClass)
						.newInstance();
				notResolvedProvider = ServiceDelegateGenerator.makeNotResolvedServiceProvider(getClass(),
						serviceClass.getSimpleName() + "_notResolvedProvider", serviceClass).newInstance();
			} catch (InstantiationException | IllegalAccessException e) {
				throw new RuntimeException(e);
			}
			return new ServiceWrapper<>(serviceClass, delegate, desc, new ServiceProviderRegistration<>(
					new SimpleServiceProviderLoader<>(notResolvedProvider), Integer.MIN_VALUE));
		});
	}
	...
	@Override
	public <T> Undoable register(@Nonnull Class<T> serviceClass, @Nonnull ServiceProviderLoader<T> providerLoader,
			int priority) {
		serviceClass.getClass(); // NPE
	...
	private static class ServiceWrapper<T> {
		...
		public ServiceWrapper(Class<T> serviceClass, ServiceDelegate<T> delegate, Service desc,
				ServiceProviderRegistration<T> notResolvedRegistration) {
		...
		private void switchTo(ServiceProviderRegistration<T> newProvider) {
			...
			MinecraftForge.EVENT_BUS.post(new ServiceSwitchEvent.Pre(serviceClass, delegate,
					oldProvider == null ? null : oldProvider.providerLoader, newProvider.providerLoader));
			if (oldProvider != null) {
			...
			MinecraftForge.EVENT_BUS.post(new ServiceSwitchEvent.Post(serviceClass, delegate,
					oldProvider == null ? null : oldProvider.providerLoader, newProvider.providerLoader));
		}
		...
		private void forceSwitchToMostRelevant() {
			if (providers.isEmpty()) {
				...
				switchTo(providers.stream()
						.sorted(Comparator.comparingInt((ServiceProviderRegistration o) -> o.priority).reversed())
						.findFirst().get());
			}
		}
		...
		public synchronized Undoable addProvider(ServiceProviderRegistration<T> provider) {
			if (desc.singleProvider() && providers.size() != 0)
				throw new IllegalStateException("Tried to register second provider for single-impl service'"
						+ serviceClass.getName() + "'. First provider: " + providers.get(0).providerLoader
	...
	private static class SimpleServiceProviderLoader<T> implements ServiceProviderLoader<T> {
		...
		public String toString() {
			return "SimpleServiceProviderLoader{" + "provider=" + provider + '}';
		}
	}
}
