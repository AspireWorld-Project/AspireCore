...
public class ItemBlocker {
	...
	private final TIntObjectMap<PerWorldBlocker> map = new TIntObjectHashMap<>();
	private PerWorldBlocker global;

	public ItemBlocker() {
		storage = new File(ConfigurationHandler.getSettingDir(), "itemblocker.yml");
	}
	...
	public void onPlayerInteractEvent(PlayerInteractEvent e) {
		...
		if (set != null && set.useItem) {
			...
			if (e.entityPlayer.isEntityPlayerMP() && ((EntityPlayerMP) e.entityPlayer).playerNetServerHandler != null) {
				e.entityPlayer.addChatMessage(new ChatComponentTranslation("ultramine.tools.itemblocker.useitem")
		...
		set = getBlockingSettings(e.entityPlayer.dimension, Block.getIdFromBlock(e.world.getBlock(e.x, e.y, e.z)),
				e.world.getBlockMetadata(e.x, e.y, e.z));
		if (set != null && set.useBlock) {
			...
			if (e.entityPlayer.isEntityPlayerMP() && ((EntityPlayerMP) e.entityPlayer).playerNetServerHandler != null) {
				e.entityPlayer.addChatMessage(new ChatComponentTranslation("ultramine.tools.itemblocker.useblock")
						.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
			}
		}
	}
	...
	public void onEntityItemPickup(EntityItemPickupEvent e) {
		...
		if (set != null && set.rmItem) {
			...
			e.item.setDead();
			if (e.entityPlayer.isEntityPlayerMP() && ((EntityPlayerMP) e.entityPlayer).playerNetServerHandler != null) {
				e.entityPlayer.addChatMessage(new ChatComponentTranslation("ultramine.tools.itemblocker.rmblock")
						.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
			}
		}
	}
		...
		if (e.phase == TickEvent.Phase.END) {
			for (EntityPlayerMP player : GenericIterableFactory.newCastingIterable(
				...
				if (player.openContainer != player.inventoryContainer) {
					for (Slot slot : GenericIterableFactory.newCastingIterable(player.openContainer.inventorySlots,
							Slot.class)) {
						...
						if (is.getItem() == null) {// Fix for broken items
							slot.putStack(null);
							continue;
						}
						...
						if (set != null && set.rmItem) {
							slot.putStack(null);
							player.addChatMessage(new ChatComponentTranslation("ultramine.tools.itemblocker.rmblock")
									.setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
						}
					}
				}
	...
	public static class ItemBlockerSettings {
		...
		public Map<Integer, BlockingWorldList> worlds = new HashMap<>();
