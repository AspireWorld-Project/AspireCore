...
public class GlobalExecutors {
	private static final ExecutorService writing = Executors.newSingleThreadExecutor(
			new ThreadFactoryBuilder().setNameFormat("UM IO writing #%d").setDaemon(true).build());
	private static final ExecutorService cached = new ThreadPoolExecutor(2, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS,
	 ...
	 * единственный поток, т.к. при сохранениее не требуется наискорейшее выполнение
	 * задачи.
	 */
	...
	/**
	 * Executes tasks in main server thread. Executes synchronously, in
	 * {@code execute} method context, if it is invoked from main server thread, or
	 * on next tick otherwise.
	 */
