...
public class InventoryUtil {
	public static boolean isStacksEquals(ItemStack is1, ItemStack is2) {
		return is1 == null && is2 == null || is1 != null && is2 != null && is1.isItemEqual(is2)
				&& !(is1.stackTagCompound == null && is2.stackTagCompound != null)
				&& (is1.stackTagCompound == null || is1.stackTagCompound.equals(is2.stackTagCompound));
	}
	...
	public static ItemStack addItem(IInventory inv, ItemStack is, boolean checkValid, boolean markDirty) {
		...
		if (maxSize > 1) {
			for (int i = 0, s = inv.getSizeInventory(); i < s; i++) {
				ItemStack it = inv.getStackInSlot(i);
				if (it != null && it.stackSize < maxSize && isStacksEquals(is, it)
						&& (!checkValid || inv.isItemValidForSlot(i, is))) {
