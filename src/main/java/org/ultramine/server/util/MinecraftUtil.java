...
public class MinecraftUtil {
	...
	private static class LeavesPathFinder {
		...
		public boolean canLeavesStay() {
			...
			if (!world.checkChunksExist(bx - distance - 1, by - distance - 1, bz - distance - 1, bx + distance + 1,
					by + distance + 1, bz + distance + 1))
				return true;
		...
		private boolean recursivePathFind(int x, int y, int z, int depth) {
			...
			if (current == EMPTY) {
				...
				current = block.canSustainLeaves(world, bx + x, by + y, bz + z) ? WOOD
						: block.isLeaves(world, bx + x, by + y, bz + z) ? LEAVES : WALL;
			...
			if (recursivePathFind(x + 1, y, z, newDepth))
				return true;
