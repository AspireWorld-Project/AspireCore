...
public class WarpLocation {
	...
	public boolean equals(WarpLocation loc) {
		if (this == loc)
			return true;
		return Math.abs(x - loc.x) < 0.1 && Math.abs(y - loc.y) < 0.1 && Math.abs(z - loc.z) < 0.1;
	}
	...
	public static WarpLocation getFromPlayer(EntityPlayer player) {
		return new WarpLocation(player.dimension, Math.round(player.posX * 100) / 100.0,
				Math.round(player.posY * 100) / 100.0, Math.round(player.posZ * 100) / 100.0,
				Math.round(player.rotationYaw * 100) / 100.0F, Math.round(player.rotationPitch * 100) / 100.0F);
	}
}
