...
public class MultiWorld {
	...
	private final TIntObjectMap<WorldDescriptor> dimToWorldMap = new TIntObjectHashMap<>();
	private final Map<String, WorldDescriptor> nameToWorldMap = new HashMap<>();
	...
	void sendDimensionToAll(int dim, int pid) {
		...
		channel.writeAndFlush(new ForgeMessage.DimensionRegisterMessage(dim,
				provTranslt.containsKey(pid) ? provTranslt.get(pid) : pid));
	}
	...
	public void onPlayerLoggedIn(FMLNetworkEvent.ServerConnectionFromClientEvent event) {
		...
		channel.attr(FMLOutboundHandler.FML_MESSAGETARGETARGS)
				.set(event.manager.channel().attr(NetworkDispatcher.FML_DISPATCHER).get());
	...
	private void checkDuplicates(List<WorldConfig> list) {
		TIntObjectMap<WorldConfig> idMap = new TIntObjectHashMap<>();
		Map<String, WorldConfig> nameMap = new HashMap<>();

		for (WorldConfig config : list) {
			{
				WorldConfig prev = idMap.put(config.dimension, config);
				if (prev != null)
					throw new RuntimeException("Duplicate dimension ID in worlds.yml! Dimension: " + config.dimension
							+ ", Names: " + prev.name + " " + config.name);
			}

			if (config.name != null) {
				WorldConfig prev = nameMap.put(config.name, config);
				if (prev != null)
					throw new RuntimeException("Duplicate world names in worlds.yml! Name: " + prev.name
							+ ", Dimensions: " + prev.dimension + " " + config.dimension);
			}
		}
	}
	...
	private WorldDescriptor getOrCreateDescriptor(int dim, String name) {
		...
		if (desc == null) {
			desc = new WorldDescriptor(server, this,
					!server.isSinglePlayer() && ConfigurationHandler.getServerConfig().settings.other.splitWorldDirs,
					dim, name);
	...
	public WorldDescriptor makeTempWorld(String name, int dim) {
		if (getDescByID(dim) != null || DimensionManager.isDimensionRegistered(dim))
			throw new RuntimeException(
					"WorldDescriptor for dimension " + dim + " already registered (on making temp world)");
